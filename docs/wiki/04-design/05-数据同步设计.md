...
## 8.5 v3+ 架构演进（2026-09-17）

> 源文档：[`docs/design/数据同步功能/2026-09-17-云同步异步化与分级传输设计.md`](../../design/数据同步功能/2026-09-17-云同步异步化与分级传输设计.md) 与[实施计划](../../plans/数据同步功能/2026-09-17-云同步异步化与分级传输实施计划.md)。**数据层未变**（JSONL + 软删除 + 时间戳合并照旧）。

**分级传输**：按阈值（默认 1MB）分两条路 —— 阶段一（≤阈值）走完整同步流程，实测**18 秒**完成；阶段二（>阈值）由独立队列按 50MB 分批传，每批都是独立队列任务、批次之间给高优先级同步让路。依据：实测 68% 的文件只占 1.6% 的体积。

**四个必须同时成立的设计约束**（任一缺失都会出问题）：

| 约束 | 不做的后果 |
| --- | --- |
| 阶段二待传集合**从 size+mtime 差异重建**，不落盘 | 重启丢队列 / 重复上传数百 MB |
| 阶段一跳过的大文件**必须从 stage 的删除判定中排除** | 每跑一次阶段一就删掉一次阶段二的成果 |
| 阶段二用**精确 `git.add`**，不走 `stageAllChanges` 的 `add '.'` | 阶段一未提交的变化被卷进大文件提交 |
| mtime 比对用**容差**而非严格相等 | `utimesSync` 的舍入让"刚传完的文件"永远判为待传 —— 队列死循环（实测 131 批全传同一个文件） |

**超时语义**：`isPushTimeout` 由字符串匹配改为类型化（`PushTimeoutError`）；超时不再被报成失败（区分「排队中/执行中」与「已失败」）；push 预算 120s → 600s。

**死循环根治**：驱动源是 `autonomous-tick` cron（默认每 10 分钟）→ `handleEvolutionTick` → `driveConflictGoal`，且该调用位于 `isAutonomousEnabled()` 检查**之前**（无条件驱动）。落决外层超时 5 分钟 < 心跳间隔 10 分钟 —— 数学上必然重入。修法：`resolveInFlight` 守卫（含 30 分钟失效保护）。

**Agent 工具集**：`cloud_sync_read_file` / `resolve_sync_conflict`（**秒回**）/ `cloud_sync_git`（只读诊断）/ **`cloud_sync_push`（按需推指定路径，入 `WRITE_TOOL_NAMES` 需确认）** / `cloud_sync_now`。

**同步范围规则**：`syncExcludePatterns`（永不参与）与 `syncForceIncludePatterns`（无视阈值走阶段一），**排除优先**；**两处生效点缺一不可**（阶段一的 copy + 阶段二的 scan）。

**数据完整性事故（2026-09-16）**：第二台设备把远端从 918 个文件删到 22 个 —— 镜像删除安全阀的"二次确认"在自动化流程中失效（第一次挡下置位，1 分钟后的下一次同步即自动放行）。修复：布尔确认 → **待删集合指纹**，只有用户显式确认且指纹完全一致才放行；集合变化即失效。

## 9. 参考链接

- Workspace 云同步原始设计：`docs/design/数据同步功能/2026-09-05-工作空间云同步设计.md`
- 精简多设备同步方案（SQL Dump + JSONL 降级）：`docs/design/数据同步功能/2026-09-07-精简多设备同步方案.md`
- 同步策略深度分析（18 行分类矩阵 + 616MB→60MB 体积估算）：`docs/design/数据同步功能/2026-09-07-多设备数据同步策略分析.md`
- 同步策略架构反思（JSONL 低估 / Git squash 破坏 / 事件溯源路线 / 向量时钟）：`docs/design/数据同步功能/2026-09-07-同步策略反思.md`
- **v3+ 异步化与分级传输设计**：`docs/design/数据同步功能/2026-09-17-云同步异步化与分级传输设计.md`
- **v3+ 实施计划（含实测发现与验证结果）**：`docs/plans/数据同步功能/2026-09-17-云同步异步化与分级传输实施计划.md`
