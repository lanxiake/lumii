...
点击 Vault/Topic 右上角「图谱视图」切换：

```mermaid
graph TD
    W((Vault: work)) --> T1((Topic: 前端))
    W --> T2((Topic: AI 工程))
    T1 --> P1[React Hook 最佳实践.md]
    T1 --> P2[Vitest Mock 手册.md]
    T2 --> P3[Agent 编排模式.md]
    P1 -->|ref| P3
    P2 -->|ref| P3
    P3 -->|ref| P4[Shapley Value 归因.md]
```

- 节点颜色：Vault=accent，Topic=fg-2，Page=surface-raised + 边框按修改时间（越新越亮）
- 交互：点击节点跳转对应页面；拖拽固定节点；缩放支持滚轮 + 触控板。
...
| 动效尊重 | AA | 读 `prefers-reduced-motion`：开启时关闭过渡、关闭图谱动画、骨架屏不闪烁 |
