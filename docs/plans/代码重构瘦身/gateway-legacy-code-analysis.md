...
- LLM 请求：`createDirectStreamFn` 直连用户自行配置的 OpenAI 兼容端点（见 `packages/agent-runtime/src/llm/direct-stream.ts`）
- 生图：`RightAPIClient` + `RightCodesDrawClient` 直连上游（见 `apps/windows/src/main/agent-runtime/rightapi-image-client.ts`、`apps/windows/src/main/agent-runtime/right-codes-draw-client.ts`）
- Gateway 客户端、Node Mode Coordinator、设备配对服务等实际**从未在独立版启动**（见 `apps/windows/src/main/index.ts` 的注释 `// 不构造 apiClient / gatewayClient / nodeModeCoordinator / devicePairingService`）
...
**连带清理：**
- `packages/agent-runtime/package.json` — 移除 `"@mtbot/protocol": "workspace:*"` 依赖
- `apps/windows/package.json` — 移除 `"@mtbot/protocol": "workspace:*"` 依赖
- `apps/windows/electron.vite.config.ts` — externalize 列表移除 `'@mtbot/protocol'`
- `apps/windows/electron.vite.config.ts` — optimizeDeps.exclude 移除 `'@mtbot/protocol'`
- `apps/windows/electron.vite.config.ts` — resolve.alias 移除 `'@mtbot/protocol'` 映射
- pnpm-workspace.yaml — 如有 packages 枚举需同步移除
...
**连带清理：**
- `apps/windows/package.json` — 移除 `"@mtbot/client-sdk": "workspace:*"` 依赖
- `apps/windows/electron.vite.config.ts` — externalize 列表移除 `'@mtbot/client-sdk'`
- `apps/windows/electron.vite.config.ts` — optimizeDeps.exclude 移除 `'@mtbot/client-sdk'`
- `apps/windows/electron.vite.config.ts` — resolve.alias 移除 `'@mtbot/client-sdk'` 映射
...
#### 3.1.1 `packages/agent-runtime/src/llm/gateway-stream.ts` — **文件级可删除**

- 632 行，实现 `createGatewayStreamFn`（通过 `POST /v1/llm/stream` 走网关 SSE 代理）
- **运行时**：`bridge-instance-factory.ts` 中 `streamFnFactory` 虽仍传入 gateway 配置，但 `resolveModel` 固定返回 `streamFnKind: 'direct'`（见 `apps/windows/src/main/agent-runtime/bridge-instance-factory.ts`），且 `wrapStreamFn` 外层统一用 `buildLiveDirectStream()` 覆盖（见 `apps/windows/src/main/agent-runtime/bridge-instance-factory.ts`）。故**该函数在独立版从未被真正调用**。
...
#### 3.1.2 `packages/agent-runtime/src/llm/llm-error.ts` — **可移除 Gateway 专属错误码**
...
#### 3.1.3 `packages/agent-runtime/src/llm/model-router.ts` — **注释与 `resolve()` 分支需清理**
...
#### 3.1.4 `packages/agent-runtime/src/llm/index.ts` — 移除 gateway-stream 全部导出

#### 3.1.5 `packages/agent-runtime/src/__tests__/gateway-error.test.ts` — 整个删除

---

### 3.2 host-kit 装配层：Gateway 工厂分支

文件：`packages/agent-runtime/src/host-kit/stream-fn-factory.ts`
...
当前从 `llm/index.ts` re-export 了 `createGatewayStreamFn`、`DEFAULT_GATEWAY_STREAM_PATH`、`gatewayErrorFromHttpResponse`、`GatewayStreamConfig`、`GatewayStreamDiagnostic`、`StreamMetadata`、`GatewayLlmErrorDetail`、`AssistantMessageWithLlmError`。**删除上述全部 Gateway 专属导出**（见 `packages/agent-runtime/src/index.ts`）。

同文件 host-kit 部分：删除 `createGatewayStreamFnFactory` + `GatewayStreamFnFactoryConfig` 导出（见 `packages/agent-runtime/src/index.ts`）。

---

### 3.4 `packages/agent-runtime/dist/` — 旧编译产物

`packages/agent-runtime/dist/llm` 下的 `gateway-stream.d.ts / .js / .map` — **直接删除整个 dist/ 目录**（该包源码直出，不需要 dist；如仍保留则是历史遗留）。
...
#### 4.1.1 `apps/windows/src/main/config/types.ts`
...
#### 4.1.2 `apps/windows/config/server-config.json`

**整个文件可删除**（或移除 `gatewayUrl` 字段；但从注释 `"Offline placeholders only"` 可看整个文件是无意义占位）。

#### 4.1.3 `apps/windows/.env.example`
...
`apps/windows/src/main/agent-runtime/bridge-types.ts`：
...
| `callGateway?: (method, params)` | L62-L64 | **删除**（现已注入为 `async (_method, _params?) => {}` 空实现，见 `apps/windows/src/main/index.ts`） |
...
#### 4.5.1 `apps/windows/src/main/agent-runtime/gateway-image-client.ts` — **整个删除**

129 行，`generateImageViaGateway()` 向 `/v1/image/generate` POST。

#### 4.5.2 `apps/windows/src/main/agent-runtime/bridge-image-services.ts`
...
- `apps/windows/src/renderer/hooks/business/useSettings/useSettings.types.ts` — 定义了 `GatewayConfig { url, autoConnect, wsUrl? ... }` 接口与 `AllSettings.gateway: GatewayConfig` 字段
- `apps/windows/src/renderer/hooks/business/useSettings/useSettings.ts` — `DEFAULT_SETTINGS.gateway` 默认值
- `apps/windows/src/renderer/hooks/business/useSettings/useSettings.ts` — `load()` 中对 `base.gateway` 做云端 URL → 本地占位的替换 + 强制 `autoConnect:false`
- `apps/windows/src/renderer/hooks/business/useSettings/useSettings.ts` — `updateGateway()` 方法
- `apps/windows/src/renderer/hooks/business/useSettings/useSettings.ts` — 导出 `updateGateway`
...
| 文件 | 命中内容 | 建议 |
|------|---------|------|
| `docs/standards/dual-connection-architecture.md` | **整个文件讲 Gateway 双连接（UI Token + Node Token）**，所有架构图、代码清单、验证清单都指向旧时代 | **整个删除或重命名为 `.md.deprecated`** |
| `docs/standards/project-structure.md` | 列出 `gateway-client.ts`、`node-connection.ts`、`useGateway/`、`gateway-service.ts`、架构图 `A→C[网关客户端 Gateway Client]` | 更新：删除 Gateway 相关条目，重新映射主进程模块表 |
| `docs/standards/feature-development-standards.md` | L47、L143、L178 等同上 | 同步更新 |
| `docs/standards/improvements-summary.md` | L19 "hooks 列表新增 useGateway" | 更新 hooks 列表 |

### 6.4 `docs/plans/` 与 `docs/design/` 历史计划

| 文件 | 处理 |
|------|------|
| `docs/plans/%E5%A4%A7%E6%96%87%E4%BB%B6%E9%87%8D%E6%9E%84%E5%88%86%E6%9E%90%E5%A4%84%E7%90%86/code-location-index.md` | 引用 gateway 位置 → 更新或保持（历史文件） |
| `docs/plans/2026-08-05-ui-tech-refresh-client-implementation.md` | 历史计划，保留但不强制更新 |
| `docs/design/2026-08-14-channel-outbound-hub-design.md` | P5 "message 工具与 Gateway 耦合" L90、L236、L417、L454 — 多处注明 Gateway 遗留分支**已移除**（✅ 标注）。该文件本身是设计文档，保持为历史记录即可。 |
...
- `packages/agent-runtime/dist/llm` — 直接删除（源码直出项目不应保留 dist，如有 .gitignore 已忽略则可能无需手动删）
