...
- Hermes 参考实现（精确行号）：
  - CompressionCommitFence 完整实现（begin_commit/finish_commit/commit_in_flight/revoke_commit_admission）：conversation_compression.py:L457-L632
  - commit-in-flight 永不中断循环 + WARNING→ERROR 升级：conversation_compression.py:L1008-L1054
  - 失败冷却 + anti-thrash 失效保护（见 turn_context.py:L368 cooldown_active 参数）：turn_context.py:L397-L399
  - 尾部压力降级 Pass 4：context_compressor.py:L3613-L3688
