...
| 5 · 数据层规范 | 16 个 hook 迁移到 useQuery/useAsync；pages/components 中 169 处直调 electronAPI 收敛到 services | 大，可再细分 | 中 | **已完成**（2026-09-13）：5A hooks 2 迁移 / 14 豁免；5B services 收敛收官，渲染层非白名单直调归零（仅剩 agentRuntime 命令面白名单与按计划不动的 `pet/`）；5C 评估已落地三项最小第一步（useWikiPage 抽类型+命令助手、useSettings 瘦身、录屏确认刷新恢复），useSkillStore 按触发条件未动。详见 [05 文档 §七/§八/§九/§十](./05-数据层规范.md) |
| 6 · 消息列表虚拟化 | 消息行包一层 `.message-row`（保住 flex 对齐语义）+ 窗口化卸载离屏 DOM + 离屏行 `content-visibility`；附带 React key 稳定化与入场错峰迁移 | renderer 5 文件 + 1 测试 | 中 | **已完成**（2026-09-15，3 个提交）→ [实施计划](./06-消息列表虚拟化.md)。起因：perf 日志实证渲染进程 3 小时从 394MB 涨到 1737MB，73% 是 Blink 分配器碎片（`blinkTotal` 1GB vs 活跃 145MB），而 JS 堆只有 116MB —— 正解是少挂 DOM。经 /code-review 两轮复核共修掉 12 条真缺陷（含修复自身引入的 2 条），4 项列入"后续观察项"（非待办）。**09-16 实测复核生效**：10.5 小时内容涨 3 倍、RSS 只涨 3%、`blinkTotal` 持平（修复前 2.9 小时涨 161%）；同日暴露的 detached DevTools 与大会话首次挂载峰值不属本片，已登记在文档 §六之二。**待人工冒烟** |
