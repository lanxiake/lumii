...
| 6 · 消息列表虚拟化 | 消息行包一层 `.message-row`（保住 flex 对齐语义）+ 窗口化卸载离屏 DOM + 离屏行 `content-visibility`；附带 React key 稳定化与入场错峰迁移 | renderer 4 文件 + 1 测试 | 中 | **已完成**（2026-09-15）→ [实施计划](./06-消息列表虚拟化.md)。起因：perf 日志实证渲染进程 3 小时从 394MB 涨到 1737MB，73% 是 Blink 分配器碎片（`blinkTotal` 1GB vs 活跃 145MB），而 JS 堆只有 116MB —— 正解是少挂 DOM。待人工冒烟 |
