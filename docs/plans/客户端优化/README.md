...
| 5 · 数据层规范 | 16 个 hook 迁移到 useQuery/useAsync；pages/components 中 169 处直调 electronAPI 收敛到 services | 大，可再细分 | 中 | **已完成**（2026-09-13）：5A hooks 2 迁移 / 14 豁免；5B services 收敛收官，渲染层非白名单直调归零（仅剩 agentRuntime 命令面白名单与按计划不动的 `pet/`）；5C 四个大 hook 评估报告已出（含 useWikiPage 认知修正）。详见 [05 文档 §七/§八/§九](./05-数据层规范.md) |
