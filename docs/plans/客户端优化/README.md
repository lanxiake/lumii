...
| 2 · ui 组件库收敛 | 6 个零引用 ui 组件逐个复核后清理、桶导出处理、补 IconButton/Tabs 场景，按页替换标准按钮 | 组件层，可多次提交 | 低 | **已完成**（2026-09-12）→ [实施计划](./02-ui组件库收敛.md) |
| 3 · Wiki 样式归位 | `WikiTab.css`(2597 行全局样式) 按 13 个子组件切 module.css，消除 21 个文件 × 359 处字面类名耦合 | 大但机械 | 中 | 未开始 |
