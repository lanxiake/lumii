...
> 关联：`docs/plans/代码重构/客户端切片/`（renderer 侧切片计划，与本议题边界相邻但目标不同）
...
> 切分原则同 [客户端切片](../代码重构/客户端切片/README.md)：每片可独立提交、独立验证、可单独回滚。**启动时再为每片写实施计划。**
...
| 与「客户端切片」计划的边界重叠 | 两者都动 `apps/windows`，但前者动 renderer，本议题动 main 宿主边界 | 明确不同时并行推进；启动本议题时确认 renderer 侧无进行中的切片 |
