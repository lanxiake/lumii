...
- Hermes 参考实现（精确行号，移植时须对照）：
  - Per-Model 最长子串匹配：context_compressor.py:L1820-L1843
  - Idle 纯谓词 4 条件 AND：turn_context.py:L368-L400
  - Idle 实际触发：turn_context.py:L782-L851
  - ProgressFence 核心字段 / 方法：conversation_compression.py:L457-L505
  - 等待循环 + 动态 wait_slice + 续命：conversation_compression.py:L943-L973
