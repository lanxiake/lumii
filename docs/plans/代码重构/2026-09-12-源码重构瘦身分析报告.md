...
> 行数口径：**含空行与注释**（与 [`大文件与死代码/2026-08-21-大文件重构分析报告.md`](大文件与死代码/2026-08-21-大文件重构分析报告.md) 的"排除纯空行"口径不同，两者数值不可直接比较）
> 关联文档：[死代码清理清单](./大文件与死代码/dead-code-cleanup-list.md)、[大文件重构分析报告](大文件与死代码/2026-08-21-大文件重构分析报告.md)、[Gateway 遗留代码分析](./大文件与死代码/2026-08-20-Gateway遗留代码分析.md)
...
[`大文件重构分析报告`](大文件与死代码/2026-08-21-大文件重构分析报告.md)（2026-08-21）的方案本身是合理的，但**执行后缺少门禁，文件重新膨胀**：
...
> 明细与逐项复核记录见 [死代码清理清单](./大文件与死代码/dead-code-cleanup-list.md)。
...
完整的 138 个 >400 行文件分级与拆分建议，见 [大文件与死代码/code-location-index.md](大文件与死代码/code-location-index.md)。
...
- 删除零引用源文件（逐项按 [清单](./大文件与死代码/dead-code-cleanup-list.md) 人工确认）
...
沿用 [大文件重构分析报告](大文件与死代码/2026-08-21-大文件重构分析报告.md) 的方案，按收益/风险比重排：
...
| 删除"看似无用"的代码导致运行时缺失 | 所有删除项必须先在 [清单](./大文件与死代码/dead-code-cleanup-list.md) 中登记证据并复核 |
...
> B 级清单与复核证据见 [dead-code-cleanup-list.md](./大文件与死代码/dead-code-cleanup-list.md) 的 B 级章节。**B 级复核推翻了原分析的三处结论**（autonomous 6 模块、segment-memory-service、contracts 导出数量），详见该文档。
...
2. **A4 实际依赖簇比原清单更大** —— 除清单所列 6 个死组件外，`PipelineGraph.tsx`(185)、`pipeline-utils.ts`(135)、`PipelinesTab.module.css`(169)、`ScheduleTab.module.css`(84) 同样只被死代码引用；`useCron/types.ts` 的 `Pipeline` / `PipelineEdge` / `CronViewTab` 随之失效。详见 [清单](./大文件与死代码/dead-code-cleanup-list.md) 的 A4 章节。
