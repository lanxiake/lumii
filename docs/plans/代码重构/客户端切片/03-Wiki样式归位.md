...
> 创建：2026-09-12 · 状态：**已完成**（2026-09-12，9 个提交，见执行记录）
...
全部 8 个批次 + 1 个补漏修复完成，共 9 个提交（`a4ee775` → `82be7a8`）：

| 批次 | 提交 | 内容 | WikiTab.css 行数 |
|---|---|---|---|
| B0 | `a4ee775` | 死样式退场：58 类 68 条规则 + 4 处就地改写 | 2598 → 2102（净删 496） |
| B1 | `ea49187` | 导航簇（LeftNav/TopBar/MoreMenu/Breadcrumb） | 2102 → 1685 |
| B2 | `76600b9` | 文件列表簇 + 建立 wiki-shared.module.css | 1685 → 1180 |
| B3 | `21ff960` | 图谱 + 清理视图 | 1180 → 1013 |
| B4 | `16fafa4` | 任务中心 + 资料抽屉（@media 拆两块） | 1013 → 596 |
| B5 | `c3924de` | 编目/主题/迁移簇（4 个 module） | 596 → 135 |
| B6 | `788579b` | WikiTab 布局 11 类迁入 + **删除 WikiTab.css** | 135 → 0（文件删除） |
| B7 | `71a77aa` | HelpDrawer/SourceMeta 重命名转 module | — |
| 补漏 | `82be7a8` | WikiLeftNav「更多」按钮未转换修复（见下） | — |

**最终形态**：3 个全局 CSS 文件（2847 行 / 310 类）→ **19 个 module.css**；58 个死类名删除；20 个动态模板类改显式映射表（INBOX_STATUS_CLASS / TASK_PHASE_* / REASON_CLASS / MIGRATE_STATUS_CLASS / TASK_PILL_TONE_CLASS）；跨组件共享类与跨组件复合规则由 `wiki-shared.module.css` 承载。

### 执行期自查发现并修复的两个问题

1. **B2 配置漏列 5 个收件箱状态类**：`wiki-inbox-item-status--*` 未随 InboxPanel 迁移，映射表一度指向缺失类；B6 收尾扫描发现并 append 修复。
2. **B1 漏转「更多」按钮**：`WikiLeftNav` 「更多」入口的 className 条件写法（`active.kind === 'more'`）与常规项不同，批量替换未命中，且当次扫查模式（仅覆盖 `'` `"` 两种引号）漏检反引号模板；最终全仓复扫发现并修复。

**教训（供后续切片）**：批量替换后必须做**全覆盖模式**的残留扫描——覆盖 `"..."`、`'...'`、`` `...` ``、`{'...'}`、字符串拼接五种形式，不能只按常规写法扫。

### 验证汇总（每批执行）

- **等价性脚本**：迁移器内置逐类比对（声明文本 + 选择器 + 规则顺序），全部通过
- **残留扫描**：每批迁移文件零字面引用（白名单 = 19 处无样式类名，见 03a 第六节）
- **typecheck + build**：每批通过
- **待人工**：暗/亮主题视觉冒烟（Memories 页各区域：左栏/顶栏/文件列表/收件箱/任务中心/资料抽屉/图谱/清理/主题树/迁移预览/帮助抽屉）

### 遗留

- 19 处无样式类名（登记在 03a 第六节，未动）
- WikiFileList 的 `wiki-file-list-action-btn` 作为 `ui/Button` className 传递的用法保留（现有实现）
