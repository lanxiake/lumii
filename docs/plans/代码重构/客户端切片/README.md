...
| **8–14 · 主题色系收尾** | 08 令牌契约补齐 / 09 死回退值清理 / 10 语义色与中性色收敛 / 11 图表调色板 / 12 canvas 与宠物色层 / 13 上下文分类色板 / 14 文件类型色板 | 约 1850 处 / 136 文件，拆 7 片 | 低→高递增 | **规划中**（2026-09-20）→ [切片总览](./07-主题色系/README.md)。成因：第 7 片完成后用户决定"剩余零散硬编码单独立项处理"。**实测基线**：`var(--x,#hex)` 737 处 / 裸 hex 906 处 / `rgba()` 541 处。建议顺序 08→09→10→11→13→12→14 |
