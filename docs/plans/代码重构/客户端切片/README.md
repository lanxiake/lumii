...
| **7 · 主题色系** | 主题机制支持第三态 `eye-care` + 新增护眼主题块 + 亮色提饱和 + 清页面级 token 覆写 + RGB 变体按主题走 + 命名色收敛 + 主题块完整性守卫测试 | 主题机制/令牌/CSS，~30 文件（多为定点替换） | 中 | **规划中**（2026-09-20）→ [实施计划](./07-主题色系/实施计划.md)。起因：用户反馈亮色发淡 + 要护眼主题 + 换色系要只改变量。**核心病灶**：`DashboardPage.module.css:16-19` 页面内覆写 `--mt-accent-500` 使该页面对换色免疫；`--mt-accent-rgb` 全局单值使 `rgba(var(--mt-accent-rgb),…)` 不随主题变。本片只做契约层，~430 处零散硬编码色登记为后续切片 |
