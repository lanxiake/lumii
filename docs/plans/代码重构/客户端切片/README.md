...
> 关联：[源码重构瘦身分析报告](../2026-09-12-源码重构瘦身分析报告.md)（本计划是其「批次 2/3」在客户端一侧的重新排期）
...
| **7 · 主题色系** | 主题机制支持第三态 `eye-care` + 新增护眼主题块 + 亮色提饱和 + 清页面级 token 覆写 + RGB 变体按主题走 + 命名色收敛 + 主题块完整性守卫测试 | 主题机制/令牌/CSS，~30 文件（多为定点替换） | 中 | **已完成**（2026-09-20，3 个提交）→ [实施计划](./07-主题色系/实施计划.md)。起因：用户反馈亮色发淡 + 要护眼主题 + 换色系要只改变量。**核心病灶**：`DashboardPage.module.css:16-19` 页面内覆写 `--mt-accent-500` 使该页面对换色免疫；`--mt-accent-rgb` 全局单值使 `rgba(var(--mt-accent-rgb),…)` 不随主题变。本片只做契约层，零散硬编码色见下 |
| **8–14 · 主题色系收尾** | 08 令牌契约补齐 / 09 死回退值清理 / 10 语义色与中性色收敛 / 11 图表色板令牌化 / 12 canvas 与宠物色层 / 13 上下文分类色板 / 14 文件类型色板 | 实测 ~1200 处 / 90+ 文件，7 片 | 低→高递增 | **已完成**（2026-09-20，7 个提交：`8cda9e95` `29040b27` `a3ec05dd` `16ed0978` `b37765bf` `6efa03b5` `eb7c1801`）→ [切片总览](./07-主题色系/README.md)。**最终效果：换色系只需改 `design-system.css` 三个主题块里的变量。** 规划期的规模与判断普遍不准，七片实施中累计推翻十余处结论，各片末尾均有"执行记录"
