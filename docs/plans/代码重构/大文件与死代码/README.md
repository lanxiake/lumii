...
| [`2026-09-24-knip接入与死代码清理计划.md`](2026-09-24-knip接入与死代码清理计划.md) | **knip 接入**：模块图可达性 + 依赖卫生（与守卫互补，非替代）。含 1453 条 exports/types 的**按引用形态归类**、任务清单 K0–K5、逐条执行进度。**K0–K5 与 K4-2 均已做完（2026-09-24）**：exports/types **1380 → 0**、死依赖清零、4 个暴露出的死文件删除，棘轮预算收到 **2**；另记录了两条能力边界 —— knip 的三类假阳盲区（含 `@lintignore` 出口）与 `knip --fix` 为何不可用 |
