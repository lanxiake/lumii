...
| [`2026-09-24-knip接入与死代码清理计划.md`](2026-09-24-knip接入与死代码清理计划.md) | **knip 接入**：模块图可达性 + 依赖卫生（与守卫互补，非替代）。含 1453 条 exports/types 的**按引用形态归类**、任务清单 K0–K5、逐条执行进度。**K0–K5、K4-2 / K4-3 与 K6 均已做完（2026-09-24）**：exports/types **1380 → 0**、死依赖清零、4 个因此暴露的死文件删除，棘轮预算收到 **2** 并**已进 CI**（`ci.yml` 的 typecheck job）；K6 把 b 组 31 条逐条结清（**删 18 / 接线 7 / 待决策 6**）。另记录三条能力边界 —— knip 的三类假阳盲区（含 `@lintignore` 出口）、`knip --fix` 为何不可用、以及**新发现的第三条盲区：私有且零引用的声明**（`noUnusedLocals` 没开，apps/windows 一家 315 条，待决策） |
