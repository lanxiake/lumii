...
> 上游文档：[Lumii 源码重构瘦身分析报告](../2026-09-12-源码重构瘦身分析报告.md)
> 复核方法：`grep` 全仓引用统计 + 编译配置交叉验证；标注 ✅ 的条目已逐条人工复核
> 续篇：[接线守卫与无消费者 IPC 清单](2026-09-24-接线守卫与无消费者IPC清单.md)（2026-09-24，改用 AST 判定并接入 CI 门禁）
...
**验证结论**：`pnpm build` 通过；`@mtbot/agent-runtime` 1832 个用例全绿；`lumii-windows` 的 39 个失败用例经 stash 对照证明为**既有失败**（见 [README 执行记录](./README.md)）。
