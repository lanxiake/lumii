...
> 创建：2026-09-13
> 前置设计：[专项 Agent 设计（一等公民）](../../design/专项Agent/2026-09-13-专项Agent设计.md)（v3.1，§9 实施路线）
> 原则：小步快跑——每片可独立提交、独立验证、可单独回滚；完成一片结算一片（体例参考 [客户端切片](../代码重构/客户端切片/README.md)）。
...
| 07 | [新手指引（暂时不做）](./07-新手指引-暂时不做复杂且价值不大.md) | 功能目录与只读工具、向导体验、使用统计、主动推荐、会话内推荐、询问组件优化、指南应用内入口 | **v2 已修订**（评审 + 二次核实）后，2026-09-15 定为**暂时不做**（复杂且价值不大）；片 6「询问组件优化」代码已完成，其余片不开工 |
...
| 11 | [情报与维护深化](./11-情报与维护深化.md) | 归属与可见性、维护体检报告落库 + 概览页卡片、资讯卡片期刊化（每次推送 = 一期） | **P0 三项完成**；CK 套件（`docs/test/lumii-cli/agent-curation/`）真机已跑；待人工冒烟 |
