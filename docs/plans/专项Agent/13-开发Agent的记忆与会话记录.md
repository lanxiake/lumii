...
> 创建：2026-09-16
> 状态：**讨论稿 · 部分拍板 · B1 与 A2 已落地**（见 §七；A1 暂不做、A3 不推荐）
...
**B1 ✅ → A2 ✅（两件都已落地，见 §七）。A1 暂不做（已拍板，理由见文首），A3 不推荐。**

- **B1 先做**：这两个缺口里唯一「数据已经齐、只差组装」的，且它让开发会话的痕迹第一次
  完整留存。A2 若要落地，也因为 B1 才有据可查。
- **A2 已落地**（`93f61144`）：最小成本让「转交」这条路径闭环，风险低。它的窄覆盖是已知
  取舍，不是缺陷——转交恰恰是主助手主动派活的主路径。粒度（仅成功 + 按任务线去重）见 §七。
...
**顺序上的一个额外好处**：B1 先落地，让 A2 写记忆时"有据可依"——开发会话里能看到具体过程了。
原本还设想让 A2 带一个 `source_message_id` 指回具体回合，实际**没带**：查下来它虽有列有类型，
但渲染层没有任何消费方（记忆页只展示来源段与宫殿片段），带了是死数据。要做的话得先在 UI 上
给它一个出口。
...
| 5 | ~~A2 写记忆的粒度~~ | ✅ **已定并落地：仅成功 + 按任务线去重**（`93f61144`，见 §七） |
...
### A2 ✅ 转交完成时落一条开发记忆（提交 `93f61144`）

`reportToOriginSession` 收尾时，把「提案摘要 + 结果正文（压单行、截断 400 字）」记进
**code-dev 名下**的工作记忆（`project` 类、importance 0.6）。

记在 code-dev 名下是有意的：记事与维护的 `readView` 都是 `user`（跨 Agent 可读），所以
日报汇总得到、维护做跨 Agent 去重时也看得见——这正是缺口 A 里"那一块从来没有过输入"的补法。
而开发 Agent 自己的 `readView` 是缺省 `own`，这条它自己也读得到。

**粒度（拍板：仅成功 + 按任务线去重）**：

- **仅成功** —— 失败与中止不记。半成品进记忆库只会污染日报与维护的判据。
- **按开发会话去重** —— 一个开发会话就是一条任务线（复用会话即延续任务，见
  `findRecentCodeDevSession` 的语义），同会话的后续转交**覆盖**前一条，一个任务反复迭代
  不会灌进一串半成品。实现用 tags 存 `['handoff', devSessionKey]` 当 upsert 键
  （`updateMemory` 只改正文、tags 原样保留）；内容一字不差的重复触发另有
  `MemoryRepo.saveCandidate` 自身的幂等兜底。
- **写记忆跑在汇报的 try 之外** —— 它失败只记日志，不能把「汇报成功」变成「汇报失败」。

测试 5 例（成功落库含字段断言 / 同会话覆盖而非新增 / 失败不记 / 别的开发会话不误认 /
写失败不影响汇报）+ `buildHandoffMemoryContent` 3 例。

**未做**：端到端真机验证。跑通一次真实转交需要临时把 user-global 后端从 `lumii` 改成
`claude`（否则 `runDevHandoff` 的绑定预检会直接拒绝——那是 09-P3 特意加的守卫），会消耗
一次真实 ACP 任务并往记忆库写一条真数据。逻辑已由上述 8 例覆盖，端到端只增"接线在运行时
确实通"这一条证据，留待有真实转交时顺带观察。
