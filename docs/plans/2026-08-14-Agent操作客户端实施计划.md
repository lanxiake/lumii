...
**规格：** `docs/design/客户端UI/2026-08-13-Agent操作客户端设计.md`（v0.5，MVP 验收通过 + 二期代码核查修正版）
...
> **已拆出独立计划：** 请改读并执行  
> [`docs/plans/渠道与CLI/2026-08-15-CLI统一控制面实施计划.md`](./渠道与CLI/2026-08-15-CLI统一控制面实施计划.md)  
> （Task 1–8，含循环依赖规避、速率限制模块、完整验收表）。以下 Part D 正文保留作历史对照，**不要再按本节实现**。

> **规格：** 设计 v0.7 §14（`docs/design/客户端UI/2026-08-13-Agent操作客户端设计.md`）。  
