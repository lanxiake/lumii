...
### 6.2.2 动作元数据契约：按组声明的能力

`install-pet.mjs` 的 `ACTIONS` 表上现在有五个字段，用来声明**归一化判据量不出来的素材属性**。

**原则（硬规则 8）**：**凡是判据量不出来的，就必须由生产侧说。**

这些字段按归属分成三类契约：

#### 一、几何契约（`invertY`、`flipX`）

**素材的空间朝向**——是否需要垂直翻转、是否需要水平镜像。

**为什么必须显式声明**：翻转不改包围盒。归一化把内容底边对齐到锚点，装完之后翻与不翻的包围盒**一模一样**，量不出来。

**什么时候需要标记**：

| 字段 | 触发条件 | 实测案例 |
| --- | --- | --- |
| `invertY` | 天花板动作（`Crawl`）必须倒挂：脚朝上、头朝下 | 月兔小仙的 `Crawl`（腾云驾雾）：H3 出的是正的（`POSES.creep` 写"趴在地上"），装时必须 `flip()` |
| `flipX` | 素材朝左而不是朝右（素材契约：一律朝右，渲染器经 `setFlip` 镜像） | 月兔小仙的 `Crawl`：腾云素材眼睛在左、头列在左，朝向判据 `-10.0% / 0.27`（负对照通过） |

**后果**：
- 不标 `invertY`：**宠物头朝上吊在天花板上**（用户原话「反了，应该底部朝上，头部朝下」）
- 多标一次 `invertY`：给本来就倒挂的素材再翻一次，同样头朝上（`import-shimeji.mjs` 里记着一次）
- 不标 `flipX`：所有动作都朝右、往左走时镜像，唯独这一组**不镜像**——它自己就朝左，`setFlip` 把它掰回朝右（月兔的 `Crawl` 修了三轮）

#### 二、帧裁剪契约（`dropFirst`、`dropLast`）

**首尾帧是否需要显式丢弃**——首帧是机位摆拍、末帧是占位图或内容比中间大。

**为什么必须显式声明**：判据有盲区。

| 字段 | 判据的盲区 | 实测案例 |
| --- | --- | --- |
| `dropFirst` | `probeWrap` 判的是"末格像不像首格"，**首帧不在比对范围内** | 月兔 `Climb`：帧 0 是机位摆拍（云比中间大 34%），循环每转一圈都会在格 0 **闪一下云占满屏** |
| `dropLast` | MAD 超阈值时被误判为"不是重复" | 月兔 `Climb`：帧 15 的云同样比中间大 34%，MAD 25.99 > 6 → 判据认为"不重复" |

**触发条件**：
- **首帧 ≠ 首姿态**：姿势图那轮用的是"拍一张侧身站立"→ 横爬那组**不能用它当首帧**（会在格 0 闪站姿）
- **末帧是占位图状态**：模型在收尾时把道具放大（月兔的云）、角色回到中立姿势（某些 pose 轮）

**判据门限**：
- `probeWrap` 认为"末格是首格的复件" ⇔ `MAD < 6 && boxDiff < 0.06 && shift ≤ 6`
- 首帧判据：无（必须显式声明）

**后果**：
- 不丢首帧：循环里闪一个"异常帧"（用户原话「还是会突然显示一个比较不合理的起始帧」）
- 不丢末帧：同上（「还是会出现异常帧的画面」）

#### 三、素材引用契约（`sheet`）

**借用别的动作的表**——某些动作的姿势完全一样（跳 / 欢呼都需要蹲低），不重出、只引用。

**为什么必须显式声明**：这是关联关系，不是测量问题。

**什么时候需要标记**：
- 多个动作的**首帧姿势完全一样**（如 `cheer` 借用 `hop` 的首帧，因为两者都需要"角色压低、头顶留空"）
- 同一组的多个视角（如 `walk-front` 借用 `walk-side` 的时序，只换机位）

**实测案例**：
- 月兔 `cheer` 借用 `hop` 的首帧（`sheet: 'hop'`），避免重出一份会各自漂移的副本
- 用户定的依赖关系用**名字显式声明**，不是复制文件
 ...
 *        ↑ 首尾的云比中间大 34% ↑
 * 为什么**两条路都判不出来**… 末帧：`probeWrap` 判的是「末格像不像首格」，MAD 25.99 超阈值…
...
```js
/**
 * 月兔 Climb：腾云（首尾云比中间大 34%，见规范§6.2.2「帧裁剪契约」）
 */
{ key: 'climb', group: 'Climb', label: '攀爬', kind: 'loop', dropFirst: true, dropLast: true },
```

**"为什么判据量不出来"** 和 **"MAD 阈值是多少"** 属于规范，不属于代码。
