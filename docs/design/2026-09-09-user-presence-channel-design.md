...
渠道消息到达（非斜杠命令）
      │
      ▼
前置守卫：本渠道当前路由的会话必须属于本渠道本人
      （已 /link 或上次接续到别的会话时不再询问——用户已经在明确选定的会话里）
      │
      ▼
检查：该 channelUserId 是否在其他渠道有活跃会话（客户端/微信/飞书/企微，最近 3 天有消息）
      │ 有，且本渠道会话未标记"已询问过"
      ▼
先回复询问：
  「检测到你在【客户端】有进行中的对话：<标题>
   是否接续？回复 1 是，0 否（1 分钟内不回复默认接续）」
      │
      ├─ 回复 1 → setActiveSessionKey 到该会话（ChannelSessionStore 持久化），后续消息走该会话
      ├─ 回复 0 → 显式回到本渠道自己的会话（见下），标记"已询问过"
      └─ 1 分钟超时 → 默认接续（同回复 1），标记"已询问过"
```

**路由落点（2026-09-14 修订）**：

- **活跃会话持久化在 `ChannelSessionStore`**（`channel/channel-session-store.ts`，key `channel:active:{channelType}:{channelUserId}`），
  记录 `active`（当前路由，可能是接续借来的）与 `own`（本渠道本人最近用的会话）。
  以前只有内存 Map，重启即丢，导致「不接续」时回落到一个从没用过的 `{渠道}:{uid}` 被建成空会话。
- **回复 0 / 接续被拒 = 回到 `own`**（`adapter.resetToChannelSession()`），不是"什么都不做"。
  该动作只在「当前路由恰好就是那条 `/link` 绑定」时才解绑，避免误伤用户显式的绑定。
- **接续不写 `weixin:binding:`**：那是 `/link` 的存储，接续只是临时借用别的会话。
  两者混用会让一次接续覆盖、甚至销毁用户的 `/link` 绑定。
- **`/back`**（四个渠道都有）：随时回到本渠道自己的会话；接续回执里会提示它。
- **只问一次**：会话级标志位（内存 Set，重启后重问一次成本可接受）
- **连续对话直接跳过**：同一会话后续消息不再询问；被扣住的那条消息在询问作废（答复无法识别 / 斜杠命令插队）时**必须补投**，不能跟着一起丢
