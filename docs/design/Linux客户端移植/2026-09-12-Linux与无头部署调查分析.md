...
- **2026-09-19**：第一期设计已产出 → [`2026-09-19-Linux桌面客户端第一期设计.md`](2026-09-19-Linux桌面客户端第一期设计.md)。该文新增决策 **D12–D18**（第一期仅桌面形态、桌宠屏蔽入口改精灵图重写、录屏舍弃、语音收窄为在线 TTS、统一屏蔽矩阵、构建固定 Node 22、解耦约束顺延），并在其 **§11 勘误本文的 6 处条目**（含 §10 缺陷 4 实为描述不准确、`forceKillProcess` 实为 4 份、CI 已存在等）。阅读本文时请以勘误为准。
