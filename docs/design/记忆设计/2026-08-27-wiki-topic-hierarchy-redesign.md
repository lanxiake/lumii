...
> 日期：2026-08-27
> 状态：设计定稿 — **v4.1**（一/二/三期完整规格；§11.3 短文整合已于 2026-08-28 落地）
...
**目录**：§0 问题与分类法 → §1 目录模型 → §2–§4 数据与归档（一期核心）→ §5–§7 检索/前端/并发 → §8–§12 二期（改树、重编目、笔记、综述与短文整合、清理）→ §13 三期图谱 → §14–§19 流程、命令、分期、迁移、成功标准、不做。
...
**`整合长文` 小类**：系统预留的统一归档位，供「短文整合」（§11.3）产出写入；AI 分类器**不得**把原始上传文件直接分进此小类（仅整合接受流程写入）。
...
沿用 `wiki_syntheses` 候选态（显式触发、引用保真）。**产出改为普通资料文件**，`accept()` 不再写 `wiki_pages`。

综述（`mode: synthesis`）与短文整合（`mode: consolidate`）共用同一 synthesizer 分块管线，UI 与落库语义不同（见 §11.3）。
...
- 同目录 ≥ 3 篇「短文」时顶栏提示，可「整合全部短文」
- 多选 ≥ 2 篇短文 →「整合为长文」
- 短文判定：正文 `extracted_text` 有效字数 < 800；无正文时用标题长度估算
...
| 阈值 | 常量 | 行为 |
|---|---|---|
| 资料数 > 40 | `SYNTHESIS_SOURCE_CONFIRM_LIMIT` | 抛 `WIKI_SYNTHESIS_CONFIRM_REQUIRED`，UI 二次确认后带 `confirmed: true` 重发 |
| 分块数 > 20 | `SYNTHESIS_MAX_CHUNKS` | 拒绝并提示分批（整合后续可扩展自动分批） |

### 11.3 短文整合（consolidate）

**动机**：同一小类下多篇同主题短摘录（如多篇网页剪藏）造成目录碎片化；合并为一篇 ≥ 1000 字的长文，原条目归档。

**与综述的差异**

| 项 | 综述 | 整合 |
|---|---|---|
| `wiki:synthesis:create.mode` | `synthesis`（默认） | `consolidate` |
| 候选标题 | 用户/目录名 +「综述」 | 前缀 `[整合] ` + 目录名 +「整合」 |
| 终稿 prompt | 归纳要点 | 结构化长文，≥ 1000 字，去重 |
| 接受后目录 | 用户选择器 | **固定** `{来源大类} / 整合长文` |
| 接受后原短文 | 不自动处理 | 默认 **归档**（`archiveSources: true`） |

**统一归档目录**

- 各大类预设小类 **`整合长文`**（§0.4 默认树已含；用户删过则 `acceptAsSource` 时自动 `addSubtopic`）
- 例：在 `学习资料 / 读书摘抄整理` 整合 7 篇摘录 → 产物进 **`学习资料 / 整合长文`**，7 篇原资料 `archived_at` 置位

**端到端（已实现）**

1. `wiki:synthesis:create`（`mode: consolidate`，`sourceIds`）
2. 轮询 `wiki:synthesis:get` 直至候选就绪（进度字段清空且有 `outputPath`）
3. **自动** `wiki:synthesis:accept-as-source` → `{category: 当前大类, subtopic: 整合长文, archiveSources: true}`
4. 主区跳转到 `整合长文` 小类并高亮新文件
5. 生成失败/超时：留在「综述合成」候选列表，可手动「归档到整合长文」

**IPC 扩展**

```ts
// wiki:synthesis:create
{ mode?: 'synthesis' | 'consolidate'; confirmed?: boolean; ... }

// wiki:synthesis:accept-as-source
{ archiveSources?: boolean; ... }  // 整合推荐 true
```

错误码 `WIKI_SYNTHESIS_CONFIRM_REQUIRED` 写入 message 前缀，主进程不记 error 日志；渲染端按 code/count 弹确认框，确认时须携带**同一批** `sourceIds` 重发。
...
正式目录 ─打开► openPath
        ...
        ├ 多选整合短文(二期) ───► 候选 ─自动接受► {大类}/整合长文 + 归档原短文
        ├ 重新编目(二期) ───────► 候选 ─接受► 改两列
        └ 抽取实体(三期) ───────► ERO ─► 图谱结构+关系
...
| `wiki:synthesis:create` 等 | 2 | 入参 `sourceIds`；`mode: consolidate` 为短文整合；>40 须 `confirmed` |
| `wiki:synthesis:accept-as-source` | 2 | 接受为资料；`archiveSources` 归档被合并原短文 |
...
| 整合误并不同主题短文 | 仍候选态；接受前可丢弃；原短文归档可恢复 |
| 整合超 40 篇 | 二次确认；确认须带完整 sourceIds |
| 抽取费用 | 仅手动；hash 增量跳过 |
...
- 同小类多篇短文整合后，长文出现在 `{大类} / 整合长文`，原短文已归档
- 用户新增小类后，下一份上传可以被分进去
