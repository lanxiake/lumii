...
> 状态：v0.1，设计已评审待实施  
> 产出目录约定：`docs/design/`  
> 相关：`bridge-app-ui-tools.ts`、`desktopCapturer`、`MediaRecorder`（语音克隆麦克风路径）、`client-data-root.ts`、`docs/design/客户端UI/2026-08-13-Agent操作客户端设计.md`（曾将录屏标为 YAGNI，本设计单独立项覆盖）
...
| **二期** | 暂停/继续；系统声；时间线预览；可选 MP4；窗口捕获体验加固（B1）；停录后 SRT+TTS 配音（默认烧字幕）。详见 `docs/design/录屏与教程/2026-08-15-客户端录屏二期设计.md` |
