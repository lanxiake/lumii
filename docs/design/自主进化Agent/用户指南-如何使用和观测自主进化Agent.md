...
**当前已知限制**：目标生成后写入数据库为 `pending`，**没有任何通知动作，也没有超时**
（`intrinsic-goal-generator.ts:223`、`:238`）。所以现在目标事实上无人可批，会一直堆积。

**解决方案**见 `前端可视化实施方案.md` 第十节「离线审批架构」，三层机制：

1. **风险分级自动审批** —— 纯内部改进（学习、能力提升、记忆优化）只改 Agent 自身状态，
   自动批准并记审计、可回滚；不必等你确认。
2. **渠道推送** —— 需你确认的目标通过飞书/微信/企微推送，回复 `1`/`2`/`3` 即可完成审批。
   复用项目已有的渠道出站能力（`channel-outbound-router.ts:37`），无需新建通知体系。
3. **超时兜底** —— 每类目标有各自的「安全侧」默认动作，**不是一律自动批准**：
   - 学习/能力提升：4h 超时 → 自动批准（做了无风险，不做则无进化）
   - 主动消息：2h 超时 → 自动拒绝（过期的主动消息反而是骚扰）
   - 技能改进：24h 超时 → 归档，**绝不自动批准**（有副作用）

**送达失败时会收紧**：如果飞书未登录导致你根本没收到请求，非内部目标一律归档，
不会当作「默示同意」执行。

在该机制落地前，你只能通过 SQL 手动审批：

```sql
-- 查看待审批
SELECT id, type, description, priority FROM autonomous_goals WHERE status = 'pending';

-- 手动批准
UPDATE autonomous_goals SET status = 'approved', approved_at = datetime('now') WHERE id = 'goal_xxx';
```
...
| 优先级 | 功能 | 工时 | 价值 |
|-------|------|------|------|
| **P0** | 风险分级 + 自动审批（L0 内部目标） | 4h | 最高 - 解开进化死锁 |
| **P0** | 渠道审批送达 + 超时兜底 | 18h | 最高 - 离线也能审批 |
| **P0** | CLI 状态查看命令 | 2h | 高 - 立即可观测 |
| **P1** | 简化版仪表板（满意度 + 待审批 + 审批设置） | 10h | 高 - 可视化与配置 |
| **P1** | 能力雷达图（P1 特性） | 4h | 中 - 能力透明 |
| **P2** | 完整仪表板（含反思、Prompt 统计） | 12h | 中 - 完整体验 |
| **P2** | 本机桌面通知（辅助提醒） | 1h | 低 - 仅在电脑前有效 |
| **P3** | 历史趋势分析页面 | 8h | 低 - 高级功能 |

**排序理由的变化**：桌面通知从 P0 降到 P2。它只在用户守着电脑时有效，
无法承担审批送达；而"自动审批 + 渠道送达 + 超时兜底"才是让进化链路真正跑起来的前提。
详见 `前端可视化实施方案.md` 第十节。
...
2. **短期**：实现自动审批 + 渠道审批送达（解开进化死锁），再做简化版仪表板
