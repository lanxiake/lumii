...
| L3 真实对话 | CK-05 ~ CK-10 | 是 | 建指定 Agent 的会话跑真回合/真任务 |
...
| CK-10 | 建 **info-curator** 会话，说「以后少推「<探针>」这类内容」 | 探针字符串出现在 `user-memory.md` 的 `## 资讯偏好` 章节**之内**，且调用了 `news_preference` | 结构化偏好的全部意义就是「落点固定」——只断言「文件里有这个词」会漏掉「散在某段自由文本里」这种失败 |
...
- 不修改任何用户配置；CK-05/CK-08/CK-09 在指令里明确要求「只读、不要修改」；
- **CK-10 会改 `user-memory.md`**（唯一一处）：跑前快照全文、跑后整份还原，探针用 `__ck-probe-<时间戳>__` 以免与真实偏好撞车；还原在 `finally` 里做，用例失败也不留痕。
