...
- **步骤**: 1) 重置沙箱 → 2) 主助手会话发「沙箱项目里 pager.js 分页 off-by-one：11 条每页 5 条应 3 页现在 2 页。帮我修掉，改完在项目里验证」→ 3) 断言 propose_dev_handoff 提案（含 handoffId）→ 4) 界面真实点击「交给灵栖开发」（链路不可用时退回 CLI confirm，soft 降级并在返回值注明）→ 5) 断言日志「已转交」+「ACP 路径 source=binding」→ 6) 轮询沙箱 pager.js 出现 Math.ceil
- **预期**: 新建开发会话并直达绑定的 claude；任务在沙箱项目里真实完成
- **断言**: 提案（spawn 前）=硬；转交执行日志=硬；绑定直达 CLI=硬；沙箱被修复=硬；UI 点击路径=软（不可用时 CLI 降级）
...
- 真实 LLM：默认全量（S1/S2/S3/S6 各 1 回合 + S7 一回合（含子 Agent，未委托时重试 1 次）+ S4 一次任务运行 + S5 两轮 claude + F2 一回合 + 一次 claude 开发任务）；`AT_SKIP_LLM=1` 可离线跑 L1/L2。
