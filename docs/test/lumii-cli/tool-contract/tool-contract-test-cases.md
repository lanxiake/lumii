...
- **优先级**：P1（回归防线）
- **层**：L3（真实聊天模拟）
- **前置**：无——它守的是既有行为，不需要 `dev:restart`
- **真实数据**：一条必然失败、输出约 20 万字符的 bash 命令（2000 行 × ~100 字符）
- **步骤**：
  1. 新建会话，要求 agent 原样执行 `for i in $(seq 1 2000); do echo "..."; done; exit 1`
  2. 从日志取该次 `tool:end` 的 `resultPreview`
- **预期**：失败调用的 preview 是「已落盘 + 路径」的提示，**不是原文**
- **断言**：**硬**
- **为什么需要它**：批次 1 复核连带影响时实测确认「失败的大输出照常落盘」——
  但那是**两套落盘 hook 并存**的结果：

  | | 阈值 | `isError` 判断 |
  | --- | --- | --- |
  | `packages` 侧 `tool-result-persist-hook` | 50K | **有**（失败时跳过） |
  | 宿主侧 `createLargeToolResultHook` | **30K** | **无**（照常按长度处理） |

  **两套都跳过才是不落盘。** 批次 2 计划把它们合并——一旦合并时保留了
  「错误结果不落盘」那条策略，`local-bash.ts` 的 **1MB** `OUTPUT_CAP` 就会直接灌进上下文。
  本用例是那次合并的回归防线。
- **实测（2026-09-18，手工复核时）**：
