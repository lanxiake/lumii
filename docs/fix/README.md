# 问题修复记录

本目录存放**故障的诊断与修复记录**。命名：`YYYY-MM-DD-<功能>-<问题>-fix.md`。

写作要点：复现步骤 + 根因（含代码位置证据）+ 修复代码 + 回归验证方法。完整模板见 [`../wiki/05-implementation/02-功能实施模板.md`](../wiki/05-implementation/02-功能实施模板.md)。

| 文档 | 问题 |
| --- | --- |
| [`2026-08-09-语音通话实时转写修复.md`](2026-08-09-语音通话实时转写修复.md) | 语音通话实时转写不显示（ASR 离线模式不支持流式中间结果） |
| [`2026-08-13-新建会话丢失修复.md`](2026-08-13-新建会话丢失修复.md) | 新建会话重启后丢失（`isIoErr()` 误判数据库锁） |
| [`2026-09-07-Agent消息乱序修复.md`](2026-09-07-Agent消息乱序修复.md) | Agent 消息顺序错乱 |
| [`2026-09-07-云同步冲突检测修复.md`](2026-09-07-云同步冲突检测修复.md) | 云同步符号链接与冲突检测误触发 Agent 处理 |
| [`2026-09-20-主进程冻结调查与修复.md`](2026-09-20-主进程冻结调查与修复.md) | 主进程主线程冻结（会话列表预览已修；native 层阻塞未解，09-19 的 54 秒疑似同源） |
| [`2026-09-21-浏览器控制工具调查与修复.md`](2026-09-21-浏览器控制工具调查与修复.md) | 浏览器工具：`browser_eval` 丢 `url` 致模型把 about:blank 误判成作用域问题（已修）；模型把工具调用写进推理文本（已加检测器）；**首次截图 79–231 秒（已查明未修）**；无标签管理（能力边界） |

已沉淀为方法论的案例另见 [`../wiki/07-debugging-and-fixes/`](../wiki/07-debugging-and-fixes/)。
