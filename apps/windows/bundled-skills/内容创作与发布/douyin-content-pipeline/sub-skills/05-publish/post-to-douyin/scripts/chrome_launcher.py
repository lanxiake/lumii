...
Manages a dedicated Chrome instance for Douyin publishing:
...
CDP_PORT = 9222
PROFILE_DIR_NAME = "DouyinProfile"
...
def get_chrome_path() -> str:
    """Find Chrome/Chromium/Edge executable on Windows."""
    candidates = []

    # Priority 1: custom anti-detection Chromium (mtbot bundled)
    custom_chromium = os.path.join(
        os.environ.get("USERPROFILE", ""),
        ".chromium-browser-snapshots",
        "chromium", "win64-1413417", "chrome-win", "chrome.exe",
    )
    if os.path.isfile(custom_chromium):
        candidates.append(custom_chromium)
    ...
    for env_var in ("PROGRAMFILES", "PROGRAMFILES(X86)", "LOCALAPPDATA"):
        base = os.environ.get(env_var, "")
        if base:
            edge_base = os.path.join(base, "Microsoft", "Edge", "Application")
            if os.path.isdir(edge_base):
                for entry in os.listdir(edge_base):
                    full = os.path.join(edge_base, entry, "msedge.exe")
                    if os.path.isfile(full):
                        candidates.append(full)
                        break  # just pick the first version dir
            candidates.append(os.path.join(base, "Microsoft", "Edge", "Application", "msedge.exe"))

    for path in candidates:
        if os.path.isfile(path):
            print(f"[chrome_launcher] Using browser: {path}")
            return path
    ...
    raise FileNotFoundError(
        "Chrome/Edge not found. Please install Google Chrome or Microsoft Edge."
    )
