...
def main():
    ...
    parser.add_argument(
        "--schedule",
        default=None,
        help="Scheduled publish time YYYY-MM-DD HH:MM (set before clicking publish)",
    )

    # Publish options file (optional)
    parser.add_argument(
        "--options",
        default=None,
        help="publish-options.json path (schedule/content_type override)",
    )
    ...
    if args.options:
        ...
        opts = PublishOptions.load(Path(args.options))
        if opts.schedule and not schedule:
            schedule = opts.schedule
        if opts.account and opts.account != "default" and not account:
    ...
    if args.auto_publish and args.mode == "image-text":
        ...
        if schedule:
            print(f"[pipeline] Scheduled for: {schedule}")
        try:
            publisher._click_publish(schedule=schedule)
            if schedule:
                print(f"SCHEDULE_STATUS: {schedule}")
