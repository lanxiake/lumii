...
脚本: `scripts/cdp_publish.py` → `_set_schedule()` / `set-schedule` 命令

- 在发布页选择「定时发布」并填入 `YYYY-MM-DD HH:MM`
- 推荐时段由 `scripts/schedule_advisor.py` 按内容类型生成（见 `references/schedule-strategy.md`）
- 配置可写入笔记目录 `publish-options.json` 的 `schedule` 字段

### 8. 用户确认并发布

- 脚本填写完成后暂停，提示用户在浏览器中检查预览
- 若需定时，先调用 `set-schedule` 或 `click-publish --schedule`
- 用户确认后，脚本点击发布按钮
- 或用户选择手动点击发布按钮
...
长文编辑器的图片不能用 `file:///` + `appendChild` 注入 —— ProseMirror 会忽略直接 DOM 改动。正确做法是模拟用户点击工具栏图片按钮，触发文件选择器，再用 CDP 喂入文件。脚本: `_insert_long_images()` / `insert_images_at_anchors()`。

要点：

1. **工具栏图片按钮靠 SVG path 签名匹配**，不靠文字或下标（图标无文字、顺序可能变）。签名 `M6.91635 8.25017`，下标 8 作兜底。见 `_find_long_image_button()`。
2. **必须用"可信"鼠标事件打开文件选择器**。合成的 `element.click()` 不携带 user activation，浏览器会拦截文件对话框。改用 CDP `Input.dispatchMouseEvent`（mouseMoved/Pressed/Released）派发真实点击。见 `_real_click_and_catch_filechooser()`。
3. **拦截文件对话框**：`Page.setInterceptFileChooserDialog(enabled=True)` → 真实点击触发 `Page.fileChooserOpened` 事件（拿到 `backendNodeId`）→ `DOM.setFileInputFiles(backendNodeId, [file])` 喂文件。这样不弹原生系统对话框。
4. 上传成功后图片在编辑器里表现为 `blob:https://creator.xiaohongshu.com/...` 的 `<img>`。
5. **按占位符插图（in-flow）**：插入前先把光标移到目标文字段（`_place_cursor_at_anchor(anchor, where)`，`where` ∈ `top`/`before`/`after`，用 Range + Selection 定位），再插图，图片就落在对应段落而不是全堆在末尾。CLI 用 `--placements <json>`。
...
脚本: `scripts/cdp_publish.py` → `upload_cover(cover_path)`

小红书长文封面 **不在正文里**，而是在「一键排版」页单独上传：

1. 封面上传入口在**左侧第一页预览**（最小 `left` 的竖版卡片，约 150–420px 宽、180–620px 高）。
2. 用可信 `Input.dispatchMouseEvent` 的 `mouseMoved` **悬停**到第一页中心，页面下方浮出两个按钮：「Ai 换配图」和「从本地上传」。
3. 真实点击「从本地上传」→ 同正文插图一样走 `Page.setInterceptFileChooserDialog` + `fileChooserOpened` + `DOM.setFileInputFiles` 喂入封面文件。
4. 不上传自定义封面时，平台会用正文图自动生成「有图封面1/2」「无图封面」，可在右侧「封面设置」面板里选（`.cover-item`）。
5. CLI：`python cdp_publish.py upload-cover --cover 01-cover.png`，成功输出 `COVER_UPLOADED`。
...
| 长文图片按钮 | `.menu-item` 内 SVG path 含 `M6.91635 8.25017` | 下标 8 | 工具栏图片按钮，按签名匹配 |
| 模板卡片 | `.template-card` | `.template-card.selected`（已选） | 排版模板列表 |
| 模板名称 | `.template-card .template-title` | - | 模板卡片内的名称 span |
| 封面项（自动） | `.cover-item` | - | 右侧"封面设置"里的有图/无图封面 |
| 封面上传按钮 | 悬停第一页预览后文本"从本地上传" | "Ai 换配图" | 自定义封面入口，浮层 |
...
# 推荐定时时段
python schedule_advisor.py --type 干货 --json
```
...
#    正文配图优先用 --placements（按占位符插图），不要用 --images 堆末尾
python cdp_publish.py long-article --title-file title.txt --content-file content.txt --placements placements.json

# 4. 选择模板
python cdp_publish.py select-template --name "模板名称"

# 4a. 上传自定义封面（可选，封面不放进正文）
python cdp_publish.py upload-cover --cover 01-cover.png
...
`placements.json` 示例（`where` ∈ `top`/`before`/`after`，`anchor` 是目标段落里的一段文字）：
