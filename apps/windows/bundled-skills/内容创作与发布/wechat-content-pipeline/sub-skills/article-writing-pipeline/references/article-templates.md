...
> 所有 AI 生成图都通过 LLM-Link 调用 gpt-image-2-pro，提示词留底在 `articles/<slug>/images/prompts.md`。
>
> **分层视觉策略**（实测沉淀，按美观度优先级排）：
> - **封面图**：**专业科技/科幻/商务风**（深蓝科技底 + 数据流/神经网络/光效，人物用几何剪影或抽象全息，绝不卡通萌系）。用 gpt-image-2-pro 生成。**每篇封面要有差异**——换主色调、换核心视觉隐喻。
> - **原理/信息图（首选 gpt-image-2-pro PPT 风）**：实测 gpt-image-2-pro 现在**中文渲染基本可用**，能出 PPT 级精致信息图（霓虹描边、玻璃质感、SVG 风线性图标、流程箭头、四象限），美观度远超手工 HTML。**但文字不稳定，每张必须人工核验中文，错了重生成**（成功率：文字少/结构清晰的高，文字密集的低）。提示词务必写明"清晰准确的简体中文、不要乱码假字、不要 Emoji、用 line icon"。
...
>
> 关键教训：
> 1. **gpt-image-2-pro 中文已基本可用但不稳定**——能用，但每张必核验、备好重试（504 也常见，多试几次）。要求文字 100% 精确时用 HTML 兜底。
