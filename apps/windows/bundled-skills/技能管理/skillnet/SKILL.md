...
**Important**: The correct package name is `skillnet-ai` (not `skillnet`).
...
# Or using python -m pip (if pip command not found)
python -m pip install skillnet-ai
```

After installation, the `skillnet` command will be available:

```bash
# Verify installation
skillnet --help

# Quick start: Search for skills
skillnet search "react" --limit 5
```
