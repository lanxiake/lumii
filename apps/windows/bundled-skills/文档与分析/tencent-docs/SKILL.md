...
### 创建/写入文档内容：使用 `qclaw_tdoc_mcp_call`

涉及**创建文档**或**写入/编辑文档内容**的操作，必须使用 `qclaw_tdoc_mcp_call` 工具（命令行有参数长度限制，长内容会失败）：

```
# 创建智能文档（最常用）
qclaw_tdoc_mcp_call tool_name="create_smartcanvas_by_mdx" arguments={"title": "标题(≤36字符)", "mdx": "# 正文Markdown内容", "content_format": "mdx"}

# 编辑已有文档（追加内容到末尾）
qclaw_tdoc_mcp_call tool_name="smartcanvas.edit" arguments={"file_id": "xxx", "action": "INSERT_AFTER", "content": "# 新内容"}
```

### 其他操作：使用 mcporter CLI
