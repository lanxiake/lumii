 ...
 */
function buildInstalledLookup(local: Array<{
    id?: string;
    name: string;
    dirName?: string;
}>): {
 ...
 */
function isStoreSkillInstalled(skill: StoreSkillInfo, lookup: ReturnType<typeof buildInstalledLookup>): boolean {
 ...
 */
function normalizeLocalInstalledRaw(raw: unknown): Array<{
    id?: string;
    name: string;
    dirName?: string;
}> {
    const arr: unknown[] = Array.isArray(raw) ? raw : ((raw as {
        data?: unknown[];
    })?.data ?? []);
    return arr
        .map((item) => {
 ...
 */
function sortSkillsByFeaturedPopularOrder(list: StoreSkillInfo[], featuredOrder: Map<string, number>, popularOrder: Map<string, number>): StoreSkillInfo[] {
 ...
 */
function applyFeaturedPopularFlags(list: StoreSkillInfo[], featuredOrder: Map<string, number>, popularOrder: Map<string, number>): StoreSkillInfo[] {
    return list.map((s) => ({
        ...s,
...
interface UseSkillStoreReturn {
    ...
    /** 安装技能（下载并解压到本地） */
    installSkill: (skillId: string) => Promise<{
        success: boolean;
        error?: string;
    }>;
    /** 上传技能 */
    uploadSkill: (data: SkillUploadData) => Promise<{
        success: boolean;
        skillId?: string;
        error?: string;
    }>;
...
export function useSkillStore(): UseSkillStoreReturn {
     ...
     */
    const applyLocalInstallFlags = useCallback(async (list: StoreSkillInfo[], localRaw?: unknown): Promise<StoreSkillInfo[]> => {
    ...
    const loadStoreSkills = useCallback(async (newFilters?: StoreFilters) => {
        ...
        try {
            ...
            const featuredOrder = new Map((featResult.success && featResult.data ? featResult.data : []).map((s, i) => [s.id, i]));
            const popularOrder = new Map((popResult.success && popResult.data ? popResult.data : []).map((s, i) => [s.id, i]));
        ...
        try {
            ...
            if (result.success && result.data) {
                ...
                setSkills((prev) => {
                    ...
                    const tagged = applyFeaturedPopularFlags(deduped, featuredOrderRef.current, popularOrderRef.current);
                    return [...prev, ...tagged];
                });
     ...
     */
    const updateFilters = useCallback(async (partial: Partial<StoreFilters>) => {
     ...
     */
    const installSkill = useCallback(async (skillId: string): Promise<{
        success: boolean;
        error?: string;
    }> => {
     ...
     * 上传技能（创建用户自建技能）
     */
    const uploadSkill = useCallback(async (data: SkillUploadData): Promise<{
        success: boolean;
        skillId?: string;
        error?: string;
    }> => {
