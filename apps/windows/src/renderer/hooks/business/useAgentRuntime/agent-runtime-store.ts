 ...
 */
import type { AgentRuntimeEvent, ContentBlock, ContextBudgetSnapshot, ContextUsageBreakdownEntry, TokenUsage, } from '../../../../shared/agent-runtime-events';
...
export function setFocusPermissionRequestId(requestId: string | null): void {
    runtimeStore.setState((prev) => prev.focusPermissionRequestId === requestId
        ? prev
...
export function setPendingPetHandoff(text: string | null): void {
    runtimeStore.setState((prev) => prev.pendingPetHandoff === text ? prev : { ...prev, pendingPetHandoff: text });
}
 ...
 */
export function updateSessionState(sessionKey: string, updater: (prev: PerSessionState) => PerSessionState): void {
    runtimeStore.setState((globalPrev) => {
        ...
        if (Object.is(sessionNext, sessionPrev))
            return globalPrev;
...
export function setSteerDraft(sessionKey: string, draft: string): void {
    updateSessionState(sessionKey, (prev) => prev.steer.draft === draft ? prev : { ...prev, steer: { ...prev.steer, draft } });
}
/** 标记插话已发出（UI 据此显示「补充中」） */
export function markSteerSent(sessionKey: string): void {
    updateSessionState(sessionKey, (prev) => prev.steer.sent ? prev : { ...prev, steer: { ...prev.steer, sent: true } });
}
/**
 ...
 * 一律清空会把用户正在输入的内容误删。
 */
export function resetSteer(sessionKey: string, opts?: {
    clearDraft?: boolean;
}): void {
    updateSessionState(sessionKey, (prev) => {
        const draft = opts?.clearDraft ? '' : prev.steer.draft;
        if (draft === prev.steer.draft && !prev.steer.sent)
            return prev;
 ...
 */
export function findAnyPendingPermission(state: MultiSessionRuntimeState): {
    sessionKey: string;
 ...
 */
export function findAnyPendingAskUser(state: MultiSessionRuntimeState): {
    sessionKey: string;
...
export function getPendingAskUserSnapshot(): PendingAskUserSnapshot {
    ...
    if (prev.sessionKey === found.sessionKey &&
