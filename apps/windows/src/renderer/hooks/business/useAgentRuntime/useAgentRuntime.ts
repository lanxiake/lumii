...
import { runtimeStore, getDefaultPerSessionState, updateSessionState, findAnyPendingPermission, findAnyPendingAskUser, getPendingPermissionSnapshot, getPendingAskUserSnapshot, markSteerSent, type PendingPermissionSnapshot, type PendingAskUserSnapshot, } from './agent-runtime-store';
 ...
 * ```
 */
export function useAgentRuntimeState<T>(selector: (state: PerSessionState) => T): T {
 ...
 * ```
 */
export function useAgentRuntimeGlobalState<T>(selector: (state: MultiSessionRuntimeState) => T): T {
    const selectorRef = useRef(selector);
    selectorRef.current = selector;
...
export function useAgentRuntimeActions() {
    ...
    const abort = useCallback(async () => {
        ...
        updateSessionState(sessionKey, (prev) => ({
            ...
            currentTool: null,
            activeRunId: null,
            messages: prev.messages.map((msg) => msg.isStreaming ? { ...msg, isStreaming: false } : msg),
        }));
        await api.sendCommand({ type: 'user:abort', ...(runId ? { runId } : {}), sessionKey });
    }, []);
    const respondPermission = useCallback(async (decision: 'allow-once' | 'allow-always' | 'deny') => {
        const api = window.electronAPI?.agentRuntime;
        if (!api?.sendCommand)
            return;
     ...
     */
    const respondAskUser = useCallback(async (payload: {
    ...
    const createSession = useCallback(async (title?: string, agentId?: string, selectedModelId?: string) => {
        const api = window.electronAPI?.agentRuntime;
        if (!api?.sendCommand) {
     ...
     */
    const listSessions = useCallback(async (): Promise<readonly {
        sessionKey: string;
        title: string;
        updatedAt: string;
        agentId?: string;
    ...
    const loadOlderMessages = useCallback(async (sessionKey: string): Promise<void> => {
        ...
        if (!paging || !paging.hasMore || paging.isLoading || !paging.cursor)
            return;
        ...
        try {
            ...
            debugLog('[loadOlderMessages] sessionKey=' + sessionKey + ' loaded=' + page.items.length + ' hasMore=' + page.hasMore);
        }
        catch (err) {
     ...
     */
    const deleteMessage = useCallback(async (messageId: string, options?: {
        sessionKey?: string;
    }) => {
        const api = window.electronAPI?.agentRuntime;
        if (!api?.sendCommand) {
    ...
    const editMessage = useCallback(async (messageId: string, newContent: string, options?: {
        sessionKey?: string;
    }) => {
        const api = window.electronAPI?.agentRuntime;
        if (!api?.sendCommand)
     ...
     */
    const forkConversation = useCallback(async (sourceSessionKey: string, uptoMessageId: string, newContent: string): Promise<string> => {
        const api = window.electronAPI?.agentRuntime;
        if (!api?.sendCommand)
     ...
     */
    const editAndResend = useCallback(async (messageId: string, newContent: string, options?: {
        sessionKey?: string;
    }): Promise<void> => {
        const api = window.electronAPI?.agentRuntime;
        if (!api?.sendCommand)
        ...
        updateSessionState(sessionKey, (prev) => {
            ...
            const kept = prev.messages.slice(0, idx + 1).map((m) => m.id === messageId
                ? { ...m, content: [{ type: 'text' as const, text: newContent }] }
                : m);
        ...
        if (result && result.success === false) {
            ...
            window.dispatchEvent(new CustomEvent('mtbot:agent-error', {
     ...
     */
    const syncModelCatalog = useCallback(async (entries: readonly {
        id: string;
        contextWindow?: number;
        maxTokens?: number;
    }[]) => {
        const api = window.electronAPI?.agentRuntime;
        if (!api?.sendCommand || entries.length === 0)
            return;
     ...
     */
    const applyContextUsage = useCallback((sessionKey: string, raw: SessionContextUsageResult) => {
     ...
     */
    const refreshContextUsage = useCallback(async (sessionKey: string) => {
        const api = window.electronAPI?.agentRuntime;
        if (!api?.sendCommand || !sessionKey)
            return;
        try {
     ...
     * 更新当前会话的思考模式与推理强度
     */
    const setSessionThinkingPrefs = useCallback(async (sessionKey: string, patch: {
     ...
     */
    const setGlobalThinkingPrefs = useCallback(async (patch: {
        thinkingEnabled?: boolean;
        reasoningEffort?: 'high' | 'max';
    }) => {
        const api = window.electronAPI?.agentRuntime;
