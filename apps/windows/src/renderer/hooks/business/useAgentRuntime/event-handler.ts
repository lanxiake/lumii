...
import { applyAssistantPartEvent, describeLlmError, finalizeAssistantParts, type AssistantPart, type AssistantPartEvent, type LlmErrorDetail, } from '@mtbot/agent-runtime/browser';
 ...
 */
function extractAcpBackend(model?: string): {
    backendId: string;
    label: string;
} | null {
    if (!model || !model.startsWith('acp:'))
        return null;
 ...
 */
function applyRuntimeAssistantPartEvent(parts: readonly AssistantPart[], event: AssistantPartEvent): AssistantPart[] {
...
function clearTurnTextStart(sessionKey: string, messageId?: string): void {
    ...
    for (const key of turnTextStartByMessage.keys()) {
        if (key.startsWith(prefix))
            turnTextStartByMessage.delete(key);
    }
}
// ============================================================
 ...
 */
function applyThinkingDeltaBatch(sessionKey: string, instanceId: string | undefined, text: string): void {
    updateSessionState(sessionKey, (prev) => {
        ...
        for (let i = msgs.length - 1; i >= 0; i--) {
            ...
            if (message.role !== 'assistant' || !message.isStreaming)
                continue;
            if (instanceId && message.sourceAgent?.instanceId !== instanceId)
                continue;
            if (!instanceId && message.sourceAgent)
                continue;
            msgs[i] = {
                ...message,
 ...
 */
function applyMainTextDeltaBatch(sessionKey: string, messageId: string | undefined, text: string): void {
    updateSessionState(sessionKey, (prev) => {
        const msgs = [...prev.messages];
 ...
 */
function applySubAgentTextDeltaBatch(sessionKey: string, instanceId: string, text: string): void {
 ...
 */
function partsWithLlmErrorIfNeeded(parts: readonly AssistantPart[], err: LlmErrorDetail | undefined, contentText: string | undefined): AssistantPart[] {
    if (!err || contentText?.trim())
        return [...parts];
 ...
 */
function finalizeStreamingAssistantMessages(messages: readonly RuntimeMessage[], closeMessages = false, interrupted = false): readonly RuntimeMessage[] {
    let changed = false;
    const next = messages.map((message) => {
        if (message.role !== 'assistant')
            return message;
        const hasStreamingPart = message.parts.some((part) => (part.type === 'thinking' || part.type === 'text') && part.status === 'streaming');
        const hasRunningTool = interrupted && message.parts.some((part) => part.type === 'tool' && part.status === 'running');
        if (!hasStreamingPart && !hasRunningTool && !(closeMessages && message.isStreaming))
            return message;
        changed = true;
        return {
            ...message,
...
function resolveTargetSessionKey(event: AgentRuntimeEvent): string | null {
    ...
    const runId = 'runId' in event ? (event as {
        runId?: string;
    }).runId : undefined;
    if (runId) {
    ...
    const sk = 'sessionKey' in event ? (event as {
        sessionKey?: string;
...
function isSubAgentStreamEvent(event: AgentRuntimeEvent): boolean {
    if (!('rootSessionKey' in event) || !event.rootSessionKey)
        return false;
 ...
 */
function findMsgIndexByInstanceId(msgs: readonly RuntimeMessage[], sessionKey: string, instanceId: string): number {
    const mid = getSubAgentMsgId(sessionKey, instanceId);
    if (mid) {
    ...
    for (let i = msgs.length - 1; i >= 0; i--) {
        const m = msgs[i]!;
        if (m.role === 'assistant' && m.sourceAgent?.instanceId === instanceId)
            return i;
    }
    return -1;
}
 ...
 */
function buildStreamMetrics(usage: {
    outputTokens: number;
}, durationMs: number): StreamMetrics {
...
export function handleRuntimeEvent(event: AgentRuntimeEvent): void {
    ...
    switch (event.type) {
        case 'agent:message:start': {
            ...
            if (isSubAgentStreamEvent(event) && event.instanceId) {
                ...
                updateSessionState(sessionKey, (prev) => {
                    ...
                    const existingIdx = prev.messages.findIndex((m) => m.id === event.messageId && m.role === 'assistant');
                    if (existingIdx >= 0) {
                        const msgs = [...prev.messages];
            ...
            updateSessionState(sessionKey, (prev) => {
                ...
                const baseMessages = reuseIdx >= 0
                    ? prev.messages
                    : prev.messages.map((m) => m.role === 'assistant' && !m.sourceAgent && m.isStreaming
                        ? { ...m, isStreaming: false }
                        : m);
                if (reuseIdx >= 0) {
        ...
        case 'agent:message:delta': {
            if (isSubAgentStreamEvent(event) && event.instanceId) {
                enqueuePendingDelta({ kind: 'sub_agent_text', sessionKey, instanceId: event.instanceId }, event.delta);
            }
            else {
                enqueuePendingDelta({ kind: 'main_text', sessionKey, messageId: event.messageId }, event.delta);
            }
            scheduleDeltaFlush();
            break;
        }
        case 'agent:message:end': {
            ...
            if (isSubAgentStreamEvent(event) && event.instanceId) {
                ...
                updateSessionState(sessionKey, (prev) => {
                    ...
                    const llmErrorBlock = err
                        ...
                        : undefined;
                    const mergedContent = err && (!finalContent[0]?.text?.trim())
                        ? ([{ type: 'text' as const, text: describeLlmError(err) }] as const)
                        : finalContent;
                    msgs[idx] = {
                        ...last,
                        content: mergedContent,
                        parts: finalizeAssistantParts(partsWithLlmErrorIfNeeded(last.parts, err, finalContent[0]?.text)),
            ...
            updateSessionState(sessionKey, (prev) => {
                ...
                if (!failed && !keepStreamingMain && event.usage) {
                    streamMetrics = buildStreamMetrics({ outputTokens: event.usage.outputTokens }, durationMsMain);
                }
                const llmErrorBlock = err
                    ? { code: err.code, message: err.message, retryable: err.retryable }
                    : undefined;
                const mergedContent = err && (!finalContent[0]?.text?.trim())
                    ? ([{ type: 'text' as const, text: describeLlmError(err) }] as const)
                    : finalContent;
                const injected = event.injectedMemories && event.injectedMemories.length > 0
                ...
                const routeAfterMessage: {
                    llmRouteStatus: PerSessionState['llmRouteStatus'];
                    llmRouteDetail: string | null;
                } = failed
                    ? {
        ...
        case 'agent:thinking:delta': {
            const instanceId = isSubAgentStreamEvent(event) && event.instanceId ? event.instanceId : undefined;
            enqueuePendingDelta({
                kind: 'thinking',
                sessionKey,
        ...
        case 'agent:thinking:end': {
            ...
            updateSessionState(sessionKey, (prev) => {
                ...
                for (let i = msgs.length - 1; i >= 0; i--) {
                    ...
                    if (message.role !== 'assistant')
                        continue;
                    if (isSubAgentThinking && message.sourceAgent?.instanceId !== event.instanceId)
                        continue;
                    if (!isSubAgentThinking && message.sourceAgent)
                        continue;
                    msgs[i] = {
                        ...message,
        ...
        case 'agent:tool:progress': {
            updateSessionState(sessionKey, (prev) => {
                if (!prev.currentTool || prev.currentTool.id !== event.toolCallId)
                    return prev;
                return {
                    ...prev,
        ...
        case 'agent:tool:end': {
            ...
            const isSubAgentTool = 'instanceId' in event &&
            ...
            updateSessionState(sessionKey, (prev) => {
                ...
                for (let i = msgs.length - 1; i >= 0; i--) {
                    ...
                    if (message.role !== 'assistant')
                        continue;
                    if (isSubAgentTool && message.sourceAgent?.instanceId !== event.instanceId)
                        continue;
                    if (!isSubAgentTool && message.sourceAgent)
                        continue;
        ...
        case 'agent:turn:start': {
            ...
            updateSessionState(sessionKey, (prev) => {
                ...
                const messages = prev.messages.filter((msg) => !(msg.role === 'assistant' && msg.isStreaming && !msg.sourceAgent && (msg.content[0]?.text ?? '') === ''));
                return {
                    ...prev,
                    messages,
                    isStreaming: true,
        ...
        case 'agent:turn:end': {
            ...
            if (isSubAgentStreamEvent(event)) {
                ...
                if (subInstanceId) {
                    updateSessionState(sessionKey, (prev) => {
                        const hasStreaming = prev.messages.some((msg) => msg.isStreaming && msg.sourceAgent?.instanceId === subInstanceId);
                        if (!hasStreaming)
                            return prev;
                        return {
                            ...prev,
                            messages: prev.messages.map((msg): RuntimeMessage => msg.isStreaming && msg.sourceAgent?.instanceId === subInstanceId
            ...
            updateSessionState(sessionKey, (prev) => {
                ...
                let messages: readonly RuntimeMessage[] = hasStreaming
                    ? prev.messages.map((msg): RuntimeMessage => msg.isStreaming ? { ...msg, isStreaming: false } : msg)
                    : prev.messages;
        ...
        case 'agent:turn:file-changes': {
            if (event.fileChanges.length === 0)
                break;
            updateSessionState(sessionKey, (prev) => {
                let targetIdx = prev.messages.findIndex((message) => message.id === event.messageId && message.role === 'assistant');
                if (targetIdx < 0) {
                    for (let i = prev.messages.length - 1; i >= 0; i--) {
        ...
        case 'agent:idle': {
            ...
            updateSessionState(sessionKey, (prev) => ({
                ...
                messages: finalizeStreamingAssistantMessages(prev.messages, true).map((m) => m.steerPending ? { ...m, steerPending: false } : m),
                activeRunId: null,
                isStreaming: false,
                isThinking: false,
                currentTool: null,
            ...
            if (isSubAgentStreamEvent(event)) {
                ...
                updateSessionState(sessionKey, (prev) => ({
                    ...
                    messages: prev.messages.map((msg) => {
                        ...
                        if (msg.role !== 'assistant' || !msg.sourceAgent)
                            return msg;
                        if (abortedInstanceId && msg.sourceAgent.instanceId !== abortedInstanceId)
                            return msg;
                        return {
                            ...msg,
        ...
        case 'agent:permission:request': {
            ...
            if (event.timeoutMs > 0) {
                ...
                setTimeout(() => {
                    updateSessionState(sessionKey, (prev) => {
                        if (prev.pendingPermission?.requestId !== reqId)
                            return prev;
        ...
        case 'agent:ask-user:request': {
            ...
            if (event.timeoutMs > 0) {
                ...
                setTimeout(() => {
                    updateSessionState(sessionKey, (prev) => {
                        if (prev.pendingAskUser?.requestId !== reqId)
                            return prev;
        ...
        case 'agent:activity:snapshot': {
            ...
            updateSessionState(sessionKey, (prev) => {
                ...
                const nextMessages = prev.messages.map((msg) => {
                    if (!msg.sourceAgent)
                        return msg;
                    const preferredName = resolveBackfilledAgentLabel(msg.sourceAgent.label, msg.sourceAgent.instanceId, idToName.get(msg.sourceAgent.instanceId));
                    if (!preferredName)
                        return msg;
        ...
        case 'agent:subagent:completed': {
            ...
            if (event.status === 'cancelled')
                break;
            const focusedOnParent = typeof document !== 'undefined'
                && document.hasFocus()
        ...
        case 'session:cleared' as AgentRuntimeEventType: {
            const clearedKey = (event as unknown as {
                sessionKey: string;
            }).sessionKey;
        ...
        case 'session:compact-request' as AgentRuntimeEventType: {
            const { sessionKey: targetKey, keepRecentTurns } = event as unknown as {
                sessionKey: string;
        ...
        case 'session:switch-request' as AgentRuntimeEventType: {
            const { sessionKey: targetKey } = event as unknown as {
                sessionKey: string;
            };
        ...
        case 'settings:think-level' as AgentRuntimeEventType: {
            const { level } = event as unknown as {
                level: string;
            };
            try {
        ...
        case 'settings:backend-changed' as AgentRuntimeEventType: {
            const { backendId } = event as unknown as {
                backendId: string;
            };
            try {
                localStorage.setItem('mtbot:acp-backend', backendId);
            }
            catch { }
