 ...
 * })
 */
export function useQuery<T>(options: UseQueryOptionsObject<T>): UseQueryReturn<T> {
    const { queryKey, queryFn, enabled = true, refetchInterval = 0, refetchOnWindowFocus = false, retryCount = 3, retryDelay = 1000, onSuccess, onError, } = options;
    // 将 queryKey 转换为字符串
