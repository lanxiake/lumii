 ...
 */
function mixMicIntoDestination(audioCtx: AudioContext, micStream: MediaStream): MediaStream {
 ...
 * 两路皆无时返回 null。
 */
export function mixDesktopAndMic(audioCtx: AudioContext, desktopStream: MediaStream | null, micStream: MediaStream | null): MediaStream | null {
    const dest = audioCtx.createMediaStreamDestination();
    let connected = false;
 ...
 */
export async function splitBlobToChunks(blob: Blob, maxBytes: number): Promise<ArrayBuffer[]> {
