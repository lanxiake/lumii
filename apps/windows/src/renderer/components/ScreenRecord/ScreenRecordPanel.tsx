/**
 * ScreenRecordPanel — 录制 / 成片 Tab + 字幕编辑入口
 */
...
export const ScreenRecordPanel: React.FC<ScreenRecordPanelProps> = ({
  ...
  const [editorImport, setEditorImport] = useState<string | undefined>(undefined)
  ...
  const refreshLibrary = useCallback(async () => {
    ...
    try {
      ...
      else setLibraryMsg(`列表失败：${r.error}`)
    } catch (e) {
    ...
    <>
                          ...
                          void onNarrate({ path: lastRecording.path, cues })
                            .then((raw) => {
                              ...
                              if (r?.ok && r.path) {
                                ...
                                setEditorPath(lastRecording.path)
                              } else {
            ...
            <>
              <div className={styles.panelBody}>
                <div className={styles.libraryToolbar}>
                  <Button variant="ghost" size="sm" disabled={libraryLoading} onClick={() => void refreshLibrary()}>
                    {libraryLoading ? '刷新中…' : '刷新'}
                  </Button>
                  {libraryMsg && <span className={styles.switchHint}>{libraryMsg}</span>}
                </div>
                <ul className={styles.libraryList}>
                  ...
                  {!libraryLoading && recordings.length === 0 && (
                    <li className={styles.empty}>暂无成片</li>
                  )}
                </ul>
              </div>
              <footer className={styles.panelFooter}>
      ...
      {editorPath && (
        <RecordingSubtitleEditor
          ...
          initialImportText={editorImport}
          onClose={() => {
            setEditorPath(null)
            setEditorImport(undefined)
