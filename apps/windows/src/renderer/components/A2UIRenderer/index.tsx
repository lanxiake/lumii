...
import type { A2UISpec, A2UIComponent, A2UIText, A2UICard, A2UIImage, A2UIButton, A2UIList, A2UIDivider, } from './types';
...
const CardComponent: React.FC<A2UICard & {
    onAction?: ActionHandler;
}> = ({ title, subtitle, components, onAction }) => (<div className={styles['a2ui-card']}>
    {title && <div className={styles['a2ui-card-title']}>{title}</div>}
    {subtitle && <div className={styles['a2ui-card-subtitle']}>{subtitle}</div>}
    {components && components.map((comp) => (<ComponentDispatch key={comp.id} component={comp} onAction={onAction}/>))}
  </div>);
const ImageComponent: React.FC<A2UIImage> = ({ src, alt, width, height }) => (<img className={styles['a2ui-image']} src={src} alt={alt || ''} width={width} height={height} loading="lazy" onClick={() => src && window.open(src, '_blank')}/>);
const ButtonComponent: React.FC<A2UIButton & {
    onAction?: ActionHandler;
}> = ({ id, label, variant, disabled, onAction }) => (<button className={`${styles['a2ui-button']} ${styles[`a2ui-button-${variant || 'primary'}`] || ''}`} disabled={disabled} onClick={() => onAction?.(id, 'click', { label })}>
    {label}
  </button>);
const ListComponent: React.FC<A2UIList & {
    onAction?: ActionHandler;
}> = ({ items, ordered, onAction }) => {
    const Tag = ordered ? 'ol' : 'ul';
    return (<Tag className={styles['a2ui-list']}>
...
const FallbackComponent: React.FC<{
    type: string;
}> = ({ type }) => (<div className={styles['a2ui-fallback']}>未知组件类型: {type}</div>);
// ---------------------------------------------------------------
// 组件分发
// ---------------------------------------------------------------
type ActionHandler = (componentId: string, action: string, payload?: unknown) => void;
const ComponentDispatch: React.FC<{
    ...
    switch (component.type) {
        ...
        default:
            return <FallbackComponent type={(component as {
                type: string;
            }).type}/>;
    }
};
...
export const A2UIRenderer: React.FC<A2UIRendererProps> = ({ spec, onAction }) => {
    if (!spec.components || spec.components.length === 0)
        return null;
    return (<div className={styles['a2ui-surface']}>
      {spec.components.map((comp) => (<ComponentDispatch key={comp.id} component={comp} onAction={onAction}/>))}
    </div>);
};
