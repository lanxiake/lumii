...
export function ArtifactBlock({ content, language = 'text', title, messageStreaming = false }: ArtifactBlockProps) {
    ...
    const codeView = (<pre className={styles['code-block']}>
      {/* dangerouslySetInnerHTML: hljs 输出已对内容 HTML 转义，安全 */}
      <code className={`hljs language-${language.toLowerCase()}`} 
    // eslint-disable-next-line react/no-danger
    dangerouslySetInnerHTML={{ __html: highlightedHtml }}/>
    </pre>);
    const previewView = isPreviewable ? (<IframeArtifact key={runKey} content={content} language={language} title={title} fullscreen={false}/>) : null;
    // 全屏弹窗（Portal 到 document.body）
    const fullscreenModal = fullscreen
        ? createPortal(<div className={styles['fullscreen-overlay']} onClick={(e) => { if (e.target === e.currentTarget)
            setFullscreen(false); }}>
          <div className={styles['fullscreen-dialog']}>
            <div className={styles['floating-controls']}>
              <button className={styles['floating-btn']} onClick={() => setFullscreen(false)} title="缩小预览" aria-label="缩小预览">
                缩小
              </button>
              <button className={`${styles['floating-btn']} ${styles['floating-btn-danger']}`} onClick={() => {
                setFullscreen(false);
            ...
            <ArtifactToolbar language={language} onCopy={handleCopy} onTogglePreview={handleTogglePreview} onToggleFullscreen={handleToggleFullscreen} onRerun={handleRerun} previewActive={previewActive} fullscreen={true}/>
            <div className={styles['fullscreen-content']}>
              {previewActive && isPreviewable ? (<IframeArtifact key={`fullscreen-${runKey}`} content={content} language={language} title={title} fullscreen={true}/>) : (codeView)}
            </div>
          </div>
        </div>, document.body)
        : null;
    return (<>
      <div className={styles['artifact-block']}>
        <ArtifactToolbar language={language} onCopy={handleCopy} onTogglePreview={handleTogglePreview} onToggleFullscreen={handleToggleFullscreen} onRerun={handleRerun} previewActive={previewActive} fullscreen={false}/>
