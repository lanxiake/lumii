 ...
 */
import type { DashboardFeedCursor, DashboardFeedMeta, DashboardFeedPage, DashboardFeedBatchPage, DashboardFeedBatchCursor, } from '@main/dashboard-feed-store';
...
export async function fetchFeedBatches(feedId: string, opts?: {
    limit?: number;
    before?: DashboardFeedBatchCursor | null;
}): Promise<DashboardFeedBatchPage> {
    const api = window.electronAPI?.dashboardFeed;
    if (!api?.batches)
        throw new Error('资讯接口不可用');
    const res = await api.batches(feedId, opts);
    if (!res)
        throw new Error('资讯接口不可用');
    if (!res.success)
        throw new Error(res.error ?? '读取资讯失败');
