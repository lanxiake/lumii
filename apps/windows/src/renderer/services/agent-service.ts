 ...
 * Fork 系统 Agent
 */
export async function forkAgent(systemAgentId: string, data: {
    name?: string;
    description?: string;
    systemPrompt?: string;
} = {}): Promise<Agent> {
...
type NullableUpdateKeys = 'skillFilter' | 'skillBlacklist' | 'whenToUse' | 'triggerExamples' | 'bundledSkills' | 'category' | 'description' | 'systemPrompt';
/**
 ...
 * 更新用户 Agent
 */
export async function updateAgent(agentId: string, data: AgentUpdatePayload): Promise<Agent> {
