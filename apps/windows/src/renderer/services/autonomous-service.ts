...
export async function updateAutonomousSettings(settings: Parameters<typeof window.electronAPI.autonomous.updateSettings>[0]) {
...
/** 获取内心日记流（时间倒序，游标分页） */
export async function getDiary(limit?: number, before?: {
    timestamp: number;
    id: string;
} | null) {
...
/** 拒绝目标 */
export async function rejectGoal(goalId: string, options?: {
    reason?: string;
...
async function setAutonomousAgents(agentIds: string[]): Promise<{
    ok: boolean;
}> {
