...
}> = ({ value, children }) => (<ToolFilePreviewContext.Provider value={value}>
    {children}
...
function extractToolFileInfo(toolName: string, input: Record<string, unknown> | undefined): ToolFileInfo | null {
    if (!input)
        return null;
    ...
    if (lname.includes('todo'))
        return null;
    const isRead = lname.includes('file_read') ||
        lname === 'read' ||
        lname === 'readfile' ||
        lname === 'readlocalfile' ||
        lname.includes('view');
    const isWrite = lname.includes('file_write') ||
...
const CopyButton: React.FC<{
    ...
    return (<button className={clsx(styles['copy-btn'], copied && styles['copy-btn--copied'])} onClick={handleCopy} title="复制">
      {copied ? '✓ 已复制' : '复制'}
    </button>);
};
/** 详情块：标签 + 可选复制按钮 + 内容 */
const DetailBlock: React.FC<{
    ...
    copyText?: string;
    children: React.ReactNode;
}> = ({ label, copyText, children }) => (<div className={styles.detailBlock}>
...
function extractTargetName(input: Record<string, unknown> | undefined, toolName: string): string {
    ...
    if (lname.includes('todo')) {
        ...
        if (action === 'list')
            return '全部任务';
        if (action === 'create' && input.subject)
            return truncate(String(input.subject), 40);
    ...
    if (task)
        return truncate(String(task), 40);
    return '';
}
...
export function getStatusLabel(item: AgentWorkflowItem): string {
    ...
    if (item.type === 'subagent') {
        ...
        if (item.status === 'running')
            return `启动子 Agent: ${truncate(task, 40)}`;
        if (item.status === 'completed')
            return `子 Agent 完成: ${truncate(task, 40)}`;
        if (item.status === 'failed')
            return `子 Agent 失败: ${truncate(task, 40)}`;
    ...
    if (lname.includes('todo')) {
        ...
        if (action === 'list')
            return `${prefix}列出任务`;
        if (action === 'create')
            return target ? `${prefix}创建任务：${target}` : `${prefix}创建任务`;
        if (action === 'batch_create')
            return target ? `${prefix}${target}` : `${prefix}批量创建任务`;
        if (action === 'update')
            return target ? `${prefix}更新任务 ${target}` : `${prefix}更新任务`;
        if (action === 'batch_update')
            return target ? `${prefix}${target}` : `${prefix}批量更新任务`;
        if (action === 'delete')
            return target ? `${prefix}删除任务 ${target}` : `${prefix}删除任务`;
...
function formatInputSummary(input: Record<string, unknown> | undefined, toolName: string): string {
    ...
    if (lname.includes('todo')) {
        ...
        if (input.subject)
            lines.push(`标题: ${String(input.subject)}`);
        if (input.taskId)
            lines.push(`任务 ID: ${String(input.taskId)}`);
        if (input.status)
            lines.push(`状态: ${String(input.status)}`);
        return lines.join('\n');
    }
    ...
    if (lname.includes('grep') || (lname.includes('search') && !lname.includes('web'))) {
        ...
        if (input.pattern || input.query)
            lines.push(`搜索: ${String(input.pattern || input.query)}`);
        if (input.path)
            lines.push(`路径: ${String(input.path)}`);
        if (input.glob)
            lines.push(`过滤: ${String(input.glob)}`);
        if (lines.length > 0)
            return lines.join('\n');
    }
    // Glob 模式
    if (lname.includes('glob')) {
        const lines: string[] = [];
        if (input.pattern)
            lines.push(`模式: ${String(input.pattern)}`);
        if (input.path)
            lines.push(`路径: ${String(input.path)}`);
        if (lines.length > 0)
            return lines.join('\n');
    ...
    if (filePath) {
        ...
        if (input.pattern)
            lines.push(`模式: ${String(input.pattern)}`);
        if (input.old_string)
            lines.push(`查找: ${truncate(String(input.old_string), 60)}`);
        if (input.new_string)
            lines.push(`替换: ${truncate(String(input.new_string), 60)}`);
        if (input.offset)
            lines.push(`偏移: ${input.offset}`);
        if (input.limit)
            lines.push(`行数: ${input.limit}`);
        return lines.join('\n');
    }
    // Bash 命令
...
function parseScreenshotToolPayload(output: Record<string, unknown>): ScreenshotToolPayload | null {
    ...
    for (const block of content) {
        if (!block || typeof block !== 'object' || (block as {
            type?: string;
        }).type !== 'text')
            continue;
        const text = (block as {
 ...
 */
function extractScreenRecordOutputPaths(toolName: string, output: unknown): Array<{
    filePath: string;
    fileName: string;
    label: string;
}> {
    const lname = (toolName || '').toLowerCase();
    if (lname !== 'screen_record_stop' && lname !== 'screen_record_narrate')
        return [];
...
const ImageGeneratePreview: React.FC<{
    output: unknown;
    fullSize?: boolean;
}> = ({ output, fullSize = false }) => {
    ...
    return (<div className={styles.imagePreviewWrap}>
      ...
      {!loading && dataUrl && (<>
          <img src={dataUrl} alt={fileName} className={fullSize ? styles.imageFull : styles.imageThumb} title="点击放大查看" style={{ cursor: 'zoom-in' }} onClick={(e) => { e.stopPropagation(); setZoomed(true); }}/>
          {fullSize && imgDetails.width > 0 && (<div className={styles.imageMeta}>
              {imgDetails.model
                    ? `${imgDetails.width}×${imgDetails.height} · ${imgDetails.model} · ${fileName}`
                    : `${imgDetails.width}×${imgDetails.height} · ${fileName}`}
            </div>)}
          {zoomed && (<ImageLightbox src={dataUrl} alt={fileName} onClose={() => setZoomed(false)}/>)}
        </>)}
    </div>);
};
// ─── 智能输出组件 ─────────────────────────────────────
...
const TaskStatusBadge: React.FC<{
    ...
    return (<span className={clsx(styles.todoStatusBadge, badgeClass)}>
      {label}
    </span>);
};
/** 渲染任务行 */
...
}> = ({ task }) => (<li key={String(task.id ?? task.subject)} className={styles.todoListItem}>
 ...
 */
const TodoWriteInlinePreview: React.FC<{
    output: unknown;
}> = ({ output }) => {
    ...
    if (tasks && Array.isArray(tasks)) {
        ...
        {tasks.map((t) => (<TaskRow key={String(t.id ?? t.subject)} task={t}/>))}
      </ul>);
    }
    ...
    if (task && typeof task === 'object' && (task.id !== undefined || task.subject !== undefined)) {
        return (<div className={styles.todoSingle}>
        <span>
          <span className={styles.todoId}>#{String(task.id ?? '')}</span>
          <span className={styles.todoSubject}>{String(task.subject ?? '')}</span>
          {task.status != null && <TaskStatusBadge status={String(task.status)}/>}
        </span>
        {task.description != null && (<span className={styles.todoMeta}>{truncate(String(task.description), 72)}</span>)}
      </div>);
    }
    if (payload.status === 'partial' && tasks !== undefined) {
        // batch_update 部分成功
        return (<div className={styles.todoMeta}>
...
const SmartOutput: React.FC<{
    ...
    return (<>
      ...
      {isLong && (<span className={styles.outputToggle} onClick={() => setExpanded((p) => !p)}>
...
const ChildToolRow: React.FC<{
    ...
    const showChildTodoPreview = isChildTodoWrite && child.status === 'completed';
    const fileInfo = useMemo(() => extractToolFileInfo(child.name, child.input), [child.name, child.input]);
    ...
    const handlePreviewFile = useCallback((e: React.MouseEvent) => {
        e.stopPropagation();
        if (!fileInfo || !previewCtx)
            return;
    ...
    return (<div className={clsx(styles.childToolRow, statusClass)}>
      <div className={styles.childToolHeader} onClick={() => setIsExpanded((p) => !p)}>
        ...
        {fileInfo && (<span className={clsx(styles.fileChip, canPreviewFile && styles.fileChipClickable)} title={canPreviewFile ? `点击预览 ${fileInfo.filePath}` : fileInfo.filePath} onClick={canPreviewFile ? handlePreviewFile : undefined}>
            <span className={styles.fileChipName}>{fileInfo.fileName}</span>
            {lineRangeText && (<span className={styles.fileChipRange}>{lineRangeText}</span>)}
          </span>)}
        {(child.endTime || child.status === 'running') && (<span className={styles.childToolDuration}>
            {formatDuration(child.startTime, child.endTime)}
          </span>)}
        <span className={styles.childToolToggle}>{isExpanded ? '∧' : '∨'}</span>
      </div>
      {showChildTodoPreview && (<div className={styles.todoPreviewWrap} onClick={(e) => e.stopPropagation()}>
          <TodoWriteInlinePreview output={child.output}/>
        </div>)}
      {isExpanded && (<div className={styles.childToolDetails}>
          {child.input && Object.keys(child.input).length > 0 && (<DetailBlock label="输入" copyText={formatParams(child.input)}>
              <pre className={styles.code}>{formatInputSummary(child.input, child.name)}</pre>
            </DetailBlock>)}
          {child.output !== undefined && (<DetailBlock label="输出" copyText={formatParams(child.output)}>
              <SmartOutput output={child.output}/>
            </DetailBlock>)}
          {child.error && (<div className={styles.detailBlock}>
...
const SubagentCard: React.FC<{
    ...
    return (<div className={clsx(styles.subagentRow, isRunning && styles.running, isFailed && styles.failed)}>
      {/* 头部行 */}
      <div className={styles.subagentHeader} onClick={handleToggle}>
        ...
        <div className={styles.subagentMain}>
          ...
          <div className={styles.subagentNameRow}>
            ...
            {(isCompleted || isFailed || isRunning) && (<span className={styles.subagentDuration}>{duration}</span>)}
          ...
          {/* 运行中：状态提示 */}
          {isRunning && !errorMsg && (<span className={styles.subagentRunningHint}>
              {item.childItems && item.childItems.length > 0
                ? `正在执行… (${item.childItems.length} 个工具调用)`
                : '子 Agent 正在执行中…'}
            </span>)}
          {/* 折叠状态：显示工具调用数量 */}
          {!isRunning && item.childItems && item.childItems.length > 0 && !isExpanded && (<span className={styles.subagentChildCount}>
              {item.childItems.length} 个工具调用
            </span>)}
          {/* 失败：内联错误 */}
          {isFailed && errorMsg && (<span className={styles.subagentInlineError}>{truncate(errorMsg, 80)}</span>)}
          {/* 完成：内联摘要 */}
          {isCompleted && summary && (<span className={styles.subagentInlineSummary}>{truncate(summary, 80)}</span>)}
        </div>
      ...
      {/* 展开详情 */}
      {isExpanded && (<div className={styles.subagentDetails}>
          {/* 会话信息 */}
          <div className={styles.subagentSessionInfo}>
            {item.childSessionKey && (<div className={styles.subagentSessionRow}>
          ...
          {/* 执行摘要（折叠区顶部突出显示） */}
          {summary && (<DetailBlock label="执行摘要" copyText={summary}>
              <pre className={styles.code}>{summary}</pre>
            </DetailBlock>)}

          {/* 完整任务输出 */}
          {fullOutput && fullOutput !== summary && (<DetailBlock label="任务输出" copyText={fullOutput}>
              <SmartOutput output={fullOutput}/>
            </DetailBlock>)}

          {/* 如果 output 是非标准结构（没有 result/text/content/summary），显示原始 output */}
          {outputObj && !summary && !fullOutput && (<DetailBlock label="任务输出" copyText={formatParams(outputObj)}>
              <SmartOutput output={outputObj}/>
            </DetailBlock>)}

          {/* 子 Agent 工具调用列表 */}
          {item.childItems && item.childItems.length > 0 && (<DetailBlock label={`工具调用 (${item.childItems.length})`}>
              <div className={styles.childItemsList}>
                {item.childItems.map((child) => (<ChildToolRow key={child.toolCallId} child={child}/>))}
              </div>
            </DetailBlock>)}
          ...
          {/* 错误信息 */}
          {errorMsg && (<DetailBlock label="错误" copyText={errorMsg}>
              <span className={styles.errorText}>{errorMsg}</span>
            </DetailBlock>)}

          {!summary && !fullOutput && !outputObj && !errorMsg && isRunning && (<div className={styles.detailBlock}>
...
const ToolCallCard: React.FC<ToolCallCardProps> = ({ item }) => {
    ...
    const showTodoPreview = isTodoWrite && !isRunning && (item.status === 'completed' || item.status === 'failed');
    const toolNameLower = (item.name || '').toLowerCase();
    const isImagePreviewTool = toolNameLower === 'image_generate' ||
        toolNameLower === 'app_screenshot' ||
        toolNameLower === 'screen_screenshot';
    const showImagePreview = isImagePreviewTool && item.status === 'completed' && !!extractToolImagePreviewDetails(item.output);
    // 提取文件读写元数据：存在时在头部显示"可点击文件名 + 行号范围"
    const fileInfo = useMemo(() => extractToolFileInfo(item.name, item.input), [item.name, item.input]);
    const lineRangeText = fileInfo ? formatLineRange(fileInfo.startLine, fileInfo.endLine) : '';
    const canPreviewFile = !!(fileInfo && previewCtx && !isRunning);
    /** 录屏成片 chip（stop / narrate 结果 path） */
    const recordingChips = useMemo(() => item.status === 'completed' ? extractScreenRecordOutputPaths(item.name, item.output) : [], [item.name, item.output, item.status]);
    const handlePreviewFile = useCallback((e: React.MouseEvent) => {
        e.stopPropagation();
        if (!fileInfo || !previewCtx)
            return;
     ...
     */
    const handlePreviewRecording = useCallback((e: React.MouseEvent, filePath: string, fileName: string) => {
        e.stopPropagation();
        if (!previewCtx)
            return;
        previewCtx.onPreview({ filePath, fileName });
    }, [previewCtx]);
    return (<div className={clsx(styles.row, isRunning && styles.running, isFailed && styles.failed, isExpanded && styles.expanded)}>
      {/* 单行头部 */}
      <div className={styles.rowHeader} onClick={handleToggle}>
        <span className={clsx(styles.icon, isRunning && styles.spinIcon)}>
          {isRunning ? '◌' : icon}
        </span>
        {item.agentLabel && (<span className={styles.agentLabelBadge}>{item.agentLabel}</span>)}
        ...
        {recordingChips.map((chip) => (<span key={`${chip.label}:${chip.filePath}`} className={clsx(styles.fileChip, previewCtx && styles.fileChipClickable)} title={previewCtx ? `点击预览 ${chip.filePath}` : chip.filePath} onClick={previewCtx
        ...
        {(item.status === 'completed' || item.status === 'failed' || item.status === 'running' || item.status === 'interrupted') && (<span className={styles.duration}>
      ...
      {showTodoPreview && (<div className={styles.todoPreviewWrap} onClick={(e) => e.stopPropagation()}>
          <TodoWriteInlinePreview output={item.output}/>
        </div>)}

      {showImagePreview && !isExpanded && (<div onClick={(e) => e.stopPropagation()}>
          <ImageGeneratePreview output={item.output} fullSize={false}/>
        </div>)}

      {/* 可展开的详情区 */}
      {isExpanded && (<div className={styles.details}>
          {showImagePreview && (<ImageGeneratePreview output={item.output} fullSize={true}/>)}
          {item.input && Object.keys(item.input).length > 0 && (<DetailBlock label="输入" copyText={formatParams(item.input)}>
              <pre className={styles.code}>{formatInputSummary(item.input, item.name)}</pre>
            </DetailBlock>)}
          {!showImagePreview && item.output !== undefined && (<DetailBlock label="输出" copyText={formatParams(item.output)}>
              <SmartOutput output={item.output}/>
            </DetailBlock>)}
          {item.error && (<DetailBlock label="错误" copyText={item.error}>
