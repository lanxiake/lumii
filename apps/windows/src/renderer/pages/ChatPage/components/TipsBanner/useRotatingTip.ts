...
export const INPUT_TIPS: InputTip[] = [
  ...
  // 「运行中怎么和 Agent 交互」是这组提示里最容易踩空的一类：等整轮跑完才发消息会白等，
  // 而插话能立刻把话递进去（在下一次模型调用前生效）
  { text: 'AI 正在执行长任务时，点「中途插话」可以补充要求、纠正方向，不用等它跑完' },
  { text: '想知道 AI 做到哪了？运行中点「中途插话」问一句「现在进展如何」，它会在当前步骤结束后汇报' },
]
