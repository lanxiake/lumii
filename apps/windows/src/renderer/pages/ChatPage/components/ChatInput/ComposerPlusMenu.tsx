 ...
 */
const ComposerPlusMenu: React.FC<ComposerPlusMenuProps> = ({ disabled = false, onAttachFiles, agents = [], selectedAgent = null, agentsLoading = false, onAgentChange, onManageSkills, onManageMcp, onManageAgents, onOpenWiki, }) => {
    ...
    useEffect(() => {
        ...
        void (async () => {
            ...
            setSessionDisabledMcp((mcp as {
                disabledServers?: readonly string[];
            })?.disabledServers ?? []);
            setSessionDisabledSkills((skills as {
                disabledSkills?: readonly string[];
            })?.disabledSkills ?? []);
        })();
    }, [open, sessionKey]);
    const { installedSkills, isLoading: skillsLoading } = useSkills();
    const { tools, mcpStatus, isLoading: toolsLoading, refresh: refreshTools, } = useToolSearch();
    /** 关闭菜单并复位到主面板 */
    ...
    const mcpServers = useMemo(() => {
        ...
        return Array.from(byServer.entries())
            ...
            if (row.name.toLowerCase().includes(q))
                return true;
            return row.tools.some((t) => t.name.toLowerCase().includes(q) || t.label.toLowerCase().includes(q));
        })
    ...
    return (<div className={styles.root} ref={rootRef}>
      <button type="button" className={clsx(styles.plusBtn, open && styles.plusBtnActive)} onClick={toggleOpen} disabled={disabled} aria-label="添加附件、技能与 Agent" title={`添加附件 / 技能 / MCP / Agent（当前：${currentAgentLabel}）`} aria-expanded={open} aria-haspopup="menu">
      ...
      {open && (<div className={styles.menu} role="menu">
          {panel === 'main' && (<>
              <button type="button" className={styles.menuItem} role="menuitem" onClick={() => {
                    onAttachFiles();
                    closeMenu();
                }}>
              ...
              {onOpenWiki && (<button type="button" className={styles.menuItem} role="menuitem" onClick={() => {
                        onOpenWiki();
                        closeMenu();
                    }}>
                  <span className={styles.menuIcon} aria-hidden>
          ...
          {panel === 'skills' && (<SubPanel title="技能" query={query} onQueryChange={setQuery} searchPlaceholder="搜索技能…" searchInputRef={searchInputRef} onBack={() => { setPanel('main'); setQuery(''); }} manageLabel="管理技能" onManage={() => { onManageSkills?.(); closeMenu(); }}>
              {skillsLoading ? (<div className={styles.empty}>加载中…</div>) : filteredSkills.length === 0 ? (<div className={styles.empty}>{query ? '没有匹配的技能' : '暂无已安装技能'}</div>) : (filteredSkills.map((skill) => {
                    const name = skill.skill?.name || skill.skillItemId;
                    const busy = togglingSkillId === skill.skillItemId;
                    return (<div key={skill.id} className={styles.listRow}>
                      <div className={styles.listMain}>
                        <span className={styles.listName} title={skill.skill?.description}>{name}</span>
                        {!skill.isEnabled ? (<span className={styles.listDesc}>已在技能中心全局停用</span>) : skill.skill?.description ? (<span className={styles.listDesc}>{skill.skill.description}</span>) : null}
                      </div>
                      <Switch size="sm" checked={skill.isEnabled && !sessionDisabledSkills.includes(skill.skillItemId)} disabled={busy || !skill.isEnabled} onChange={(checked) => void handleToggleSkill(skill.skillItemId, checked)}/>
                    </div>);
                }))}
            </SubPanel>)}

          {panel === 'mcp' && (<SubPanel title="MCP 服务" query={query} onQueryChange={setQuery} searchPlaceholder="搜索 MCP…" searchInputRef={searchInputRef} onBack={() => { setPanel('main'); setQuery(''); }} manageLabel="管理 MCP" onManage={() => { onManageMcp?.(); closeMenu(); }}>
              {toolsLoading && mcpServers.length === 0 ? (<div className={styles.empty}>加载中…</div>) : mcpServers.length === 0 ? (<div className={styles.empty}>{query ? '没有匹配的 MCP' : '暂无 MCP 服务'}</div>) : (mcpServers.map((server) => {
                    const busy = togglingMcpServer === server.name;
                    const errorHint = server.serverEnabled && !server.connected ? server.lastError : undefined;
                    return (<div key={server.name} className={styles.listRow}>
                      <div className={styles.listMain}>
                        <span className={styles.listName}>
                          {server.name}
                          {errorHint ? (<span className={styles.listNameError} title={errorHint}>
                              {' '}· {errorHint}
                            </span>) : null}
                        </span>
                        <span className={clsx(styles.listDesc, !server.serverEnabled ? undefined : !server.connected && styles.listDescError)}>
                      ...
                      {server.tools.length > 0 || server.serverEnabled ? (<Switch size="sm" checked={server.enabled} disabled={busy || !server.serverEnabled || !sessionKey} onChange={(checked) => void handleToggleMcpServer(server.name, checked)}/>) : (<span className={styles.listStatus} title={errorHint}>
          ...
          {panel === 'agents' && (<SubPanel title="切换 Agent" query={query} onQueryChange={setQuery} searchPlaceholder="搜索 Agent…" searchInputRef={searchInputRef} onBack={() => { setPanel('main'); setQuery(''); }} manageLabel="管理 Agent" onManage={() => { onManageAgents?.(); closeMenu(); }}>
              {agentsLoading ? (<div className={styles.empty}>加载中…</div>) : (<>
                  {(!query.trim() || '系统默认'.includes(query.trim())) && (<button type="button" className={clsx(styles.selectRow, !selectedAgent && styles.selectRowActive)} onClick={() => handleSelectAgent(null)}>
                      <span className={styles.listName}>系统默认</span>
                      {!selectedAgent && <span className={styles.checkMark} aria-hidden>✓</span>}
                    </button>)}
                  {filteredAgents.length === 0 && query.trim() ? (<div className={styles.empty}>没有匹配的 Agent</div>) : (filteredAgents.map((agent) => (<button type="button" key={agent.id} className={clsx(styles.selectRow, selectedAgent?.id === agent.id && styles.selectRowActive)} onClick={() => handleSelectAgent(agent)}>
                        <span className={styles.listName}>{agent.name}</span>
                        {selectedAgent?.id === agent.id && (<span className={styles.checkMark} aria-hidden>✓</span>)}
                      </button>)))}
                </>)}
            </SubPanel>)}
        </div>)}
...
/** 子面板：返回 + 搜索 + 列表 + 管理入口 */
function SubPanel({ title, query, onQueryChange, searchPlaceholder, searchInputRef, onBack, manageLabel, onManage, children, }: SubPanelProps) {
    return (<div className={styles.subPanel}>
      <div className={styles.subHeader}>
        ...
        <input ref={searchInputRef} className={styles.searchInput} value={query} onChange={(e) => onQueryChange(e.target.value)} placeholder={searchPlaceholder} aria-label={searchPlaceholder}/>
        {onManage && (<button type="button" className={styles.manageLink} onClick={onManage} title={manageLabel}>
            管理
