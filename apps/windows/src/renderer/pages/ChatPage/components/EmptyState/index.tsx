...
import { Mail, ClipboardList, BarChart2, Calendar, FileText, Target, FileSignature, Briefcase, PenLine, MessageSquare, Video, Newspaper, Mic, BookOpen, ShoppingBag, Search, GraduationCap, Languages, Lightbulb, Bug, Zap, Terminal, Layers, TestTube, RefreshCw, Globe, UtensilsCrossed, Plane, PiggyBank, Dumbbell, Gift, Home, Smartphone, Heart, FilePenLine as FileUser, Handshake, TrendingUp, Presentation, Bot, Clock, FolderOpen, Users, } from 'lucide-react';
...
const EmptyState: React.FC<EmptyStateProps> = ({ onSuggestionClick }) => {
    ...
    return (<div className={styles['empty-state']}>
      ...
      <div className={styles['empty-suggestions']}>
        {displayed.map((item, index) => {
            ...
            return (<button key={index} className={styles['suggestion-btn']} onClick={() => onSuggestionClick?.(item.prompt)} title={item.category}>
