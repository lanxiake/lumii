...
import { parseA2UIBlocks, tryParseA2UISpecFromJson, tryParseA2UIBlockBodyLoose, } from '../../../../utils/parseA2UIBlocks';
...
import { readRunFailure, readRunInterrupted, type SubAgentRun, } from '../ChatContainer/sub-agent-runs';
...
import { markLargeCodeBlocks, PLAIN_TEXT_THRESHOLD, MATH_MAX_CHARS, type HastNode, } from './markdown-limits';
 ...
 */
const MarkdownInlineImage: React.FC<{
    src?: string;
    alt?: string;
}> = ({ src, alt }) => {
    ...
    return (<>
      <img src={src} alt={alt || ''} className={styles['md-image']} loading="lazy" style={{ cursor: 'zoom-in' }} title="点击预览" onClick={handleClick}/>
      {external && lightboxOpen && (<ImageLightbox src={src} alt={fileName} onClose={() => setLightboxOpen(false)}/>)}
    </>);
};
    ...
    return {
        code({ className, children, ...props }) {
          ...
          <div className={styles['code-header']}>
            ...
            <button className={styles['code-copy-btn']} onClick={() => navigator.clipboard.writeText(codeText)}>
              复制
            </button>
          </div>
...
const LargeTextBlock: React.FC<LargeTextBlockProps> = ({ content }) => {
    ...
    return (<div className={styles['large-text']}>
      ...
      {mode === 'plain' ? (<pre className={styles['large-text__body']}>{content}</pre>) : (<ReactMarkdown remarkPlugins={remarkPluginsFor(content)} rehypePlugins={rehypePluginsFor(content)} components={LARGE_TEXT_MARKDOWN_COMPONENTS}>
          {content}
        </ReactMarkdown>)}
    </div>);
};
...
function MediaFileChips({ mediaFiles }: {
    mediaFiles: Array<{
        filePath: string;
        fileName: string;
    }>;
    ...
    return (<div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginTop: 4 }}>
      {mediaFiles.map((f) => (<span key={f.filePath} className={clsx(toolCardStyles.fileChip, previewCtx && toolCardStyles.fileChipClickable)} title={previewCtx ? `点击预览 ${f.filePath}` : f.filePath} onClick={previewCtx ? (e) => { e.stopPropagation(); previewCtx.onPreview({ filePath: f.filePath, fileName: f.fileName }); } : undefined}>
...
const ThinkingBlock: React.FC<ThinkingBlockProps> = ({ thinkingText, isStreaming, isLive, compact = false }) => {
    ...
    return (<div className={clsx(styles['rt-thinking-card'], compact && styles['rt-thinking-card--compact'])}>
      <button type="button" className={styles['rt-thinking-card-header']} onClick={handleToggle} aria-expanded={expanded} title={expanded ? '收起思考内容' : '展开思考内容'}>
        <span className={clsx(styles['rt-unit-chevron'], expanded && styles['rt-unit-chevron--open'])} aria-hidden>›</span>
        {isStreaming && (<span className={styles['rt-thinking-wave']} aria-hidden>
            <i /><i /><i /><i /><i />
          </span>)}
        <span className={styles['rt-thinking-card-title']}>{isStreaming ? '正在思考' : '思考'}</span>
        {isLive && isStreaming && (<span className={styles['rt-live-badge']}>实时</span>)}
        {!expanded && (<span className={styles['rt-thinking-card-preview']}>{preview}…</span>)}
        <span className={styles['rt-unit-hint']}>{expanded ? '收起' : '展开'}</span>
      </button>
      {expanded && (<pre ref={contentRef} className={clsx(styles['rt-thinking-pre'], heightLocked && styles['rt-thinking-pre--locked'])}>
          {thinkingText}
        </pre>)}
    </div>);
};
...
function toWorkflowItem(part: Extract<AssistantPart, {
    type: 'tool';
}>, context: TimelineContext): AgentWorkflowItem {
    return {
        id: part.id,
        type: 'tool',
        name: part.name,
 ...
 */
function readSpawnInstanceId(part: Extract<AssistantPart, {
    type: 'tool';
}>): string | undefined {
...
function buildRenderUnits(parts: readonly AssistantPart[], context: TimelineContext): RenderUnit[] {
    ...
    let pending: Extract<AssistantPart, {
        type: 'tool';
    }>[] = [];
    const flush = () => {
        if (pending.length === 0)
 ...
 */
function splitProcessAndAnswer(units: RenderUnit[]): {
    process: RenderUnit[];
    answer: RenderUnit[];
} {
...
function buildProcessSummary(process: RenderUnit[]): string {
    ...
    if (allTools.length > 0)
        parts.push(summarizeToolBatch(allTools));
...
// 组件
// ---------------------------------------------------------------
const ChatMessage: React.FC<ChatMessageProps> = ({ message, sessionBusy = false, toolItems, streamingThinkingText, noEnter = false, fileAttachments, replayMessageId, subAgentRuns, }) => {
     ...
     */
    const handleToolFilePreview = useCallback((args: {
        filePath: string;
        fileName: string;
        startLine?: number;
        endLine?: number;
    ...
    const handleEditSave = (newContent: string) => {
        ...
        const toSave = message.role === 'user'
    ...
    const buildCopyContent = useCallback((): string => {
        const parts: string[] = [];
        const thinking = (streamingThinkingText?.trim() ? streamingThinkingText : undefined)
    ...
    const markdownComponents = React.useMemo(() => buildMarkdownComponents(!!message.isStreaming), [message.isStreaming]);
    const renderTextContent = (content: string) => {
        // 流式输出中：跳过昂贵的 remark/rehype 解析，直接渲染纯文本，避免每个 delta 触发全量重解析
        if (message.isStreaming) {
            return (<div className={styles['message-content-text']}>
        ...
        if (segments.length === 1 && segments[0].type === 'markdown') {
            return (<div className={styles['message-content-text']}>
          <ReactMarkdown remarkPlugins={remarkPluginsFor(segments[0].content)} rehypePlugins={rehypePluginsFor(segments[0].content)} components={markdownComponents}>
            {segments[0].content}
          </ReactMarkdown>
        </div>);
        }
        ...
        {segments.map((seg, i) => {
                if (seg.type === 'markdown') {
                    return (<ReactMarkdown key={i} remarkPlugins={remarkPluginsFor(seg.content)} rehypePlugins={rehypePluginsFor(seg.content)} components={markdownComponents}>
                {seg.content}
              </ReactMarkdown>);
                }
                if (seg.type === 'a2ui') {
    ...
    const wrapSubAgent = (node: React.ReactNode) => {
        ...
        return (<SubAgentRunBlock run={run}>
        <div className={styles['sub-agent-inner']}>{node}</div>
      </SubAgentRunBlock>);
    };
    /** 流式首 token 未到达时的占位 */
    const renderThinkingPlaceholder = () => (<div className={styles['rt-placeholder']}>
     ...
     * 与折叠块外的最终答案（玻璃气泡）区分开。
     */
    const renderUnit = (unit: RenderUnit, inFold: boolean, runs: readonly SubAgentRun[] = [], isStreaming = false) => {
        if (unit.kind === 'thinking') {
            return (<ThinkingBlock key={unit.part.id} thinkingText={unit.part.text} isStreaming={unit.part.status === 'streaming' && !!message.isStreaming} isLive={!!message.isStreaming} compact={inFold}/>);
        }
        if (unit.kind === 'text') {
            return (<div key={unit.part.id} className={clsx(inFold ? styles['fold-note'] : styles['message-text'], styles['part-block'])}>
          {renderTextContent(unit.part.text)}
          {unit.part.status === 'streaming' && message.isStreaming && (<span className={styles['streaming-cursor']}/>)}
        </div>);
        }
        if (unit.kind === 'handoff') {
            return (<div key={unit.part.id} className={styles['part-block']}>
        ...
        if (unit.kind === 'spawn') {
            ...
            return (<div key={unit.part.id} className={styles['part-block']}>
          <SpawnAgentCard part={unit.part} messageStreaming={isStreaming} runs={claimed.length > 0 ? claimed : undefined} renderRunBody={(run) => renderTimeline(run.parts, {
        ...
        return (<div key={unit.key} className={styles['part-block']}>
     ...
     * 而被折叠隐藏。
     */
    const renderTimeline = (parts: readonly AssistantPart[], context: {
        timestamp: Date;
        runId?: string;
        isStreaming: boolean;
        durationMs?: number;
        ...
        const claimedInstanceIds = new Set(pinnedUnits
        ...
        return (<div className={styles['parts-timeline']}>
        {processOnly.length > 0 && (<ActivityFold summary={buildProcessSummary(processOnly)} currentStatus={isStreaming ? buildCurrentStatus(processOnly) : undefined} isStreaming={isStreaming} durationMs={context.durationMs} startTime={context.timestamp}>
            {processOnly.map((u) => renderUnit(u, true, [], isStreaming))}
          </ActivityFold>)}
        {pinnedUnits.map((u) => renderUnit(u, false, runs, isStreaming))}
        {unclaimedRuns.map((run) => (<SubAgentRunBlock key={run.instanceId} run={run}>
        ...
        {context.fileChanges && context.fileChanges.length > 0 && (<TurnFileChangesCard changes={context.fileChanges} onReview={actions.reviewFileChanges}/>)}
      </div>);
    };
    /** 父回合的时间线（渲染入口，保持既有调用点不变） */
    const renderPartsTimeline = () => renderTimeline(message.parts ?? [], {
        timestamp: message.timestamp,
        runId: message.runId,
        isStreaming: !!message.isStreaming,
    ...
    const renderLegacyAssistantBody = () => {
        ...
        if (!hasContent && !hasTools) {
            ...
            if (liveThinking || persistedThinking) {
                return (<ThinkingBlock thinkingText={liveThinking || persistedThinking || ''} isStreaming={!!message.isStreaming} isLive={!!liveThinking}/>);
            }
            return null;
        }
        return (<>
        {(liveThinking || persistedThinking) && (<ThinkingBlock thinkingText={liveThinking || persistedThinking || ''} isStreaming={!!message.isStreaming} isLive={!!liveThinking}/>)}
        {hasContent && (<div className={styles['message-text']}>
            {renderTextContent(message.content)}
            {message.isStreaming && <span className={styles['streaming-cursor']}/>}
          </div>)}
        {hasTools && (<div className={styles['part-block']}>
            <ToolBatchGroup items={toolItems!}/>
          </div>)}
        {message.fileChanges && message.fileChanges.length > 0 && (<TurnFileChangesCard changes={message.fileChanges} onReview={actions.reviewFileChanges}/>)}
      </>);
    };
    ...
    const renderMemoryHint = () => {
        ...
        const preview = injected.length === 1
            ? `「${first.content.length > 36 ? `${first.content.slice(0, 36)}…` : first.content}」`
            : `共 ${injected.length} 条`;
        return (<div className={styles['memory-hint']}>
        <button type="button" className={styles['memory-hint-btn']} onClick={() => setMemoryExpanded((v) => !v)} aria-expanded={memoryExpanded}>
          <span className={styles['memory-hint-icon']}><Lightbulb size={14}/></span>
          <span>根据记忆回复</span>
          <span className={styles['memory-hint-preview']}>{preview}</span>
        </button>
        {memoryExpanded ? (<ul className={styles['memory-hint-list']}>
            {injected.map((mem) => (<li key={mem.id}>
    ...
    const renderTokenUsage = () => {
        ...
        if (!hasUsage && !hasMetrics && !llmError)
            return null;
        return (<div className={styles['token-usage']}>
        {hasUsage && (<>
    ...
    const renderContent = () => {
        if (message.error) {
            return (<div className={clsx(styles['message-text'], styles['message-error'])}>
        ...
        if (message.role === 'system') {
            ...
            return (<div className={clsx(styles['message-text'], styles['message-system-info'])}>
        ...
        if (message.role === 'assistant') {
            if (message.isAborted) {
                ...
                return (<>
            {hasParts ? wrapSubAgent(renderPartsTimeline()) : (message.content && (<div className={styles['message-text']}>
        ...
        // User message
        if (message.isAborted) {
            return (<div className={styles['message-text']}>
          {message.content && (<div className={styles['message-content-partial']}>
              {renderTextContent(message.content)}
            </div>)}
          <div className={styles['message-aborted-badge']}>
        ...
        if (message.content) {
            ...
            if (message.isVoice) {
                ...
                return (<div className={styles['message-text']}>
            <div className={styles['voice-message-bubble']} onClick={message.audioWavBase64 ? handlePlayVoice : undefined} style={message.audioWavBase64 ? { cursor: 'pointer' } : undefined} title={message.audioWavBase64 ? '点击回放录音' : undefined}>
              <span className={styles['voice-wave']}>
                <span /><span /><span />
              </span>
            </div>
            {message.content && (<p className={styles['voice-message-transcript']}>{message.content}</p>)}
          </div>);
            }
          ...
          {mediaFiles.length > 0 && (<MediaFileChips mediaFiles={mediaFiles}/>)}
        </div>);
        }
        return (<div className={clsx(styles['message-text'], styles['message-empty'])}>
    ...
    return (<ToolFilePreviewProvider value={{ onPreview: handleToolFilePreview }}>
    <div ref={messageBubbleRef} className={clsx(styles.message, styles[message.role], message.isStreaming && styles.streaming, message.error && styles['has-error'], noEnter && styles['message--no-enter'])} 
    // 划词层靠这几个属性认出处（见 selection/snapshot.ts 的 readSource）。
    // 只加属性，不动渲染逻辑；role 为 system 时会被读取方忽略。
    data-lumii-source="chat-message" data-lumii-message-id={message.id} data-lumii-role={message.role}>
      {/* 内联文件预览：由工具卡片点击文件名触发，展示在消息顶部 */}
      {previewFileId && (<FilePreviewModal fileId={previewFileId} fileName={previewFileName} userId="local-user" mdBasePath={previewEditablePath} editablePath={previewEditablePath} onClose={() => { setPreviewFileId(null); setPreviewEditablePath(undefined); }}/>)}
      {previewByPath && (<FilePreviewModal filePath={previewByPath.filePath} fileName={previewByPath.fileName} userId="local-user" {...(typeof previewByPath.startLine === 'number' ? { startLine: previewByPath.startLine } : {})} {...(typeof previewByPath.endLine === 'number' ? { endLine: previewByPath.endLine } : {})} onClose={() => setPreviewByPath(null)}/>)}
      <div className={styles['message-avatar']}>{roleLabel}</div>
      <div className={styles['message-content-wrapper']}>
        {message.role === 'user' && message.isSteer && (<div className={message.steerPending
                ? `${styles['steer-badge']} ${styles['steer-badge--pending']}`
                : styles['steer-badge']} title={message.steerPending
        ...
        */}
        ...
        {!message.isStreaming && (<div className={styles['message-actions-wrap']}>
          <MessageActions messageId={message.id} role={message.role} content={message.role === 'user'
                ? parseMediaAttachments(message.content).textWithoutMedia
                : message.content} isEditing={isEditing} onCopy={handleCopy} onEditStart={handleEditStart} onEditCancel={handleEditCancel} onEditSave={handleEditSave} onDelete={actions.deleteMessage} onRegenerate={actions.regenerateMessage} sessionBusy={sessionBusy} isVoice={message.isVoice} isReplaying={replayMessageId === message.id} onReplay={() => actions.replayFromMessage(message.id)} bubbleRef={messageBubbleRef}/>
          </div>)}

        <div className={styles['message-meta']}>
          {message.acpBackendLabel && (<span className={styles['acp-backend-badge']}>{message.acpBackendLabel}</span>)}
