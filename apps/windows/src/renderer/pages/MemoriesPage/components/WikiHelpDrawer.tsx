...
const QUICK_SECTIONS = [
  ...
  {
    title: '从文件夹导入',
    body: '收件箱右上角「从文件夹导入」：选一个目录（如 outputs/ 下的任务文件夹），系统会预览可导入文件数量，确认后批量收进 Wiki。原文件保留在原处，Wiki 只建立引用。开启「⋯ 更多 → AI 自动分类」后会直接归档到目录，不再弹出映射确认页。',
  },
  {
    title: '让 Agent 帮忙整理',
  ...
  {
    title: '高级功能（⋯ 更多）',
    body: 'AI 自动分类（开关）、清理、重建索引、编辑主题树、全库重新编目（跑完直接改目录；收件箱请用「让 AI 分类」）。日常整理用不到时可忽略；鼠标悬停各菜单项可查看简要说明。',
  },
  {
    title: '文件存在哪',
