...
import { getProviderConfig, saveProviderConfig, listProviderModels, testProviderConnection, PROVIDER_DEFAULT_BASE_URL, PROVIDER_TYPE_DEFAULTS, PROVIDER_TYPE_LABEL, listProviderTypesForSlot, supportsApiFormatChoice, CAPABILITY_SLOT_LABEL, CAPABILITY_SLOT_DESC, CAPABILITY_SLOTS, createDefaultSlotsConfig, type LocalProviderConfigView, type ProviderSlotsConfigView, type ProviderType, type CapabilitySlot, type ListedModel, type ThinkingFormat, defaultContextWindowK, defaultSupportsReasoning, } from '../../../../services/model-config-service';
...
export function ModelConfigSection() {
    ...
    const providerDraftCacheRef = useRef<Partial<Record<string, {
        baseUrl: string;
        modelId: string;
    ...
    const handleSaveProvider = useCallback(async () => {
        ...
        try {
            ...
            if (saved.chat.enabled && saved.chat.modelId) {
                window.dispatchEvent(new CustomEvent('mtbot:chat-model-changed', { detail: { modelId: saved.chat.modelId } }));
            }
    ...
    const renderSlotCard = (slot: CapabilitySlot) => {
        ...
        const reasoningFor = (modelId: string) => modelReasoning[modelId] ?? defaultSupportsReasoning(modelId);
        const setModelReasoning = (modelId: string, value: boolean) => {
            patchSlot(slot, { modelReasoning: { ...modelReasoning, [modelId]: value } });
        };
        const thinkingCheckbox = (modelId: string) => (<Checkbox checked={reasoningFor(modelId)} aria-label={`${modelId} 支持思考`} onChange={(next) => setModelReasoning(modelId, next)}>
         ...
         */
        const imageModelOptions = slot === 'image' ? [...new Set([...models.map((m) => m.id), ...allowedIds])] : [];
        /**
         * 每个模型行右侧的附加配置。**生图模型没有这两个概念**，不渲染——
         * 给 image 槽挂上「上下文长度(K)」和「思考」只会让人以为它们有用。
         */
        const modelExtras = (modelId: string) => slot === 'image' ? null : (<>
          <Input type="number" min={1} step={1} value={String(contextWindowK[modelId] ?? defaultContextWindowK(modelId))} onChange={(e) => setContextWindowK(modelId, e.target.value)} style={{ width: 90 }} aria-label={`${modelId} 上下文长度（K）`}/>
          <span className={styles['setting-desc']}>K</span>
          {thinkingCheckbox(modelId)}
        </>);
        return (<Card key={slot}>
        <div className={styles['setting-item']}>
          ...
          <div className={styles['setting-control']} style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
            {slot !== 'chat' && (<Button variant="secondary" size="sm" onClick={() => copyFromChat(slot)}>
                从文本对话复制
              </Button>)}
            <div className={styles['slot-enable-group']} role="group" aria-label={`${CAPABILITY_SLOT_LABEL[slot]}启停`}>
              <button type="button" className={`${styles['slot-enable-btn']} ${cfg.enabled ? styles['slot-enable-btn--on'] : ''}`} onClick={() => {
                patchSlot(slot, { enabled: true });
                setExpandedSlots((s) => ({ ...s, [slot]: true }));
            }}>
                已启用
              </button>
              <button type="button" className={`${styles['slot-enable-btn']} ${!cfg.enabled ? styles['slot-enable-btn--off'] : ''}`} onClick={() => patchSlot(slot, { enabled: false })}>
                未启用
              </button>
            </div>
            <button type="button" className={styles['about-link']} onClick={() => setExpandedSlots((s) => ({ ...s, [slot]: !expanded }))}>
              {expanded ? '收起配置' : '展开配置'}
            </button>
          </div>
        </div>
        ...
        {expanded && (<>
            <div className={styles['setting-item']}>
              ...
              <div className={styles['setting-control']}>
                <Select value={cfg.type} options={listProviderTypesForSlot(slot).map((t) => ({
                    value: t,
                    label: PROVIDER_TYPE_LABEL[t],
                }))} onChange={(e) => {
                    const nextType = e.target.value as ProviderType;
                    const isUntouched = !cfg.baseUrl?.trim() ||
            ...
            <div className={styles['setting-item']}>
              ...
              <div className={styles['setting-control']}>
                <Input type="text" value={cfg.baseUrl} placeholder={PROVIDER_DEFAULT_BASE_URL[cfg.type]} onChange={(e) => patchSlot(slot, { baseUrl: e.target.value })} disabled={cfg.type === 'deepseek'}/>
              </div>
            </div>
            ...
            <div className={styles['setting-item']}>
              <div className={styles['setting-label']}>
                ...
                {isLocalProvider && (<span className={styles['setting-desc']}>本地 Provider 通常无需填写</span>)}
              </div>
              <div className={styles['setting-control']}>
                <Input type={showApiKeyBySlot[slot] ? 'text' : 'password'} value={cfg.apiKey} placeholder={isLocalProvider ? '（可留空）' : 'sk-...'} onChange={(e) => patchSlot(slot, { apiKey: e.target.value })} suffix={<button type="button" className={styles['about-link']} aria-label={showApiKeyBySlot[slot] ? '隐藏 API Key' : '显示 API Key'} onClick={() => setShowApiKeyBySlot((s) => ({ ...s, [slot]: !s[slot] }))} style={{ display: 'inline-flex', alignItems: 'center', padding: 0, border: 'none', background: 'transparent', cursor: 'pointer' }}>
              ...
              {/*
                ...
                */}
              {cfg.apiKeyDecryptFailed && !cfg.apiKey && (<div className={styles['field-msg']} role="alert" style={{
            ...
            {supportsApiFormatChoice(cfg.type) && (<div className={styles['setting-item']}>
                <div className={styles['setting-label']}>
                ...
                <div className={styles['setting-control']}>
                  <Select value={cfg.apiFormat ?? 'responses'} options={[
            ...
            {supportsApiFormatChoice(cfg.type) && (<div className={styles['setting-item']}>
                ...
                <div className={styles['setting-control']}>
                  <Select value={cfg.thinkingFormat ?? 'auto'} aria-label="思考参数格式" options={[
                    ...
                    ]} onChange={(e) => patchSlot(slot, { thinkingFormat: e.target.value as ThinkingFormat })}/>
                </div>
              </div>)}
            ...
            <div className={styles['setting-item']}>
              ...
              <div className={styles['setting-control']} style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {models.length > 0 ? (<div style={{ display: 'flex', flexDirection: 'column', gap: 6, maxHeight: 220, overflow: 'auto' }}>
                    {models.map((m) => {
                        ...
                        return (<div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          <Checkbox checked={checked} onChange={(next) => {
                                ...
                                const nextModelId = nextIds.includes(cfg.modelId) ? cfg.modelId : (nextIds[0] ?? '');
                ...
                */}
                {slot === 'image' && imageModelOptions.length > 0 && (<Select value={cfg.modelId} aria-label="默认生图模型" options={imageModelOptions.map((id) => ({ value: id, label: id }))} onChange={(e) => patchSlot(slot, {
                        modelId: e.target.value,
                        allowedModelIds: [...new Set([...allowedIds, e.target.value])],
                    })}/>)}
                <Input type="text" value={slotModelIdsText[slot] ?? (cfg.allowedModelIds ?? []).join(', ')} placeholder="模型 ID，多个用逗号分隔；可手动输入，勾选后自动填充" onChange={(e) => {
                    setSlotModelIdsText((t) => ({ ...t, [slot]: e.target.value }));
                }} onBlur={() => commitSlotModelIdsText(slot)} onKeyDown={(e) => {
                    if (e.key === 'Enter') {
                        e.preventDefault();
                        commitSlotModelIdsText(slot);
                    }
                }}/>
                {slot !== 'image' && models.length === 0 && allowedIds.map((id) => (<div key={id} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                {allowedIds.length > 0 && (<span className={styles['setting-desc']}>
            ...
            <div className={styles['category-save-actions']} style={{ gap: 8 }}>
              <Button variant="secondary" loading={!!slotListing[slot]} onClick={() => { void handleListModels(slot); }}>
                获取模型列表
              </Button>
              <Button variant="secondary" loading={!!slotTesting[slot]} onClick={() => { void handleTestSlot(slot); }}>
                测试连接
              </Button>
            </div>
          </>)}
    ...
    return (<div className={styles['settings-section']}>
      ...
      {CAPABILITY_SLOTS.map((slot) => (<div key={slot} style={{ marginBottom: 12 }}>
          {renderSlotCard(slot)}
        </div>))}
