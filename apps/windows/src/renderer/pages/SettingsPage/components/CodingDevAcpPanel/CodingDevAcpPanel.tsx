...
import { useCodingDevProjects, useCodingDevProjectModals, } from '../../../../hooks/business/useCodingDevProjects';
import { getCodingDevEnvInfo, detectCodingDevTool, listCodingDevToolsMetadata, installCodingDevTool, previewUninstallCodingDevTool, uninstallCodingDevTool, loginCodingDevTool, getCodingDevAgentBindings, setCodingDevAgentBindings, type CodingDevAgentBinding, } from '../../../../services/coding-dev-service';
...
export const CodingDevAcpPanel: React.FC = () => {
    ...
    const [bindableAgents, setBindableAgents] = useState<Array<{
        id: string;
        name: string;
    }>>([]);
    useEffect(() => {
        const load = async () => {
            try {
                ...
                setBindableAgents((agentRes.agents ?? [])
    ...
    useEffect(() => {
        const load = async () => {
            try {
                ...
                if (!api?.sendCommand)
                    return;
                const res = (await api.sendCommand({ type: 'codingDev:getBackend' })) as {
                    backendId?: string;
                };
                if (!aliveRef.current)
                    return;
    ...
    const detectOne = useCallback(async (toolId: string) => {
        ...
        try {
            ...
            if (!aliveRef.current)
                return;
            setTools((prev) => prev.map((t) => (t.id === toolId ? { ...t, ...status, detectState: 'done' } : t)));
        }
        catch {
            if (!aliveRef.current)
                return;
    ...
    const handleAiInstall = useCallback((t: LocalAcpToolStatusView) => {
        window.dispatchEvent(new CustomEvent('mtbot:chat-draft-request', {
    ...
    return (<Card>
      <div className={styles.panel}>
        ...
        <div className={styles.toolGrid}>
          {tools.map((t) => (<div key={t.id} className={styles.toolCard}>
              <div className={styles.toolCardHeader}>
                <ToolIcon id={t.id} label={t.label}/>
                <span className={styles.toolName}>{t.label}</span>
                {t.detectState === 'done' ? (<span className={t.installed ? styles.statusDotOk : styles.statusDotOff} title={t.installed ? '已安装' : '未安装'} aria-label={t.installed ? '已安装' : '未安装'}/>) : (<span className={styles.detectingTag} aria-live="polite">检测中…</span>)}
              </div>
              ...
              <div className={styles.toolCardBody}>
                {t.installed ? (<>
                    ...
                    {t.latestVersion && (<div className={styles.versionRow}>
                        <span className={styles.versionLabel}>最新版本</span>
                        <span>{t.latestVersion}</span>
                      </div>)}
                    {t.authStatus && (<div className={styles.versionRow}>
                    ...
                    {t.resolvedPath && (<code className={styles.path} title={t.resolvedPath}>{t.resolvedPath}</code>)}
              ...
              <div className={styles.toolCardFooter}>
                {!t.installed ? (<>
                    ...
                    <button type="button" className={styles.linkBtn} onClick={() => handleAiInstall(t)} title="新开会话，让 AI 查最新文档后安装并配置">
                      让 AI 安装
                    </button>
                    <Button size="sm" loading={installingId === t.id} disabled={t.detectState !== 'done' || (installingId != null && installingId !== t.id)} onClick={() => void handleInstall(t.id)}>
                      一键安装
                    </Button>
                  </>) : (<>
                    {t.latestVersion && t.latestVersion !== t.currentVersion ? (<Button size="sm" loading={installingId === t.id} disabled={(installingId != null && installingId !== t.id) || uninstallingId === t.id || loggingInId === t.id} onClick={() => void handleInstall(t.id)}>
                        升级到 {t.latestVersion}
                      </Button>) : t.selfUpdateCommand ? (<Button size="sm" variant="secondary" loading={installingId === t.id} disabled={(installingId != null && installingId !== t.id) || uninstallingId === t.id || loggingInId === t.id} onClick={() => void handleInstall(t.id)} title={`执行 ${t.selfUpdateCommand}`}>
                        检查更新
                      </Button>) : null}
                    {t.authStatus === 'required' && (<Button size="sm" variant="secondary" loading={loggingInId === t.id} disabled={installingId != null || uninstallingId != null || (loggingInId != null && loggingInId !== t.id)} onClick={() => void handleLogin(t.id)} title="打开浏览器完成 OAuth 授权">
                        {loggingInId === t.id ? '登录中…' : '登录'}
                      </Button>)}
                    <button type="button" className={styles.linkBtn} disabled={installingId != null || uninstallingId != null || loggingInId != null} onClick={() => void beginUninstall(t.id)}>
                      {uninstallingId === t.id ? '卸载中…' : '卸载'}
                    </button>
                  </>)}
              </div>
            </div>))}
          {tools.length === 0 && (<div className={styles.empty}>
              {detecting ? '正在加载工具列表…' : '点击「重新检测」扫描本机 CLI'}
            </div>)}
        </div>

        <div className={styles.sectionLabel}>ACP 工作目录</div>
        {info && (<div className={styles.cwdBox}>
            <div className={styles.cwdMeta}>
              <span className={styles.cwdLabel}>当前目录</span>
              {info.usesDedicatedWorkspace ? (<span className={styles.badgeOk}>活动项目</span>) : (<span className={styles.badgeOff}>主工作区</span>)}
            </div>
            <code className={styles.path}>{info.resolvedWorkspace}</code>
          </div>)}

        <div className={styles.projectList}>
          {projects.length === 0 && (<div className={styles.empty}>暂无项目。新建或打开后可设为 ACP 工作目录。</div>)}
          {projects.map((p) => {
            const isActive = p.name === activeProject;
            return (<div key={p.name} className={`${styles.projectRow} ${isActive ? styles.projectActive : ''}`}>
                ...
                <div className={styles.toolActions}>
                  {!isActive && (<button type="button" className={styles.linkBtn} onClick={() => void handleActivate(p.name)}>
                      设为活动
                    </button>)}
        ...
        <div className={styles.projectRow}>
          ...
          <div className={styles.toolActions}>
            <select value={defaultBackend} onChange={(e) => void handleDefaultBackendChange(e.target.value)} title="全局默认编码后端">
              {backendOptions.map((o) => (<option key={o.id} value={o.id}>
                  {o.label}
                </option>))}
            </select>
          </div>
        ...
        <div className={styles.projectList}>
          ...
          {bindableAgents.map((a) => {
            ...
            return (<div key={a.id} className={`${styles.projectRow} ${enabled ? styles.projectActive : ''}`}>
                <div className={styles.projectMain}>
                  <span className={styles.toolName}>{a.name}</span>
                  <label style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12 }}>
                    <input type="checkbox" checked={enabled} onChange={(e) => upsertBinding(a.id, { enabled: e.target.checked })}/>
                    启用
                  </label>
                </div>
                <div className={styles.toolActions}>
                  <select value={binding?.backendId ?? 'claude'} disabled={!enabled} onChange={(e) => upsertBinding(a.id, { backendId: e.target.value as CodingDevAgentBinding['backendId'] })} title="使用的编码 CLI">
                    {(['claude', 'codex', 'cursor', 'opencode'] as const).map((id) => (<option key={id} value={id}>
                        {id}
                      </option>))}
                  </select>
                  <select value={binding?.workspace ?? ''} disabled={!enabled} onChange={(e) => upsertBinding(a.id, { workspace: e.target.value })} title="绑定的项目目录（缺省跟随全局活动项目）">
                    <option value="">跟随全局活动项目</option>
                    {projects.map((p) => (<option key={p.name} value={p.realPath}>
                        {p.name}
                      </option>))}
                  </select>
                  {binding && (<button type="button" className={styles.dangerBtn} onClick={() => removeBinding(a.id)}>
                      移除
                    </button>)}
                </div>
              </div>);
        ...
        {info?.weixinSlashHint && (<div className={styles.hint}>{info.weixinSlashHint}</div>)}
        {modals}

        <ConfirmModal open={uninstallTarget != null} layer="elevated" title={`卸载 ${uninstallTarget?.label ?? ''}`} content={buildUninstallConfirmText(uninstallTarget)} confirmText={uninstallTarget?.automatic ? '确认卸载' : '我知道了'} confirmVariant="danger" onConfirm={() => {
            if (uninstallTarget?.automatic)
