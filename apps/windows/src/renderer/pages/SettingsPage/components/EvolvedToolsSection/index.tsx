...
export function EvolvedToolsSection() {
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            if (!listRes.ok)
                throw new Error(listRes.error || '加载失败');
    ...
    const run = useCallback(async (key: string, fn: () => Promise<{
        ok: boolean;
        error?: string;
    }>, successMsg: string) => {
    ...
    const handleConfirm = (name: string) => void run(`confirm-${name}`, () => sendCommand({ type: 'tool-evolution:confirm', toolName: name }), `已启用工具「${name}」`);
    const handleReject = (name: string) => void run(`reject-${name}`, () => sendCommand({ type: 'tool-evolution:reject', toolName: name }), `已丢弃候选「${name}」`);
    const handleToggle = (tool: EvolvedToolInfo) => void run(`toggle-${tool.name}`, () => sendCommand({ type: 'tool-evolution:set-enabled', toolName: tool.name, enabled: !tool.enabled }), tool.enabled ? `已禁用「${tool.name}」` : `已启用「${tool.name}」`);
    const handleRemove = (tool: EvolvedToolInfo) => {
        if (!window.confirm(`删除工具「${tool.name}」？此操作不可恢复。`))
            return;
    ...
    return (<div className={styles.wrap}>
      <Card className={settingsStyles.settingCard}>
        ...
        <div className={settingsStyles.settingCardContent}>
          ...
          {!loading && !error && (<>
              {/* 统计概览 */}
              {stats && (<div className={styles.statsOverview}>
                  ...
                  {/* 高频命令 Top 5 */}
                  {stats.highFrequencyCommands.length > 0 && (<div className={styles.topCommands}>
                      <h5 className={styles.topCommandsTitle}>
                        🔥 高频命令 Top {stats.highFrequencyCommands.length}（近一周）
                      </h5>
                      <div className={styles.commandList}>
                        {stats.highFrequencyCommands.map((cmd, idx) => (<div key={idx} className={styles.commandItem}>
              ...
              {pending.length === 0 && (<p className={styles.status}>暂无候选。工具进化引擎会在检测到高频命令后自动草拟。</p>)}
              {pending.map((p) => (<div key={p.name} className={styles.item}>
                  <div className={styles.itemMain}>
                    ...
                    {(p.whenToUse || p.whenNotToUse) && (<div className={styles.advice}>
                        {p.whenToUse && (<p className={styles.adviceLine}>
                            <span className={styles.fieldLabel}>建议使用</span>
                            {p.whenToUse}
                          </p>)}
                        {p.whenNotToUse && (<p className={styles.adviceLine}>
                            <span className={styles.fieldLabel}>不建议</span>
                            {p.whenNotToUse}
                          </p>)}
                      </div>)}
                    {p.similarApproved && p.similarApproved.length > 0 && (<p className={styles.dupWarn}>
                        疑似与已批准工具重复：{p.similarApproved.join('、')}。建议点「不用」。
                      </p>)}
                    {p.lowValueReason && (<p className={styles.dupWarn}>
                        低价值候选：{p.lowValueReason}。建议点「不用」。
                      </p>)}
                    <code className={styles.template}>{p.commandTemplate}</code>
                    {p.samples && p.samples.length > 0 && (<ul className={styles.sampleList}>
                        {p.samples.slice(0, 3).map((s) => (<li key={s}>
                            <code className={styles.sampleCmd}>{s}</code>
                          </li>))}
                      </ul>)}
                  </div>
                  <div className={styles.itemActions}>
                    <Button size="sm" variant="primary" disabled={isBusy(`confirm-${p.name}`)} onClick={() => handleConfirm(p.name)} title="注册为系统工具，Agent 可直接调用">
                      <Check size={14}/> 启用
                    </Button>
                    <Button size="sm" variant="ghost" disabled={isBusy(`reject-${p.name}`)} onClick={() => handleReject(p.name)} title="丢弃该候选">
                      <X size={14}/> 不用
                    </Button>
                  </div>
                </div>))}
              ...
              {tools.length === 0 && (<p className={styles.status}>还没有已批准的工具。审批一个候选后，它会出现在这里。</p>)}
              {tools.map((t) => (<div key={t.name} className={styles.item}>
                  <div className={styles.itemMain}>
                    <div className={styles.itemHead}>
                  ...
                  <div className={styles.itemActions}>
                    <Button size="sm" variant={t.enabled ? 'ghost' : 'primary'} disabled={isBusy(`toggle-${t.name}`)} onClick={() => handleToggle(t)}>
                      {t.enabled ? '禁用' : '启用'}
                    </Button>
                    <Button size="sm" variant="danger" disabled={isBusy(`remove-${t.name}`)} onClick={() => handleRemove(t)} title="删除工具（不可恢复）">
                      <Trash2 size={14}/>
                    </Button>
                  </div>
                </div>))}
