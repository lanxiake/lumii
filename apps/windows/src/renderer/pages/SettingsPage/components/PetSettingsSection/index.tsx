...
import { subscribePetModeChanged, listPetModels, getCurrentPetModelId, getPetMode, getPetPersonality, getVirtualHumanSettings, setVirtualHumanSettings, setCurrentPetModelId, switchPetMode, } from '../../../../services/pet-service';
import type { PetModelConfigDTO } from '../../../../../shared/pet-mode';
import { type VirtualHumanSettingsDTO, DEFAULT_VH_SETTINGS, } from '../../../../../shared/virtual-human';
...
export const PetSettingsSection: React.FC = () => {
    ...
    useEffect(() => {
        ...
        void getPetPersonality(modelId).then((p) => {
            if (!cancelled)
                setPetPersonality(p?.label ?? null);
        });
        return () => {
            cancelled = true;
        };
        ...
        <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div className={styles['setting-item']}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              {currentModel?.thumbnailUrl && (<img src={currentModel.thumbnailUrl} alt={currentModel.name} style={{ width: 48, height: 48, borderRadius: 8, objectFit: 'cover', background: 'rgba(0,0,0,0.1)' }}/>)}
              <Select value={vhCurrentModelId || currentModel?.id || ''} options={vhModels.map((m) => ({
            value: m.id,
            ...
            {currentModel && capabilityHint(expressionCapability(currentModel.emotionMap)) && (<span className={styles['setting-hint']}>
                {capabilityHint(expressionCapability(currentModel.emotionMap))}
              </span>)}
          </div>

          {/* 气质标签（宠物智能化第一期）：宠物是独立 Agent，它的脾气与助手互不影响 */}
          {petPersonality && (<div className={styles['setting-item']}>
          ...
          {/* 跟随模型默认 Agent */}
          <div className={styles['setting-item']}>
            <Checkbox checked={vhSettings.followModelAgent} onChange={(checked) => void patchVh({ followModelAgent: checked })}>
              跟随模型默认 Agent
            </Checkbox>
          ...
          <div className={styles['setting-item']}>
            ...
            <Select value={vhSettings.agentId} options={[
          ...
          {/* 表情标签开关 */}
          <div className={styles['setting-item']}>
            <Checkbox checked={vhSettings.enableExpressionPrompt} onChange={(checked) => void patchVh({ enableExpressionPrompt: checked })}>
              启用表情标签
            </Checkbox>
          ...
          {/* 动作描写开关 */}
          <div className={styles['setting-item']}>
            <Checkbox checked={vhSettings.enableThinkTagPrompt} onChange={(checked) => void patchVh({ enableThinkTagPrompt: checked })}>
              启用动作/神态描写
            </Checkbox>
          ...
          {/* 声音开关：文字回复是否朗读 */}
          <div className={styles['setting-item']}>
            <Checkbox checked={vhSettings.enableVoiceReply} onChange={(checked) => void patchVh({ enableVoiceReply: checked })}>
              文字回复朗读
            </Checkbox>
          ...
          {/* 待机随机动作 */}
          <div className={styles['setting-item']}>
            <Checkbox checked={vhSettings.enableIdleMotion} onChange={(checked) => void patchVh({ enableIdleMotion: checked })}>
              待机随机动作
            </Checkbox>
          ...
          {/* Agent 活动感知（R5/R6） */}
          <div className={styles['setting-item']}>
            <Checkbox checked={vhSettings.enableAgentActivity} onChange={(checked) => void patchVh({ enableAgentActivity: checked })}>
              Agent 状态感知
            </Checkbox>
            <span className={styles['setting-hint']}>
          ...
          {/* Agent 通知（R6「叫得动」） */}
          <div className={styles['setting-item']}>
            <Checkbox checked={vhSettings.enableAgentNotice} onChange={(checked) => void patchVh({ enableAgentNotice: checked })}>
              Agent 通知
            </Checkbox>
            <span className={styles['setting-hint']}>
          ...
          {/* 鼠标点击控制 */}
          <div className={styles['setting-item']}>
            <Checkbox checked={vhSettings.enableTapInteraction} onChange={(checked) => void patchVh({ enableTapInteraction: checked })}>
              鼠标点击控制
            </Checkbox>
          ...
          {/* 闲置感知 */}
          <div className={styles['setting-item']}>
            <Checkbox checked={vhSettings.enableIdleAwareness} onChange={(checked) => void patchVh({ enableIdleAwareness: checked })}>
              闲置感知
            </Checkbox>
            <span className={styles['setting-hint']}>
          ...
          <div className={styles['setting-item']}>
            <Checkbox checked={vhSettings.enablePetTask} onChange={(checked) => void patchVh({ enablePetTask: checked })}>
              允许宠物主动做事
            </Checkbox>
            <span className={styles['setting-hint']}>
              ...
              已经在路上的那一件会跑完。
              <br />
              ※ 这个开关还管它「自己找事做」那部分：每天至多自己反思一次、自己排一两件小事。
              不想要就关掉——反思、排期、写日记会一起停。
              <br />
              ※ 注意：宠物的行为由本页这几个开关控制；设置里的「自主进化」总开关管的是那几个
              系统 Agent（写简报 / 抓资讯 / 清工作区），不影响它
            </span>
          </div>

          {/* 强制穿透默认值 */}
          <div className={styles['setting-item']}>
            <Checkbox checked={vhSettings.forceIgnoreMouse} onChange={(checked) => void patchVh({ forceIgnoreMouse: checked })}>
              默认开启强制穿透
            </Checkbox>
          ...
          {/* 主动联系 */}
          <div className={styles['setting-item']}>
            <Checkbox checked={vhSettings.proactiveCareEnabled} onChange={(checked) => void patchVh({ proactiveCareEnabled: checked })}>
              开启主动联系
            </Checkbox>
            <span className={styles['setting-hint']}>
          ...
          {vhSettings.proactiveCareEnabled && (<>
              <div className={styles['setting-item']}>
                ...
                <Select value={vhSettings.proactiveCareMode} options={[
              ...
              <div className={styles['setting-item']}>
                ...
                <Input value={vhSettings.proactiveCareNickname} onChange={(e) => void patchVh({ proactiveCareNickname: e.target.value })} placeholder="比如：小明、老王" maxLength={30}/>
              </div>
            </>)}

          <div>
            <Button variant="primary" disabled={petModeBlocked} title={petModeBlocked ? (blockMessage('petMode') ?? undefined) : undefined} onClick={async () => {
            ...
            {petModeBlocked && (<p className={styles['setting-hint']}>{blockMessage('petMode')}</p>)}
          </div>
        </div>
      </Card>
    </div>);
