...
import { fetchCloudSyncConfig, fetchCloudSyncStatus, fetchCloudSyncLogs, fetchPendingMassDelete, fetchLargeQueueStats, saveCloudSyncConfig, testCloudSyncConnection, syncCloudSyncNow, retryCloudSyncConflict, confirmCloudSyncMassDelete, subscribeCloudSyncStatus, } from '../../../../services/cloud-sync-service';
...
export function CloudSyncSection() {
    ...
    return (<div className={styles['settings-section']}>
      ...
      <div className={styles['cloud-guide']}>
        <button type="button" className={styles['cloud-guide-toggle']} onClick={() => setGuideOpen((v) => !v)}>
          <span>新手引导 · 准备 GitCode 仓库（只需一次）</span>
          <span className={styles['cloud-guide-toggle-hint']}>{guideOpen ? '收起' : '展开'}</span>
        </button>
        {guideOpen && (<div className={styles['cloud-guide-body']}>
            ...
            <div className={styles['cloud-guide-actions']}>
              <Button variant="secondary" onClick={() => void openGitCode()}>
                打开 GitCode
              </Button>
            </div>
      ...
      <div className={styles['setting-group']}>
        ...
        <div className={styles['setting-item']}>
          ...
          <Select value={form.provider} onChange={(e) => setForm({ ...form, provider: e.target.value as CloudSyncConfigView['provider'] })} options={[
        ...
        <div className={styles['setting-item']}>
          ...
          <Input value={form.repoUrl} onChange={(e) => setForm({ ...form, repoUrl: e.target.value })} placeholder="https://gitcode.com/alice/notes.git"/>
        </div>
        ...
        <div className={styles['setting-item']}>
          ...
          <Input type="password" value={token} onChange={(e) => setToken(e.target.value)} placeholder={form.tokenMasked ? '留空沿用已保存令牌' : '粘贴访问令牌'}/>
        </div>
        ...
        <div className={styles['setting-row']}>
          ...
          <div className={styles['setting-item']}>
            ...
            <Input type="number" value={form.intervalMinutes} onChange={(e) => setForm({ ...form, intervalMinutes: Number(e.target.value) || 15 })}/>
          </div>
        </div>
      </div>
      ...
      {status && (<div className={styles['setting-item']}>
          ...
          <div className={styles['setting-hint']}>
            ...
            {status.queuedBehind ? (<>当前状态：排队中 · {status.message}</>) : (<>
                当前状态：{STATE_LABEL[status.state] ?? status.state}
                {status.message ? ` · ${status.message}` : ''}
              </>)}
            {status.lastSyncAt ? ` · 最近同步：${new Date(status.lastSyncAt).toLocaleString()}` : ''}
          </div>
          {status.state === 'error' && status.lastError && (<div className={styles['setting-hint']}>错误：{status.lastError}</div>)}
          {status.state === 'conflict' && status.conflict && (<div className={styles['setting-hint']}>
              冲突文件（{status.conflict.files.length}）：{status.conflict.files.join('、')}
            </div>)}
          {largeStats && largeStats.pendingFiles > 0 && (<div className={styles['setting-hint']}>
              大文件待传：{largeStats.pendingFiles} 个 ·{' '}
              {(largeStats.pendingBytes / 1048576).toFixed(1)} MB
              {largeStats.pumping ? ' · 传输中…' : ' · 等待下一轮同步'}
            </div>)}
          {status.state === 'conflict' && (<div style={{ marginTop: 8 }}>
      ...
      {pendingDelete && (<div className={styles['setting-item']}>
          ...
          <div style={{ marginTop: 8 }}>
            <Button variant="secondary" onClick={() => void confirmMassDelete(pendingDelete.fingerprint)} loading={confirmingDelete}>
      ...
      <div className={styles['setting-item']}>
        ...
        {logs.length === 0 ? (<div className={styles['setting-hint']}>暂无同步记录</div>) : (<div className={styles['cloud-log-list']}>
            {logs
                .slice()
                .reverse()
                .map((log, i) => (<div key={`${log.ts}-${i}`} className={styles['cloud-log-item']}>
                  <span className={styles['cloud-log-time']}>
                    {new Date(log.ts).toLocaleString()}
                  </span>
                  <span className={`${styles['cloud-log-state']} ${styles[`cloud-log-state-${log.state}`]}`}>
