...
import { EDGE_TTS_VOICES, QWEN3_CUSTOM_SPEAKERS, QWEN3_TTS_LANGUAGES, } from '../../../../../shared/voice-events';
...
export function VoiceSettingsSection() {
    ...
    useEffect(() => {
        ...
        if (!electronAPI?.voice?.sendCommand)
            return;
    ...
    useEffect(() => {
        ...
        const unsubscribe = electronAPI.voice.onEvent((event: any) => {
            ...
            if (event.type === 'voice:tts:preview:ended') {
                if (previewIdRef.current && event.previewId !== previewIdRef.current)
                    return;
            ...
            try {
                ...
                if (event.sampleRate === -1) {
                    ...
                    ctx.decodeAudioData(buf).then((decoded) => {
                        ...
                        if (event.isFinal)
                            source.addEventListener('ended', () => setVoicePreviewing(false));
    ...
    const saveVoiceConfig = async (partial: {
        ...
        const electronAPI = (window as any).electronAPI;
        if (!electronAPI?.voice?.sendCommand || !voiceConfig)
            return;
        const next = {
            ...voiceConfig,
            ...
            autoMuteMicWhileSpeaking: partial.autoMuteMicWhileSpeaking ?? voiceConfig.autoMuteMicWhileSpeaking,
        };
        setVoiceConfig(next);
        setVoiceSaving(true);
        try {
    ...
    const previewDisabled = !voiceConfig ||
        voicePreviewing ||
    ...
    const renderRuntimeStatus = () => {
        ...
        {voiceRuntimeStatus.detail ? (<div className={styles['voice-runtime-detail']}>{voiceRuntimeStatus.detail}</div>) : null}
      </div>);
    };
    const previewButtonLabel = (() => {
        if (!voicePreviewing && !runtimeBusy)
            return '▶ 预览合成声音';
        const phase = voiceRuntimeStatus?.phase;
        if (phase === 'installing_deps')
            return '安装依赖中…';
        if (phase === 'loading_model')
            return '加载模型中…';
        if (phase === 'checking_python' || phase === 'starting_engine')
            return '启动引擎中…';
    ...
    return (<div className={styles['settings-section']}>
      ...
      {!voiceConfig ? (<p className={styles['settings-note']}>加载语音配置中...</p>) : (<>
          {/* ═══ 1. 语音识别 ═══ */}
          <div className={styles['voice-block']}>
            ...
            <VoiceModelsPanel groups={['asr-core']} title="下载" hint="建议两项都下载。仅用文字输入可不下。"/>
            ...
            <div className={styles['setting-group']}>
              ...
              <div className={styles['setting-item']}>
                ...
                <Select value={voiceConfig.asr.provider} options={[
              ...
              {voiceConfig.asr.provider === 'openai-whisper' && (<div className={styles['setting-item']}>
                  <label className={styles['setting-label']} data-app-ui-label>OpenAI API Key</label>
                  <Input type={showVoiceApiKey ? 'text' : 'password'} placeholder="sk-..." value={voiceConfig.asr.apiKey ?? ''} onChange={(e) => saveVoiceConfig({ asr: { apiKey: e.target.value } })} style={{ width: '280px' }} suffix={<button type="button" aria-label={showVoiceApiKey ? '隐藏 API Key' : '显示 API Key'} onClick={() => setShowVoiceApiKey((v) => !v)} style={{ display: 'inline-flex', alignItems: 'center', padding: 0, border: 'none', background: 'transparent', cursor: 'pointer' }}>
                        {showVoiceApiKey ? <EyeOff size={16}/> : <Eye size={16}/>}
                      </button>}/>
                </div>)}
              <div className={styles['setting-item']}>
                <Checkbox checked={voiceConfig.autoMuteMicWhileSpeaking ?? true} onChange={(checked) => saveVoiceConfig({ autoMuteMicWhileSpeaking: checked })}>
                  AI 朗读时自动闭麦
                </Checkbox>
              </div>
              <div className={styles['setting-item']}>
                ...
                <input type="range" min={0.1} max={0.9} step={0.05} value={voiceConfig.vad?.threshold ?? 0.5} onChange={(e) => saveVoiceConfig({ vad: { threshold: parseFloat(e.target.value) } })} style={{ width: '200px' }}/>
              </div>
              <div className={styles['setting-item']}>
                <label className={styles['setting-label']} data-app-ui-label>
                  负面语音阈值：{(voiceConfig.vad?.energyGateMultiplier ?? 1.5).toFixed(1)}x
                </label>
                <input type="range" min={1.0} max={4.0} step={0.1} value={voiceConfig.vad?.energyGateMultiplier ?? 1.5} onChange={(e) => saveVoiceConfig({ vad: { energyGateMultiplier: parseFloat(e.target.value) } })} style={{ width: '200px' }}/>
              </div>
            </div>
          ...
          <div className={styles['voice-block']}>
            ...
            {!localTtsBlocked && (<VoiceModelsPanel groups={['tts-synth', 'tts-clone']} title="下载" hint="内置音色：先下 Tokenizer 12Hz，再下 0.6B CustomVoice（9 种音色）。声音克隆额外需要 0.6B Base（或 1.7B）。权重下完后台预装依赖，进度见下方「测试」状态条。"/>)}
            {/* D4：屏蔽必须给出原因，不能只是把面板藏掉 */}
            {localTtsBlocked && (<p className={styles['voice-block-desc']}>{blockMessage('localTts')}</p>)}

            <div className={styles['setting-group']}>
              <h5 className={styles['voice-block-subtitle']}>设置</h5>
              <div className={styles['setting-item']}>
                <label className={styles['setting-label']} data-app-ui-label>合成引擎</label>
                <Select value={voiceConfig.tts.provider} options={[
                ...
                if (v === 'local-vits' && !vitsDownloaded)
                    return;
                if (v === 'qwen3' && !qwen3CustomReady)
                    return;
                if (v === 'qwen3') {
              ...
              {voiceConfig.tts.provider === 'qwen3' && (<div className={styles['setting-item']}>
                  <label className={styles['setting-label']} data-app-ui-label>推理设备</label>
                  <Select value={voiceConfig.tts.qwen3Device ?? 'auto'} options={[
                    {
              ...
              <div className={styles['setting-item']}>
                ...
                <input type="range" min={0.8} max={1.5} step={0.1} value={voiceConfig.tts.speed} onChange={(e) => saveVoiceConfig({ tts: { speed: parseFloat(e.target.value) } })} style={{ width: '200px' }}/>
              </div>
              <div className={styles['setting-item']}>
                <label className={styles['setting-label']} data-app-ui-label>
                ...
                <input type="range" min={0} max={2} step={0.05} value={voiceConfig.tts.volume ?? 1.0} onChange={(e) => {
              ...
              {voiceConfig.tts.provider === 'edge' && (<div className={styles['setting-item']}>
                  <label className={styles['setting-label']} data-app-ui-label>Edge 音色</label>
                  <Select value={voiceConfig.tts.voice ?? 'zh-CN-XiaoxiaoNeural'} options={EDGE_TTS_VOICES.map((v) => ({
              ...
              {voiceConfig.tts.provider === 'qwen3' && (<>
                  <div className={styles['setting-item']}>
                    ...
                    <Select value={voiceConfig.tts.language ?? 'Auto'} options={QWEN3_TTS_LANGUAGES.map((l) => ({ label: l.name, value: l.id }))} onChange={(e) => saveVoiceConfig({ tts: { language: e.target.value } })}/>
                  </div>
                  ...
                  <div className={styles['setting-item']} style={{ flexDirection: 'column', alignItems: 'stretch' }}>
                    ...
                    return (<label key={sp.id} className={styles['voice-speaker-item']} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                            <input type="radio" name="qwen3-active-voice" checked={active} onChange={() => selectBuiltinSpeaker(sp.id)}/>
                            <span>
                              内置 · {sp.name}
                  ...
                  {qwenVariant === '1.7b-custom' && !cloneEnabled && (<div className={styles['setting-item']}>
                      <label className={styles['setting-label']} data-app-ui-label>风格指令（可选）</label>
                      <Input placeholder="例如：用特别开心的语气说" value={voiceConfig.tts.qwen3Instruct ?? ''} onChange={(e) => saveVoiceConfig({ tts: { qwen3Instruct: e.target.value } })} style={{ width: '320px' }}/>
                    </div>)}

                  <VoiceProfilesPanel selectedProfileId={cloneEnabled ? voiceConfig.tts.qwen3ProfileId : undefined} onSelectProfile={selectCloneProfile} onPreviewProfile={(id) => void handlePreviewProfile(id)} previewing={voicePreviewing || runtimeBusy} cloneReady={qwen3CloneReady && !voiceCloningBlocked}/>
                  {/* D4：克隆面板里也给出原因，否则用户只看到录制按钮不可用 */}
                  {voiceCloningBlocked && (<p className={styles['setting-hint']}>{blockMessage('voiceCloning')}</p>)}

                  {qwen3CloneReady && (<div className={styles['setting-item']} style={{ marginTop: 12 }}>
                      <label className={styles['setting-label']} data-app-ui-label>克隆模型规格</label>
                      <Select value={cloneVariant} options={[
                        {
            ...
            <div className={styles['setting-group']}>
              ...
              <div className={styles['setting-item']}>
                ...
                <input className={styles['voice-preview-input']} type="text" value={voicePreviewText} maxLength={100} onChange={(e) => setVoicePreviewText(e.target.value.slice(0, 100))} placeholder="你好，我叫 Lumii。I’m your best partner，是你的最佳伙伴呀。"/>
              </div>
              <Button variant="secondary" size="sm" onClick={handlePreview} disabled={previewDisabled || runtimeBusy}>
                {previewButtonLabel}
              </Button>
