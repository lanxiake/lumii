...
export function WorkspaceSection({ settings, defaultWorkspaceDir, updateWorkspace, save, }: WorkspaceSectionProps) {
    const toast = useToast();
    const handleSelectWorkspaceDir = async () => {
        try {
            const selectedDir = await selectWorkspaceDir(settings.workspace.directory || defaultWorkspaceDir);
            if (selectedDir) {
    ...
    return (<div className={styles['settings-section']}>
      ...
      <div className={styles['setting-group']}>
        <div className={styles['setting-item']}>
          ...
          <div className={styles['setting-row']}>
            <Input value={settings.workspace.directory || defaultWorkspaceDir} readOnly placeholder="未设置（使用默认路径）"/>
        ...
        {settings.workspace.directory && (<div className={styles['setting-item']}>
            <Button variant="secondary" onClick={handleResetWorkspaceDir}>
              恢复默认
            </Button>
          </div>)}
      </div>
      ...
      <div className={styles['category-save-actions']}>
        <Button onClick={save.save} loading={save.isSaving} disabled={save.isSaving || !save.hasChanges}>
