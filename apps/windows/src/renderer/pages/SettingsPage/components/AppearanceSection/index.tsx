...
export function AppearanceSection() {
    ...
    return (<div className={settingsStyles['settings-section']}>
      <div className={settingsStyles['setting-row']}>
        ...
        <div className={styles['theme-switch']} role="radiogroup" aria-label="界面主题">
          {THEME_OPTIONS.map((option) => (<button key={option.value} type="button" role="radio" aria-checked={theme === option.value} className={theme === option.value ? styles['theme-switch-active'] : undefined} onClick={() => setTheme(option.value)} title={option.hint}>
              <span className={styles.swatch} aria-hidden>
                {option.swatch.map((color) => (<i key={color} style={{ background: color }}/>))}
              </span>
              {option.label}
            </button>))}
        </div>
