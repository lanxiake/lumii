...
export function SkillMissingBadge({ agentId, missing, popoverAlign = 'left', onInstallSkill, onNavigateToStore, }: Props) {
    ...
    const handleDownload = async (skill: MissingSkill) => {
        ...
        if (!ok)
            setErrors((prev) => new Set(prev).add(skill.id));
    };
    ...
    return (<div ref={wrapperRef} className={styles.wrapper}>
      <button className={styles.badge} onClick={(e) => { e.stopPropagation(); setOpen((v) => !v); }} type="button">
        <span className={styles['badge-dot']}/>
        缺 {missing.length} 个技能
      </button>

      {open && (<div className={`${styles.popover} ${styles[`popover--${popoverAlign}`]}`} onClick={(e) => e.stopPropagation()}>
          <div className={styles['popover-header']}>
            ...
            <button className={styles['popover-close']} onClick={() => setOpen(false)} type="button">
              ✕
            </button>
          </div>

          <div className={styles['skill-list']}>
            {missing.map((skill) => (<div key={skill.id} className={styles['skill-row']}>
                ...
                <div className={styles['skill-action']}>
                  {skill.inStore ? (errors.has(skill.id) ? (<span className={styles['status-error']}>✕ 失败</span>) : (<button className={styles['btn-download']} disabled={downloading.has(skill.id)} onClick={() => { void handleDownload(skill); }} type="button">
                        {downloading.has(skill.id) ? '下载中…' : '⬇ 下载'}
                      </button>)) : (<button className={styles['btn-search-store']} onClick={handleGotoStore} type="button" title="前往技能商店搜索">
                      🔍 去查找
                    </button>)}
                </div>
              </div>))}
          ...
          <div className={styles['popover-footer']}>
            <button className={styles['btn-goto-store']} onClick={handleGotoStore} type="button">
              🏪 打开技能商店
            </button>
          </div>
        </div>)}
