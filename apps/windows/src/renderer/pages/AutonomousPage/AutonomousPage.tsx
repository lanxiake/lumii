...
import { TIP_BREAKDOWN, TIP_CAPABILITY_DIMENSIONS, TIP_CAPABILITY_RADAR, TIP_ENABLED, TIP_GOAL, TIP_PENDING_GOALS, TIP_PROMPT_STATS, TIP_REFLECTION, TIP_REFLECTION_LIST, TIP_SATISFACTION_CHART, TIP_SATISFACTION_OVERALL, TIP_SATISFACTION_TREND, TIP_SETTINGS, TIP_INNER_MOOD, TIP_INNER_CONCERNS, TIP_INNER_DIARY, } from './autonomousTooltips';
 ...
 */
export function AutonomousPage({ embedded = false }: {
    embedded?: boolean;
} = {}) {
    ...
    const [diaryCursor, setDiaryCursor] = useState<{
        timestamp: number;
        id: string;
    } | null>(null);
    ...
    async function loadData() {
        try {
            ...
            setSelectedReflectionId((prev) => {
                if (prev && reflectionsData.some((r: Reflection) => r.id === prev))
                    return prev;
    ...
    async function loadMoreDiary() {
        if (!diaryCursor || diaryLoadingMore)
            return;
    ...
    const selectedReflection = useMemo(() => reflections.find((r) => r.id === selectedReflectionId) ?? null, [reflections, selectedReflectionId]);
    if (loading) {
    ...
    return (<div className={`${styles.page} ${embedded ? styles.pageEmbedded : ''}`}>
      {/* 页头：标题 + 启用开关同行 */}
      <div className={styles.header}>
        ...
        <div className={styles.headerActions}>
          ...
          <label className="toggle-switch" aria-label="启用或禁用自主进化">
            <input type="checkbox" checked={autonomousEnabled} onChange={(e) => handleToggleAutonomous(e.target.checked)}/>
      ...
      <div className={styles.tabs}>
        ...
        <button className={`${styles.tab} ${activeTab === 'capabilities' ? styles.active : ''}`} onClick={() => setActiveTab('capabilities')}>
          <svg className={styles.tabIcon} width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor">
        ...
        <button className={`${styles.tab} ${activeTab === 'planned' ? styles.active : ''}`} onClick={() => setActiveTab('planned')}>
          <svg className={styles.tabIcon} width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor">
        ...
        <button className={`${styles.tab} ${activeTab === 'reflections' ? styles.active : ''}`} onClick={() => setActiveTab('reflections')}>
          <svg className={styles.tabIcon} width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor">
            <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
          反思
        </button>
        <button className={`${styles.tab} ${activeTab === 'prompt' ? styles.active : ''}`} onClick={() => setActiveTab('prompt')}>
          <svg className={styles.tabIcon} width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor">
        ...
        <button className={`${styles.tab} ${activeTab === 'inner' ? styles.active : ''}`} onClick={() => setActiveTab('inner')}>
          <svg className={styles.tabIcon} width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor">
      ...
      <div className={styles.content}>
        {activeTab === 'overview' && (<div className={styles.overviewGrid}>
            <Card header={<TitledHeader title="满意度评分" tip={TIP_SATISFACTION_OVERALL}/>} className={styles.overviewScore} bodyClassName={styles.cardBodyFill}>
            ...
            ] as const).map(([label, value]) => (<div key={label} className={styles.breakdownItem}>
                    <LabeledMetric label={label} tip={TIP_BREAKDOWN[label]} className={styles.breakdownLabel}/>
            ...
            <Card header={<TitledHeader title="满意度趋势 · 7 天" tip={TIP_SATISFACTION_CHART}/>} className={styles.overviewTrend} bodyClassName={styles.cardBodyFill}>
              <div className={styles.chartFill}>
                <SatisfactionChart history={satisfactionHistory} window="7d" fillHeight/>
              </div>
            </Card>

            <Card header={<TitledHeader title={`最近目标 (${goals.length})`} tip={TIP_PENDING_GOALS}/>} className={styles.overviewGoals} bodyClassName={styles.cardBodyFill}>
              {goals.length === 0 ? (<div className={styles.emptyCompact}>
                  ...
                  暂无目标
                </div>) : (<div className={styles.goalsList}>
                  {goals.map((goal) => (<GoalCard key={goal.id} goal={goal} onApprove={handleApprove} onReject={handleReject} onJump={handleGoalJump}/>))}
                </div>)}
            </Card>
          </div>)}

        {activeTab === 'capabilities' && (Object.keys(capabilities).length > 0 ? (<div className={styles.capabilitiesGrid}>
              <Card header={<TitledHeader title="能力雷达" tip={TIP_CAPABILITY_RADAR}/>} className={styles.radarCol} bodyClassName={styles.cardBodyFill}>
                <CapabilityRadar capabilities={capabilities} size={280}/>
              </Card>
              <Card header={<TitledHeader title="进化维度" tip={TIP_CAPABILITY_DIMENSIONS}/>} className={styles.dimensionsCol} bodyClassName={styles.cardBodyFill}>
                <div className={styles.capabilitiesList}>
                  {Object.entries(capabilities).map(([dimension, state]: [
                string,
                any
            ]) => {
                const expanded = expandedDimension === dimension;
                const tests = capabilityTests.filter((t) => t.dimension === dimension);
                return (<div key={dimension} className={styles.dimensionItem}>
                        <div className={styles.dimensionHeader} onClick={() => setExpandedDimension(expanded ? null : dimension)}>
                          <CapabilityProgressBar dimension={dimension} level={state.level} confidence={state.confidence} testCount={state.testCount} trend={state.trend || 'stable'}/>
                        </div>
                        {expanded && (<div className={styles.capabilityTests}>
                            {tests.length === 0 ? (<div className={styles.capabilityTestsEmpty}>暂无测试记录</div>) : (tests.map((test) => <CapabilityTestRow key={test.id} test={test}/>))}
                          </div>)}
                      </div>);
            })}
                </div>
        ...
        {activeTab === 'planned' && (<Card header={<div className={styles.plannedHeader}>
                ...
                <Tooltip content="清除今日规划并让 Agent 重新排期未来 24 小时" placement="bottom">
                  <button className={styles.btnSecondary} disabled={replanning} onClick={handleReplan}>
                    {replanning ? '规划中…' : '重置规划'}
                  </button>
                </Tooltip>
              </div>} bodyClassName={styles.cardBodyFill}>
            {plannedGoals.length === 0 ? (<div className={styles.empty}>
                <div className={styles.emptyIcon}>
                  <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                    <rect x="3" y="4" width="18" height="18" rx="2" strokeWidth="2"/>
                    <path d="M16 2v4M8 2v4M3 10h18" strokeWidth="2" strokeLinecap="round"/>
              ...
              </div>) : (<div className={styles.plannedList}>
                {plannedGoals.map((goal) => (<PlannedGoalCard key={goal.id} goal={goal} onDelete={handleDeletePlannedGoal}/>))}
              </div>)}
          </Card>)}

        {activeTab === 'reflections' && (reflections.length > 0 ? (<div className={styles.reflectionsGrid}>
              <Card header={<TitledHeader title="最近反思" tip={TIP_REFLECTION_LIST}/>} className={styles.reflectionListCol} bodyClassName={styles.cardBodyFill}>
                <div className={styles.reflectionNav}>
                  {reflections.map((reflection) => (<button key={reflection.id} type="button" className={`${styles.reflectionNavItem} ${selectedReflectionId === reflection.id ? styles.reflectionNavActive : ''}`} onClick={() => setSelectedReflectionId(reflection.id)}>
              ...
              <Card header={<TitledHeader title="详情" tip={TIP_REFLECTION.detail}/>} className={styles.reflectionDetailCol} bodyClassName={styles.cardBodyFill}>
                {selectedReflection ? (<ReflectionCard reflection={selectedReflection} variant="detail"/>) : (<div className={styles.emptyCompact}>请选择一条反思</div>)}
              </Card>
            </div>) : (<Card title="反思记录">
              <div className={styles.empty}>
                <div className={styles.emptyIcon}>
        ...
        {activeTab === 'prompt' && (<Card header={<TitledHeader title="Prompt 变体统计" tip={TIP_PROMPT_STATS}/>}>
            {promptStats.length > 0 ? (<PromptVariantStats stats={promptStats}/>) : (<div className={styles.empty}>
                <div className={styles.emptyIcon}>
                  <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                    <circle cx="12" cy="12" r="10" strokeWidth="2"/>
        ...
        {activeTab === 'settings' && (<Card header={<TitledHeader title="自主进化参数" tip={TIP_SETTINGS}/>}>
            {settings ? (<div className={styles.settingsForm}>
                <div className={styles.settingRow}>
                  <label className={styles.settingLabel} htmlFor="tickInterval">
                    心跳频率（分钟）
                  </label>
                  <input id="tickInterval" type="number" className={styles.settingInput} min={5} max={60} value={settings.tickIntervalMinutes} onChange={(e) => {
                    const n = Number(e.target.value);
                    if (Number.isFinite(n))
                        patchSettings({ tickIntervalMinutes: n });
                }}/>
                ...
                <div className={styles.settingRow}>
                  ...
                  <div className={styles.settingPair}>
                    <input type="number" className={styles.settingInput} min={0} max={23} value={settings.quietHours[0]} onChange={(e) => {
                    const n = Number(e.target.value);
                    if (Number.isFinite(n))
                        patchSettings({ quietHours: [n, settings.quietHours[1]] });
                }}/>
                    <span className={styles.settingDash}>–</span>
                    <input type="number" className={styles.settingInput} min={0} max={23} value={settings.quietHours[1]} onChange={(e) => {
                    const n = Number(e.target.value);
                    if (Number.isFinite(n))
                        patchSettings({ quietHours: [settings.quietHours[0], n] });
                }}/>
                  </div>
                ...
                <div className={styles.settingRow}>
                  ...
                  <input id="maxOutreach" type="number" className={styles.settingInput} min={0} max={50} value={settings.maxOutreachPerDay} onChange={(e) => {
                    const n = Number(e.target.value);
                    if (Number.isFinite(n))
                        patchSettings({ maxOutreachPerDay: n });
                }}/>
                ...
                <div className={styles.settingRow}>
                  ...
                  <input id="minInterval" type="number" className={styles.settingInput} min={0} max={1440} value={settings.minOutreachIntervalMinutes} onChange={(e) => {
                    const n = Number(e.target.value);
                    if (Number.isFinite(n))
                        patchSettings({ minOutreachIntervalMinutes: n });
                }}/>
                ...
                <div className={styles.settingRow}>
                  ...
                  <input id="maxGoals" type="number" className={styles.settingInput} min={1} max={20} value={settings.maxGoalsPerDay} onChange={(e) => {
                    const n = Number(e.target.value);
                    if (Number.isFinite(n))
                        patchSettings({ maxGoalsPerDay: n });
                }}/>
                ...
                <div className={styles.settingRow}>
                  ...
                  <input id="maxTokens" type="number" className={styles.settingInput} min={0} max={1000000} step={1000} value={settings.maxTokensPerDay} onChange={(e) => {
                    const n = Number(e.target.value);
                    if (Number.isFinite(n))
                        patchSettings({ maxTokensPerDay: n });
                }}/>
                ...
                <div className={styles.settingRow}>
                  ...
                  <select id="approvalMode" className={styles.settingInput} value={settings.approvalMode} onChange={(e) => patchSettings({ approvalMode: e.target.value as AutonomousSettings['approvalMode'] })}>
                ...
                <div className={styles.settingRow}>
                  ...
                  <input id="reflectionGoalPriorityThreshold" type="range" className={styles.settingInput} min={0} max={1} step={0.05} value={settings.reflectionGoalPriorityThreshold} onChange={(e) => {
                    const n = Number(e.target.value);
                    if (Number.isFinite(n))
                        patchSettings({ reflectionGoalPriorityThreshold: n });
                }}/>
                ...
                <div className={styles.settingsActions}>
                  <button className={styles.btnPrimary} disabled={settingsSaving} onClick={handleSaveSettings}>
        ...
        {activeTab === 'inner' && (<div className={styles.innerGrid}>
            <Card header={<TitledHeader title="现在的心情" tip={TIP_INNER_MOOD}/>}>
              {mood ? (<div className={styles.moodSummary}>
                  <MoodAvatar mood={mood} size={96}/>
                  <div className={styles.moodText}>{describeMood(mood)}</div>
                </div>) : (<div className={styles.emptyCompact}>暂无情绪数据</div>)}
            </Card>

            <Card header={<TitledHeader title="还在惦记" tip={TIP_INNER_CONCERNS}/>} className={styles.concernsCard} bodyClassName={styles.cardBodyFill}>
              {concerns.length > 0 ? (<div className={styles.concernsList}>
                  {sortConcerns(concerns).map((c) => (<div key={c.id} className={`${styles.concernItem} ${styles[`concern_${c.status}`]}`}>
                      ...
                      <div className={styles.concernMeta}>
                        ...
                        {c.status === 'open' && c.nextRaiseAfter > Date.now() && (<span>下次 {formatDiaryTime(c.nextRaiseAfter)} 可提起</span>)}
                      </div>
                    </div>))}
                </div>) : (<div className={styles.emptyCompact}>暂时没有放不下的事</div>)}
            </Card>

            <Card header={<TitledHeader title="日记" tip={TIP_INNER_DIARY}/>} className={styles.diaryCard} bodyClassName={styles.cardBodyFill}>
              {diary.length > 0 ? (<div className={styles.diaryList}>
                  {diary.map((d) => (<button key={d.id} type="button" className={styles.diaryItem} title={d.text} onClick={() => setSelectedDiary(d)}>
                      <div className={styles.diaryMeta}>{formatDiaryTime(d.timestamp)}</div>
                      <div className={styles.diaryText}>{d.text}</div>
                    </button>))}
                  {diaryHasMore && (<button type="button" className={styles.diaryLoadMore} disabled={diaryLoadingMore} onClick={loadMoreDiary}>
                      {diaryLoadingMore ? '加载中…' : '加载更早的日记'}
                    </button>)}
                </div>) : (<div className={styles.emptyCompact}>还没有日记。夜深时它会写点什么。</div>)}
            </Card>
          </div>)}
      </div>

      <Modal open={selectedDiary !== null} title={selectedDiary ? formatDiaryTime(selectedDiary.timestamp) : '日记详情'} onClose={() => setSelectedDiary(null)} width={560} layer={embedded ? 'aboveHub' : undefined}>
...
function sortConcerns(list: Concern[]): Concern[] {
    return [...list].sort((a, b) => CONCERN_STATUS_ORDER[a.status] - CONCERN_STATUS_ORDER[b.status]);
}
/** 能力测试结果中文标签 */
...
/** 单条能力测试记录 */
function CapabilityTestRow({ test }: {
    test: CapabilityTest;
}) {
    const delta = test.levelAfter != null && test.levelBefore != null ? test.levelAfter - test.levelBefore : null;
    const resultClass = {
    ...
    return (<div className={styles.capabilityTestRow}>
      ...
      <div className={styles.capabilityTestMeta}>
        ...
        {delta != null && (<span className={delta >= 0 ? styles.levelUp : styles.levelDown}>
            水平 {delta >= 0 ? '+' : ''}
            {(delta * 100).toFixed(1)}%
          </span>)}
...
function GoalCard({ goal, onApprove, onReject, onJump }: GoalCardProps) {
    ...
    return (<div className={`${styles.goalCard}${jumpable ? ` ${styles.goalCardClickable}` : ''}`} onClick={() => jumpable && onJump(goal)}>
      <div className={styles.goalHeader}>
        <span className={`${styles.goalType} ${styles[typeClass]}`}>{typeLabels[goal.type] || goal.type}</span>
        {goal.status && (<span className={`${styles.goalStatus} ${styles[`goalStatus_${goal.status}`] || ''}`}>
      ...
      {isPending ? (<div className={styles.goalActions}>
        ...
        </div>) : jumpable ? (<div className={styles.goalJumpHint}>查看关联反思 →</div>) : null}
    </div>);
}
/**
 * 规划任务卡片：紧凑单行（类型 + 描述 + 计划时间 + 删除），不展示审批/状态。
 */
function PlannedGoalCard({ goal, onDelete }: {
    goal: PlannedGoal;
    ...
    const scheduledLabel = goal.scheduledFor
        ...
        : '尽快';
    return (<div className={styles.plannedGoalRow}>
      <span className={`${styles.goalType} ${styles[typeClass]}`}>{typeLabels[goal.type] || goal.type}</span>
      <span className={styles.plannedGoalDesc}>{goal.description}</span>
      <span className={styles.plannedGoalTime}>{scheduledLabel}</span>
      <button type="button" className={styles.plannedGoalDelete} onClick={() => onDelete(goal.id)} title="删除该规划任务">
        删除
      </button>
    </div>);
}
