...
function openMenu(): void {
    act(() => {
        document.dispatchEvent(new MouseEvent('contextmenu', { bubbles: true, cancelable: true, clientX: 300, clientY: 250 }));
    });
}
/**
 * 菜单里的某一项。
...
function menuItem(label: string): HTMLElement | null {
    return ([...document.querySelectorAll<HTMLElement>('.context-menu-item')].find((el) => el.textContent?.trim() === label) ?? null);
}
...
describe('SelectionLayer 渲染契约', () => {
    ...
    it('菜单里点「翻译」：请求发出，且入口收起时没有顺手关掉气泡', async () => {
        ...
        const [request] = runSelection.mock.calls[0] as unknown as [
            {
                action: string;
                text: string;
            }
