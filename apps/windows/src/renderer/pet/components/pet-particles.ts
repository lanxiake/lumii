 ...
 * 点击烟花用的调色板（明亮、适合透明背景）
 *
 * **刻意不接主题令牌**：这里每帧随机取色，改成读令牌要么每帧 `getComputedStyle`
 * （强制样式重算，性能反模式），要么缓存 + 失效重读（复杂度不成比例）。
 * "透明桌面背景上要够亮"也是独立于主题的语义。**下一轮重构请勿"顺手统一"。**
 *
 * （2026-09-23 起宠物窗的 `<html>` **有** `data-theme` 了——`utils/pet-theme.ts`
 * 为气泡加的——所以上面说的"令牌取不到值"已不成立；不接的理由只剩前面那两条。）
 */
const PALETTE = [
  '#ff6b6b',
  '#ffd93d',
