...
function reportModelHover(isHovering: boolean): void {
    ...
    // 「靠近」会变成一个每帧触发的高频事件
    if (isHovering && !lastReportedHover)
        onModelApproach?.();
...
function toCanvasLocal(e: MouseEvent, canvas: HTMLCanvasElement): {
    x: number;
    y: number;
} {
...
export const PetCanvas = forwardRef<PetCanvasHandle, PetCanvasProps>(({ modelId, onDegrade, onModelLoaded, onInteraction, onAmbientActivity, petTuning, onRefusal, onTapped, onPetting, onContextMenu, ambientEnabled = true, bubbleHold = false }, ref) => {
    /**
    ...
    useImperativeHandle(ref, () => ({
        ...
        holdAmbientForOneShot: (ms: number) => {
            ...
            if (oneShotHoldRef.current !== null)
                clearTimeout(oneShotHoldRef.current);
    ...
    useEffect(() => {
        ...
        const setup = async () => {
            ...
            const instance = rendererType === 'sprite' ? new SpritePetRenderer() : new Live2dPetRenderer();
            await instance.init({
                canvas,
    ...
    useEffect(() => {
        ...
        log.info(`[loadModel] 模型加载 effect 触发：config=${config?.id ?? 'null'} renderer=${renderer?.backend ?? 'null'}` +
            (notCurrent ? '（实例已弃用，跳过）' : backendMismatch ? '（后端不匹配，跳过）' : ''));
        if (!renderer || !config || backendMismatch || notCurrent)
            return;
        let cancelled = false;
    ...
    useEffect(() => {
        ...
        if (!instance || renderer?.backend !== 'sprite')
            return;
    ...
    useEffect(() => {
        const instance = renderer?.instance;
        if (!ready || !instance || renderer?.backend !== 'sprite')
            return;
        if (rendererRef.current !== instance)
            return;
    ...
    useEffect(() => {
        ...
        if (ambientEnabled) {
            if (driver.isHeldBy(AMBIENT_HOLD_DISABLED))
                driver.resume(AMBIENT_HOLD_DISABLED);
        }
        else {
        ...
        const onMouseMove = (e: MouseEvent) => {
            ...
            if (dragRef.current.pressed) {
                ...
                if (dragRef.current.active && !pettingRef.current) {
                    ...
                    if (dragRef.current.samples.length > 200)
        ...
        const clampDrag = (want: {
            ...
            if (!ext) {
                ...
                if (!warnedNoExtentsRef.current) {
                    ...
                    log.warn('[clampDrag] 拿不到内容包围盒 → 拖动不夹取、贴边也不吸附（退回旧行为）。' +
            ...
            if (pointer.x <= EDGE_PUSH_PX)
                x = b.minX;
            else if (pointer.x >= vp.width - EDGE_PUSH_PX)
                x = b.maxX;
            if (pointer.y <= EDGE_PUSH_PX)
                y = b.minY;
            else if (pointer.y >= vp.height - EDGE_PUSH_PX)
                y = b.maxY;
            return { x, y };
        };
        /**
            ...
            if (!wasGrabbed) {
                ...
                if (over) {
                    ...
                    const refused = tuning?.traits
                        ? shouldRefuse(tuning.traits.agreeableness, tuning.mood?.valence ?? 0, 'interaction')
                        : false;
                    if (refused) {
                        ...
                        if (tapInteractionEnabled)
                            spawnClickFireworks(e.clientX, e.clientY);
                    }
                    /**
...
function triggerTapMotion(renderer: PetRendererProvider, hitArea: string): void {
    ...
    log.info(`[triggerTapMotion] hitArea="${hitArea}" 命中映射=${tapMotions ? JSON.stringify(tapMotions) : '无'}`);
    if (tapMotions) {
