/**
 * PetContextMenu：哪些项收起菜单、哪些项留在原地
 *
 * 用户的原话是「点击右键菜单中的选项除了需要跳转打开其他窗口的，都不需要自动关闭
 * 右键菜单……只有打开对话才需要」。所以判据分成两组：
 *   - 开关/列表类（静音、文字回复朗读、语音对话、换宠物）：**必须不关**
 *   - 要跳去别处看结果的（打开对话、关闭宠物模式）：必须关
 * 这组用例锁的是"点完还能看见勾选态变了没有"——菜单一关，回执就没了。
 *
