...
import { adaptiveScale, advanceSpriteFrame, BASE_BLINK_JITTER, BlinkScheduler, evaluateProcedural, findAnimation, hitTestPolygons, applyActivityModulation, IDENTITY_MODULATION, isIdentityModulation, mouthLevelIndex, parseAtlasIndex, randomAnimationIndex, applyOverrides, contentExtents, resolveSpriteRuntime, scaleProceduralParams, snapPixelScale, validateSpriteManifest, IDENTITY_PROCEDURAL_SCALES, type ActivityModulation, type ContentBox, type ProceduralScales, type ResolvedAnimation, type SlotState, type SlotOverride, type SpriteManifest, type SpriteRuntimeModel, } from '@mtbot/pet-core';
import type { HitArea, PetMotionPlayedInfo, PetRendererInitOptions, PetRendererProvider, } from '../types';
...
export class SpritePetRenderer implements PetRendererProvider {
    ...
    private mouthBinding: {
        slot: string;
        cat: string;
        levels: string[];
    } | null = null;
    /** 表情绑定：承载表情切换的槽位与类别，以及其全部候选部件 */
    private expressionBinding: {
        slot: string;
        cat: string;
        parts: string[];
    } | null = null;
    ...
    setProceduralScales(scales: ProceduralScales): void {
        if (this.proceduralScales === scales)
            return;
        const blinkChanged = this.proceduralScales.blink !== scales.blink ||
            this.proceduralScales.blinkJitterBonus !== scales.blinkJitterBonus;
        this.proceduralScales = scales;
        if (blinkChanged)
            this.rebuildBlinkScheduler(this.playing?.anim);
        log.info(`[setProceduralScales] bob=${scales.bob.toFixed(2)} breathe=${scales.breathe.toFixed(2)} ` +
    ...
    async loadModel(config: PetModelConfig): Promise<void> {
        if (!this.app)
            throw new Error('渲染器未初始化，请先 init()');
        this.unloadModel();
        ...
        if (runtime.idleDriftStripped.length > 0) {
            log.warn(`[loadModel] 「待机不许浮动」规则丢掉了 ${runtime.idleDriftStripped.join('、')}` +
    ...
    // 资源加载
    // -------------------------------------------------------------------------
    private async loadTextures(manifestUrl: string, atlasFile: string, atlasJsonFile: string): Promise<TextureMap> {
        ...
        if (!parsed.ok)
            throw new Error(`图集索引解析失败 — ${parsed.errors.join('; ')}`);
    ...
    private buildLayers(manifest: SpriteManifest, runtime: SpriteRuntimeModel, textures: TextureMap): void {
        ...
        for (const [slotName, def] of Object.entries(manifest.slots ?? {})) {
            if (slotName === 'base' || def.kind !== 'layered')
                continue;
        ...
        void runtime;
        void textures;
        log.info(`[buildLayers] 画布 ${manifest.canvas.w}×${manifest.canvas.h}，锚点 (${manifest.anchor[0]}, ${manifest.anchor[1]})，分层槽 ${Object.keys(layered).length} 个`);
    }
    /**
     * 认出口型层与表情层。
     *
    ...
    private bindMouthAndExpression(manifest: SpriteManifest): void {
        ...
        for (const [slotName, def] of Object.entries(manifest.slots ?? {})) {
            ...
            for (const cat of cats) {
                ...
                if (!mouth && MOUTH_NAMES.includes(lower))
                    mouth = { slot: slotName, cat };
                else if (!expression && EYES_NAMES.includes(lower))
                    expression = { slot: slotName, cat };
            }
    ...
    private onTick = (): void => {
        const app = this.app;
        if (!app || !this.loaded || !this.runtime || !this.root)
            return;
    ...
    private advanceAnimation(deltaMS: number): void {
        ...
        if (stepped.advanced)
            this.applyState(frames[p.frame]);
        this.maybeCompleteOnce(p);
    }
    private maybeCompleteOnce(p: PlayingState): void {
        if (p.anim.kind !== 'once' || p.completed)
            return;
        if (p.frame < p.anim.frames.length - 1)
            return;
        p.completed = true;
        this.motionListener?.({ group: p.anim.group, index: p.anim.index });
        if (p.anim.next)
            this.playMotion(p.anim.next);
    }
    ...
    setAgentActivityModulation(mod: ActivityModulation): void {
        ...
        if (wasIdentity !== nowIdentity) {
            log.info(`[setAgentActivityModulation] 姿态调制${nowIdentity ? '回到基线（activity 已 idle）' : '开始生效'}`);
        }
    }
    /** 程序化原语：每 tick 求值一次 */
    ...
    private applyAdaptiveScale(manifest: SpriteManifest): void {
        ...
        const s = adaptiveScale(manifest.canvas.h, this.app.renderer.height, requested, this.pixelArt, SPRITE_MAX_HEIGHT_RATIO);
        this.baseScale = s;
        log.info(`[applyAdaptiveScale] 画布高 ${manifest.canvas.h}，请求缩放 ${requested} → 实际 ${s}${this.pixelArt ? '（像素取整）' : ''}，` +
    ...
    private resetToGround(center: boolean): void {
        ...
        if (this.root)
            this.root.position.set(this.posX, this.posY);
    }
    adjustScaleByDelta(delta: number): void {
        ...
        if (Math.abs(this.userScaleFactor - before) < 1e-6)
            return;
    ...
    private measureContentBox(anim: ResolvedAnimation): ContentBox | null {
        const src = (this.baseTexture?.resource as {
            source?: unknown;
        } | undefined)?.source;
        ...
        for (const frame of anim.frames) {
            ...
            try {
                ctx.drawImage(src as CanvasImageSource, r.x, r.y, r.width, r.height, 0, 0, r.width, r.height);
            }
            catch {
    ...
    setExpression(expressionIndex: number): void {
        ...
        if (!binding || binding.parts.length === 0)
            return;
    ...
    hitTest(localX: number, localY: number): HitArea {
        ...
        if (!runtime || !this.loaded)
            return null;
        return hitTestPolygons(runtime.manifest, this.playing?.anim.group ?? null, localX, localY, this.modelTransform(runtime.manifest));
    }
    /**
