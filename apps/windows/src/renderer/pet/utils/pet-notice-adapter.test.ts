...
import { INITIAL_TURN_FACTS, advanceTurnFacts, extractTaskCompletion, isNoticeEvent, isPetMoodEvent, noticeActionLabel, toNoticeEvent, type RawAgentEvent, } from './pet-notice-adapter';
...
describe('extractTaskCompletion —— 只认「真的完成」', () => {
    ...
    it('解析失败 / 形状不符 / 缺 status → completed:false，且**不抛**', () => {
        ...
        expect(extractTaskCompletion({ content: [{ type: 'text', text: '{"summary":"没有 status"}' }] })).toEqual({ completed: false });
    });
    it('completed 但 summary 是空白 → 仍算完成，只是不带摘要（pet-core 用兜底文案）', () => {
        expect(extractTaskCompletion({ content: [{ type: 'text', text: '{"status":"completed","summary":"  "}' }] })).toEqual({ completed: true, summary: undefined });
    });
...
describe('toNoticeEvent', () => {
    ...
    it('会话键按 rootSessionKey 归一（子 Agent 各有一把 key，共用一个 root）', () => {
        const n = toNoticeEvent({ type: 'agent:permission:request', sessionKey: 'child-1', rootSessionKey: 'root-1' }, facts);
    ...
    it('子 Agent 的 name/status 映射到 subagentName/subagentStatus', () => {
        const n = toNoticeEvent({ type: 'agent:subagent:completed', sessionKey: 's', name: '查资料', status: 'failed' }, facts);
        expect(n?.subagentName).toBe('查资料');
        expect(n?.subagentStatus).toBe('failed');
    });
    it('fileChanges 折算成 fileCount', () => {
        const n = toNoticeEvent({ type: 'agent:turn:file-changes', sessionKey: 's', fileChanges: [{}, {}, {}] }, facts);
        expect(n?.fileCount).toBe(3);
    });
    it('只有 task_complete 才解析 summary', () => {
        const withTask = toNoticeEvent({ type: 'agent:tool:end', sessionKey: 's', toolName: 'task_complete', result: taskResult('完成了') }, facts);
        expect(withTask?.summary).toBe('完成了');
        const withBash = toNoticeEvent({ type: 'agent:tool:end', sessionKey: 's', toolName: 'bash', result: taskResult('不该被解析') }, facts);
    ...
    it('本轮事实被原样带上（pet-core 用它让 turn:end 让位）', () => {
        const n = toNoticeEvent({ type: 'agent:turn:end', sessionKey: 's', durationMs: 200000 }, {
...
describe('宠物感知的建议（五期 T5.1②）', () => {
    ...
    it('有建议 → 按钮是"让我去看看"', () => {
        ...
        const notice = noticeFromEvent(toNoticeEvent(withProposal('查一下「像素流水线」相关的资料'), INITIAL_TURN_FACTS)!, { now: 1000000 });
        expect(notice).not.toBeNull();
        expect(noticeActionLabel(notice!)).toBe('让我去看看');
    });
    it('没有建议 → 按钮还是"回到刚才"（对照组：别把这个默认值改掉）', () => {
        const notice = noticeFromEvent(toNoticeEvent(withoutProposal(), INITIAL_TURN_FACTS)!, { now: 1000000 });
        expect(noticeActionLabel(notice!)).toBe('回到刚才');
    });
    it('建议是空白串时不产生 proposal（按钮保持"回到刚才"）', () => {
        const notice = noticeFromEvent(toNoticeEvent(withProposal('   '), INITIAL_TURN_FACTS)!, { now: 1000000 });
