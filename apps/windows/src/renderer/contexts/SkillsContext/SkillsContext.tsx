...
interface SkillsContextType extends SkillsState {
    ...
    /** 安装技能 */
    installSkill: (params: InstallSkillParams) => Promise<{
        success: boolean;
        error?: string;
    }>;
    /** 卸载技能 */
...
export const SkillsProvider: React.FC<SkillsProviderProps> = ({ children, autoLoad = true }) => {
     ...
     * 安装技能
     */
    const installSkill = useCallback(async (params: InstallSkillParams): Promise<{
        success: boolean;
        error?: string;
    }> => {
     ...
     * 卸载技能
     */
    const uninstallSkill = useCallback(async (skillId: string): Promise<{
        success: boolean;
        error?: string;
    }> => {
     ...
     * 启用技能
     */
    const enableSkill = useCallback(async (skillId: string): Promise<{
        success: boolean;
        error?: string;
    }> => {
        ...
        try {
            ...
            if (ok) {
                setState(prev => {
                    const newSkills = prev.skills.map(s => s.id === skillId ? { ...s, enabled: true } : s);
                    saveCachedSkills(newSkills);
                    return {
                        ...prev,
                        skills: newSkills,
     ...
     * 禁用技能
     */
    const disableSkill = useCallback(async (skillId: string): Promise<{
        success: boolean;
        error?: string;
    }> => {
        ...
        try {
            ...
            if (ok) {
                setState(prev => {
                    const newSkills = prev.skills.map(s => s.id === skillId ? { ...s, enabled: false } : s);
                    saveCachedSkills(newSkills);
                    return {
                        ...prev,
                        skills: newSkills,
