 ...
 */
// ─── 通话状态枚举 ───────────────────────────────────────────────────────────
export type VoiceCallState = 'initializing' // 正在初始化引擎
 | 'listening' // 等待用户说话
 | 'recognizing' // 正在识别
 | 'thinking' // Agent 处理中
 | 'speaking' // TTS 播放中
...
// ─── 模型状态 ────────────────────────────────────────────────────────────────
/** 单项下载任务状态 */
export type VoiceModelDownloadState = 'idle' | 'downloading' | 'paused' | 'extracting' | 'ready' | 'error';
 ...
 */
export type VoiceRuntimePhase = 'idle' | 'checking_python' | 'installing_deps' | 'starting_engine' | 'loading_model' | 'synthesizing' | 'playing' | 'ready' | 'error';
...
type VoiceEvent = VoiceCallStateEvent | VoiceTranscriptEvent | VoiceTtsChunkEvent | VoiceTtsAudioFileEvent | VoiceCallEndedEvent | VoiceModelsStatusEvent | VoiceModelsProgressEvent | VoiceModelsErrorEvent | VoiceRuntimeStatusEvent | VoiceTtsPreviewEndedEvent | VoiceErrorEvent | VoiceConfigUpdatedEvent;
// ─── 错误码 ──────────────────────────────────────────────────────────────────
type VoiceErrorCode = 'mic_permission_denied' // 麦克风权限被拒绝
...
export type Qwen3TtsVariant = '0.6b-custom' | '1.7b-custom' | '0.6b-base' | '1.7b-base';
/**
 * 是否为声音克隆（Base）变体。
 *
