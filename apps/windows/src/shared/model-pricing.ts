...
function tok(input: number, output: number, cacheRead: number = 0, cacheWrite: number = 0): ModelPrice {
    return {
...
function usdTok(inputUsd: number, outputUsd: number, cacheReadUsd: number = 0, cacheWriteUsd: number = 0): ModelPrice {
    return tok(usdPerMToYuanPerM(inputUsd), usdPerMToYuanPerM(outputUsd), usdPerMToYuanPerM(cacheReadUsd), usdPerMToYuanPerM(cacheWriteUsd));
}
/** 构造按次计费的价格项 */
 ...
 */
export function estimateCostYuan(modelId: string, tokens: TokenBreakdown, tsMs?: number): number | undefined {
    ...
    const { inputTokens = 0, outputTokens = 0, cacheReadTokens = 0, cacheWriteTokens = 0, callCount = 1, } = tokens;
    const M = 1000000;
    let yuan = (inputTokens / M) * price.inputYuanPerMTok +
    ...
    return Math.round(yuan * 1000000) / 1000000;
}
/**
 ...
 */
function estimateCostCents(modelId: string, promptTokens: number, completionTokens: number): number | undefined {
    ...
    return Math.round((yuan * (100 / USD_TO_CNY)) * 10000) / 10000;
}
/**
