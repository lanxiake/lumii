...
function runGit(workspaceDir: string, gitdir: string, args: string[], opts: {
    timeoutMs?: number;
...
async function mustRun(workspaceDir: string, gitdir: string, args: string[], errPrefix: string, opts: {
    env?: Record<string, string>;
    raw?: boolean;
    stdin?: string;
} = {}): Promise<GitRunResult> {
 ...
 */
function parseStatusPorcelain(stdout: string): Array<{
    filepath: string;
...
function parseNameStatus(stdout: string): Array<{
    filepath: string;
    status: VcsFileStatus;
}> {
    const out: Array<{
    ...
    for (const line of stdout.split('\n')) {
        ...
        if (!filepath)
            continue;
        const status: VcsFileStatus = letter === 'A' ? 'added' : letter === 'D' ? 'deleted' : 'modified';
        out.push({ filepath, status });
    }
    return out;
}
...
export async function logCli(workspaceDir: string, gitdir: string, limit: number): Promise<RawLogEntry[]> {
 ...
 */
export async function statusDiffCli(workspaceDir: string, gitdir: string, ref: string): Promise<VcsDiffEntry[]> {
    const r = await mustRun(workspaceDir, gitdir, ['status', '--porcelain', '-z', '--no-renames', `--untracked-files=all`], 'git status');
 ...
 * 子进程里，工作区再大也不占主线程。
 */
export async function diffCommitsCli(workspaceDir: string, gitdir: string, fromOid: string, toOid: string, opts: {
    withHunks?: boolean;
} = {}): Promise<VcsDiffEntry[]> {
    const r = await mustRun(workspaceDir, gitdir, ['diff', '--name-status', '--no-renames', fromOid, toOid], 'git diff --name-status');
 ...
 */
export async function diffFileCli(workspaceDir: string, gitdir: string, fromOid: string, toOid: string, filepath: string): Promise<VcsDiffEntry> {
    const oldContent = fromOid === 'WORKTREE'
        ? readWorktreeFile(workspaceDir, filepath)
        : (await readFileAt(workspaceDir, gitdir, fromOid, filepath)) ?? '';
    const newContent = toOid === 'WORKTREE'
        ? readWorktreeFile(workspaceDir, filepath)
        : (await readFileAt(workspaceDir, gitdir, toOid, filepath)) ?? '';
    const status: VcsFileStatus = oldContent === '' && newContent !== '' ? 'added'
        : newContent === '' && oldContent !== '' ? 'deleted'
            : 'modified';
    if (Buffer.byteLength(oldContent, 'utf8') > MAX_DIFF_BYTES ||
...
export async function readBlobBytes(workspaceDir: string, gitdir: string, oid: string, filepath: string): Promise<Uint8Array | null> {
...
export async function readFileAt(workspaceDir: string, gitdir: string, oid: string, filepath: string): Promise<string | null> {
    const bytes = await readBlobBytes(workspaceDir, gitdir, oid, filepath);
    return bytes === null ? null : new TextDecoder().decode(bytes);
}
/** 某 commit 下的全部文件路径 */
export async function listFilesAt(workspaceDir: string, gitdir: string, oid: string): Promise<Set<string>> {
...
export async function initCli(workspaceDir: string, gitdir: string, defaultBranch = 'main'): Promise<void> {
 ...
 */
export async function commitCli(workspaceDir: string, gitdir: string, message: string, author = GIT_AUTHOR): Promise<string | null> {
    ...
    const r = await runGit(workspaceDir, gitdir, ['commit', '-q', '--no-verify', '--allow-empty', '-F', '-'], { env: authorEnv(author), stdin: message });
    if (r.code !== 0 || hasFatalError(r.stderr)) {
...
export async function hasStagedChangesCli(workspaceDir: string, gitdir: string): Promise<boolean> {
    ...
    if (r.code === 1)
        return true;
    if (/ambiguous argument|unknown revision|does not have any commits/i.test(r.stderr))
        return true;
    throw new Error(`git diff --cached 退出码 ${r.code}: ${r.stderr.trim().slice(0, 300)}`);
}
