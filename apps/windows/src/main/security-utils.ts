...
// 日志输出
const log = {
    debug: (...args: unknown[]) => console.log('[Security:DEBUG]', ...args),
...
export class SecurityUtils {
    ...
    sanitizeInput(input: string, options: {
        ...
        const { maxLength = 10000, allowedChars, stripHtml = true, stripControlChars = true, } = options;
    ...
    isCommandAllowed(command: string): boolean {
        ...
        return this.config.allowedCommands.some((allowed) => allowed.toLowerCase() === cmdBase);
    }
    /**
     * 验证 URL 是否安全
     */
    validateUrl(url: string, options: {
        ...
        if (!allowedProtocols.includes(parsedUrl.protocol)) {
            throw new SecurityError(`不允许的协议: ${parsedUrl.protocol}`, 'FORBIDDEN_PROTOCOL');
        }
        // 检查是否是本地地址（可能的 SSRF）
     ...
     */
    createSearchRegExp(pattern: string, flags = 'i'): RegExp {
        ...
        try {
            ...
            // 非法正则：退回为对原始文本的字面量子串匹配
            return new RegExp(this.escapeRegExp(pattern), flags);
        }
    }
    /**
     * 验证数字范围
     ...
     * 验证字符串
     */
    validateString(value: unknown, options: {
        minLength?: number;
        maxLength?: number;
        pattern?: RegExp;
    } = {}): string {
 ...
 * 导出便捷函数
 */
const validatePath = (path: string, basePath?: string) => securityUtils.validatePath(path, basePath);
const sanitizeInput = (input: string, options?: Parameters<SecurityUtils['sanitizeInput']>[1]) => securityUtils.sanitizeInput(input, options);
export const sanitizeFileName = (fileName: string) => securityUtils.sanitizeFileName(fileName);
export const sanitizeCommandArg = (arg: string) => securityUtils.sanitizeCommandArg(arg);
export const validateUrl = (url: string, options?: Parameters<SecurityUtils['validateUrl']>[1]) => securityUtils.validateUrl(url, options);
export const escapeRegExp = (str: string) => securityUtils.escapeRegExp(str);
export const createSafeRegExp = (pattern: string, flags?: string) => securityUtils.createSafeRegExp(pattern, flags);
export const createSearchRegExp = (pattern: string, flags?: string) => securityUtils.createSearchRegExp(pattern, flags);
