...
function hasSherpaAsrMetadata(modelPath: string): boolean {
    try {
        ...
        if (size < 1000000)
            return false;
        const readLen = Math.min(131072, size);
...
class LocalStreamingParaformerAsr implements AsrProvider {
    ...
    createStream(): AsrStream {
        if (!this.recognizer)
            throw new Error('ASR 未初始化，请先调用 initialize()');
...
class LocalOfflineParaformerAsr implements AsrProvider {
    ...
    async initialize(): Promise<void> {
        ...
        if (!hasSherpaAsrMetadata(modelPath)) {
            throw new Error('ASR 模型不是 sherpa-onnx 格式（缺少 vocab_size 元数据）。请到设置 → 语音重新下载 Paraformer。');
        }
        // eslint-disable-next-line @typescript-eslint/no-require-imports
        const SherpaOnnx = require('sherpa-onnx-node') as any;
        try {
...
export function createAsrProvider(config: {
    ...
    switch (config.provider) {
        case 'local-paraformer':
            if (!config.modelDir)
                throw new Error('local-paraformer 需要 modelDir');
