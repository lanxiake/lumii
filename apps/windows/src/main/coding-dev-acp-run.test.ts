...
import { resolveAcpTimeoutMs, ACP_TIMEOUT_ENV_VAR, DEFAULT_ACP_TIMEOUT_MS, } from './coding-dev-backends-stub/acp-config.js';
...
describe('resolveAcpTimeoutMs', () => {
    ...
    afterEach(() => {
        if (original === undefined)
            ...
            process.env[ACP_TIMEOUT_ENV_VAR] = original;
    });
    it('未设置时回落默认 60 分钟', () => {
        delete process.env[ACP_TIMEOUT_ENV_VAR];
