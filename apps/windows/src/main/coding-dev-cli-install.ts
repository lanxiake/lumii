...
import { detectLocalAcpTool, isPrimaryLocalAcpToolId, type LocalAcpToolStatus, type PrimaryLocalAcpToolId, } from './coding-dev-cli-detect.js';
...
function resolveUninstallRecipe(status: LocalAcpToolStatus): UninstallRecipe {
    ...
    switch (status.id) {
        case 'claude':
            if (npmPath)
                return npmUninstall('@anthropic-ai/claude-code');
            // 官方文档给出的原生安装卸载路径（不动 ~/.claude 用户配置）
            return {
                displayCommand: 'Remove-Item "$env:USERPROFILE\\.local\\bin\\claude.exe"; Remove-Item "$env:USERPROFILE\\.local\\share\\claude" -Recurse',
                powershellCommand: 'Remove-Item -LiteralPath "$env:USERPROFILE\\.local\\bin\\claude.exe" -Force -ErrorAction SilentlyContinue; Remove-Item -LiteralPath "$env:USERPROFILE\\.local\\share\\claude" -Recurse -Force -ErrorAction SilentlyContinue',
                documented: true,
                hint: '按官方文档移除原生安装文件；~/.claude 用户配置与登录状态保留，需要彻底清理请手动删除。',
            };
        case 'codex':
            if (npmPath)
                return npmUninstall('@openai/codex');
            return {
                displayCommand: 'Remove-Item "$env:LOCALAPPDATA\\Programs\\OpenAI\\Codex" -Recurse',
                powershellCommand: 'Remove-Item -LiteralPath "$env:LOCALAPPDATA\\Programs\\OpenAI\\Codex" -Recurse -Force -ErrorAction SilentlyContinue',
                documented: false,
        ...
        case 'cursor':
            ...
            return {
                displayCommand: 'Remove-Item "$env:USERPROFILE\\.local\\bin\\agent.exe"; Remove-Item "$env:LOCALAPPDATA\\cursor-agent" -Recurse',
                powershellCommand: 'Remove-Item -LiteralPath "$env:USERPROFILE\\.local\\bin\\agent.exe" -Force -ErrorAction SilentlyContinue; Remove-Item -LiteralPath "$env:USERPROFILE\\.local\\bin\\cursor-agent.exe" -Force -ErrorAction SilentlyContinue; Remove-Item -LiteralPath "$env:LOCALAPPDATA\\cursor-agent" -Recurse -Force -ErrorAction SilentlyContinue',
                documented: false,
                hint: 'Cursor 官方未提供卸载命令，此路径取自官方安装脚本的安装目录（~/.local/bin）。PATH 中的残留条目需手动清理。',
            };
        case 'opencode':
            if (npmPath)
                return npmUninstall('opencode-ai');
            return {
 ...
 */
function runPowershell(command: string, timeoutMs: number): Promise<{
    exitCode: number | null;
    stdout: string;
    stderr: string;
}> {
...
export async function uninstallLocalAcpTool(toolIdRaw: string): Promise<AcpUninstallResult> {
    ...
    const job = (async (): Promise<AcpUninstallResult> => {
        ...
        const { exitCode, stdout, stderr } = await runPowershell(recipe.powershellCommand, 5 * 60000);
        refreshCommonCliPathsInProcessEnv();
