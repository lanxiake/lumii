 ...
 * **注意是 `add`，不是 `commit`**：iso 的 `commit` 只读 index 里的 mode/oid，
 * 全链路不碰工作区文件内容。2026-09-20 我把「提交慢」归因到 `commit` 上并改成真 git，
 * 实测反而慢 8~23×（见 docs/fix/2026-09-20-主进程冻结调查与修复.md 附录⑤），已撤销。
