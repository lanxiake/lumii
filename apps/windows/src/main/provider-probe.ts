 ...
 */
import { type CapabilitySlot, type LocalProviderConfigView, type ProviderType, PROVIDER_DEFAULT_BASE_URL, ensureProviderBaseUrl, missingApiKeyMessage, } from './provider-config.js';
 ...
 * 测试指定能力槽连通性（最小请求）
 */
export async function testProviderConnection(slot: CapabilitySlot, cfg: LocalProviderConfigView): Promise<ProviderTestResult> {
    if (!cfg.enabled) {
