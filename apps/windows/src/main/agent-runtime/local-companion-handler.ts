 ...
 */
export async function handleLocalCompanionInstruction(instruction: string, deps: LocalCompanionDeps, options: LocalCompanionRunOptions = {}): Promise<string> {
    switch (instruction.trim()) {
        ...
        case '__wiki_ero_extract__': {
            if (!deps.runWikiEroExtract)
                return 'wiki ero extract unavailable';
            return deps.runWikiEroExtract();
        }
        case '__wiki_purge_broken_refs__': {
            if (!deps.runWikiPurgeBrokenRefs)
                return 'wiki purge broken refs unavailable';
            return deps.runWikiPurgeBrokenRefs();
        }
        case '__wiki_purge_invalid_files__': {
            if (!deps.runWikiPurgeInvalidFiles)
                return 'wiki purge invalid files unavailable';
            return deps.runWikiPurgeInvalidFiles();
        }
        case '__evolution_tick__': {
            if (!deps.runEvolutionTick)
                return 'evolution tick unavailable';
            return deps.runEvolutionTick();
        }
        case '__pet_dispatch__': {
            if (!deps.runPetDispatch)
                return 'pet dispatch unavailable';
            return deps.runPetDispatch();
        }
        case '__pet_sensing__': {
            if (!deps.runPetSensing)
                return 'pet sensing unavailable';
            // 感知走 `manual` 只是为了让「立即执行」能在主窗口自测（宠物模式门闩是软门闩）
            return deps.runPetSensing({ manual: options.manual === true });
        }
        case '__pet_evolve__': {
            if (!deps.runPetEvolve)
                return 'pet evolve unavailable';
 ...
 */
async function handleTick(deps: LocalCompanionDeps, options: LocalCompanionRunOptions = {}): Promise<string> {
 ...
 */
async function handleMemoryConsolidation(deps: LocalCompanionDeps, depth: 'fast' | 'deep'): Promise<string> {
    ...
    try {
        ...
        return `executed: ${depth === 'fast' ? '轻量' : '深度'}整理完成，${delta > 0 ? `精简 ${delta} 字` : `补充 ${-delta} 字`}`;
    }
    catch (err) {
...
function readTodayCount(db: DatabaseAdapter): number {
    try {
        const row = db.prepare<{
            value: string;
        }>(`SELECT value FROM runtime_state WHERE key = ?`).get(KV_KEY_DAILY_COUNT) as {
            value: string;
        } | undefined;
        if (!row)
            return 0;
...
function incrementTodayCount(db: DatabaseAdapter): void {
    ...
    db.prepare(`INSERT OR REPLACE INTO runtime_state (key, value, updated_at) VALUES (?, ?, ?)`).run(KV_KEY_DAILY_COUNT, JSON.stringify({ date: today, count: count + 1 }), now);
}
// ── KV 读写：上次发送时间 ──
function readLastSentAt(db: DatabaseAdapter): number | null {
    try {
        const row = db.prepare<{
            value: string;
        }>(`SELECT value FROM runtime_state WHERE key = ?`).get(KV_KEY_LAST_SENT_AT) as {
            value: string;
        } | undefined;
        if (!row)
            return null;
...
function writeLastSentAt(db: DatabaseAdapter, ts: number): void {
    const now = new Date().toISOString();
    db.prepare(`INSERT OR REPLACE INTO runtime_state (key, value, updated_at) VALUES (?, ?, ?)`).run(KV_KEY_LAST_SENT_AT, String(ts), now);
}
// ── Cron Job 种子 ──
    ...
    try {
        ...
        for (const job of COMPANION_CRON_JOBS) {
            ...
            const existing = db.prepare<{
                id: string;
            }>(`SELECT id FROM local_cron_jobs WHERE id = ?`).get(job.id) as {
                id: string;
            } | undefined;
            if (!existing) {
                db.prepare(`INSERT INTO local_cron_jobs
           (id, name, task_text, agent_id, schedule_type, schedule_expr, next_run_at, interval_ms, enabled, created_at)
           VALUES (?, ?, ?, NULL, 'cron', ?, ?, NULL, ?, ?)`).run(job.id, job.name, job.taskText, job.scheduleExpr, now, isTick && !careEnabled ? 0 : 1, now);
                log.info(`[ensureCompanionCronJobsSeeded] 新建 job id=${job.id}`);
            }
            else if (isTick) {
                db.prepare(`UPDATE local_cron_jobs SET name = ?, enabled = ? WHERE id = ?`).run(job.name, careEnabled ? 1 : 0, job.id);
            }
            else {
...
export function syncCompanionTickJobEnabled(db: DatabaseAdapter, enabled: boolean): void {
    try {
        db.prepare(`UPDATE local_cron_jobs SET enabled = ? WHERE id = ?`).run(enabled ? 1 : 0, 'companion-tick');
...
export function migrateLocalCompanionPrefsToVhSettings(db: DatabaseAdapter): void {
    try {
        const migratedRow = db.prepare<{
            value: string;
        }>(`SELECT value FROM runtime_state WHERE key = ?`).get(KV_KEY_MIGRATED) as {
            value: string;
        } | undefined;
        if (migratedRow) {
            return;
        ...
        const markMigrated = (): void => {
            const now = new Date().toISOString();
            db.prepare(`INSERT OR REPLACE INTO runtime_state (key, value, updated_at) VALUES (?, ?, ?)`).run(KV_KEY_MIGRATED, '1', now);
        };
        const currentVh = getVirtualHumanSettings();
        const isStillDefault = currentVh.proactiveCareEnabled === DEFAULT_VH_SETTINGS.proactiveCareEnabled
        ...
        const prefsRow = db.prepare<{
            value: string;
        }>(`SELECT value FROM runtime_state WHERE key = ?`).get(KV_KEY_PREFS) as {
            value: string;
        } | undefined;
        if (prefsRow) {
...
function appendHistory(db: DatabaseAdapter, content: string): void {
    try {
        ...
        const now = new Date().toISOString();
        db.prepare(`INSERT OR REPLACE INTO runtime_state (key, value, updated_at) VALUES (?, ?, ?)`).run(KV_KEY_HISTORY, JSON.stringify(updated), now);
    }
    catch (err) {
...
function readHistory(db: DatabaseAdapter): LocalCompanionAction[] {
    try {
        const row = db.prepare<{
            value: string;
        }>(`SELECT value FROM runtime_state WHERE key = ?`).get(KV_KEY_HISTORY) as {
            value: string;
        } | undefined;
        if (!row)
            return [];
...
function updateHistoryFeedback(db: DatabaseAdapter, id: string, feedback: 'accept' | 'ignore' | 'dismiss'): void {
    try {
        const history = readHistory(db);
