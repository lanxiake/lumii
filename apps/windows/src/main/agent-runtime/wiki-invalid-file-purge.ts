 ...
 */
import { purgeInvalidWikiFiles, listInvalidWikiFiles, type WikiRepo, type WikiSource, } from '@mtbot/agent-runtime';
 ...
 */
export function purgeInvalidWikiFilesOnDisk(wikiRepo: WikiRepo, workspaceRoot?: string, agentId = DEFAULT_WIKI_AGENT_ID, userId = DEFAULT_WIKI_USER_ID): {
    deleted: number;
    titles: readonly string[];
} {
    const result = purgeInvalidWikiFiles(wikiRepo, agentId, userId, (sources) => {
