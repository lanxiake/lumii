 ...
 *
 * ## 2026-09-20 定案：暂不引入（**这是决策，不是待办**）
 *
 * **理由：当前测试用例不足，无法证明它的真正效果。** 需收集足够多的用例之后
 * 才能确认，故曰"暂"——代码保留，将来可重估。
 *
 * 现有数据不足以支撑引入：
 * - v7 评测集 **n=53**：凸组合 α=0.6 得 nDCG@10 **0.906** vs 纯 FTS **0.844**
 *   （+6.2pp）——但一条翻转 ≈ **±1.9pp**，这个差只有约 3 条查询；
 * - 该集为**单人标注、无第二标注者复核**，标注者偏差未排除；
 * - top-3~10 区间**还有约 158 项未补判**，当前所有指标应视为保守下界；
 * - **纯向量单独 0.528，远差于纯 FTS 0.844**；"FTS 漏掉的查询由向量救回"的
 *   专项实验是 **0/9**（见 `2026-09-18-宫殿向量开启效果实测.md` §8.1）。
 *
 * 而代价是确定的：引入第三份 onnxruntime（与 sherpa VAD、onnxruntime-node 共存，
 * 加载顺序闸只保证不崩）、E5 模型加载、向量索引缺口需持续回填。
 *
 * **重估的前提**是先有足够的用例：评测集扩到 150+ 并引入第二标注者复核
 * （见 `docs/test/memory-eval/2026-09-19-混合检索评测方法论复盘与优化.md`
 * §9.1 / §9.4 / §9.5）。
 *
 * ## 禁止静默降级
 *
 ...
 * 默认关闭。设为 '1' 启用。
 *
 * **2026-09-20 定案：暂不引入**——当前测试用例不足，无法证明其真正效果，
 * 需收集足够多的用例后再确认（理由与数据见本文件头部，以及
 * `docs/test/memory-eval/2026-09-19-混合检索评测方法论复盘与优化.md` §9.1）。
 * **这是决策，不是待办**：别把它当成"等着被打开的功能"。
 *
 * **与 wiki 的默认值故意不同**（那边 `DEFAULT_LUMII_WIKI_VECTOR = '1'`）：
 * wiki 的模型加载与索引成本本机已经在付；宫殿是新账，连模型都不加载——
 * 没开就不该付任何代价。
 */
