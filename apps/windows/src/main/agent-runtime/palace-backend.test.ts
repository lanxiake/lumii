...
describe('withBuiltinPalace', () => {
    ...
    it('同一段重复归档幂等：仍只有一条，检索也只出一条', async () => {
        ...
        const rows = db
            .prepare<{
            c: number;
        }>('SELECT COUNT(*) AS c FROM palace_drawers')
            .get()!.c;
        expect(rows).toBe(1);
    ...
    it('作用域：agentId 缺省跨 Agent，传了就收窄（与工作记忆通道同规则）', async () => {
        ...
        expect(await config.searchPalace!('雪山', 10, { userId: 'local-user', agentId: 'agent-a' })).toHaveLength(1);
        // 别的用户搜不到（命名空间隔离）
    ...
    it('数据库未打开时降级：检索返回 null、读取返回 null、归档返回 undefined，且不抛异常', async () => {
        ...
        await expect(config.archivePalaceDrawer!({
            content: 'x',
            wing: 'w',
            room: 'r',
...
describe('onConversationEnd（每轮归档）', () => {
    ...
    function seedConversation(db: ReturnType<typeof createMigratedTestDb>, convId: string, agentId: string) {
        db.prepare(`INSERT INTO conversations (id, user_id, type, title, is_active, created_at, last_msg_at)
       VALUES (?, 'local-user', 'direct', 't', 1, '2026-09-18T00:00:00Z', '2026-09-18T00:00:00Z')`).run(convId);
        db.prepare(`INSERT INTO conversation_participants (conversation_id, participant_type, participant_id, joined_at)
       VALUES (?, 'agent', ?, '2026-09-18T00:00:00Z')`).run(convId, agentId);
        db.prepare(`INSERT INTO messages (id, conversation_id, role, content_json, timestamp, is_streaming)
       VALUES (?, ?, 'assistant', '{"type":"text","text":"x"}', '2026-09-18T00:00:01Z', 0)`).run(`m-${convId}`, convId);
    }
    ...
    it('先转发宿主原回调（自主进化的轮次结算还挂在上面）', () => {
        ...
        } as unknown as AgentRuntimeBridgeConfig, fakeLocalDb(db));
    ...
    it('宿主回调抛异常也不连累归档（内容仍要进宫殿）', () => {
        ...
        expect(db.prepare<{
            c: number;
        }>('SELECT COUNT(*) AS c FROM palace_drawers').get()!.c).toBe(1);
    });
    it('DB 未打开 / 空文本 / 空 convId 时静默跳过，不抛异常', () => {
        ...
        expect(db.prepare<{
            c: number;
        }>('SELECT COUNT(*) AS c FROM palace_drawers').get()!.c).toBe(0);
    });
    it('归档归属取会话的 Agent，而不是写死 assistant', () => {
        ...
        const row = db.prepare<{
            agent_id: string;
        }>('SELECT agent_id FROM palace_drawers').get()!;
    ...
    it('没有参与者记录时退到 messages.agent_id', () => {
        const db = createMigratedTestDb();
        db.prepare(`INSERT INTO conversations (id, user_id, type, title, is_active, created_at, last_msg_at)
       VALUES ('conv-6', 'local-user', 'direct', 't', 1, '2026-09-18T00:00:00Z', '2026-09-18T00:00:00Z')`).run();
        db.prepare(`INSERT INTO messages (id, conversation_id, agent_id, role, content_json, timestamp, is_streaming)
       VALUES ('m6', 'conv-6', 'code-dev', 'assistant', '{"type":"text","text":"x"}', '2026-09-18T00:00:01Z', 0)`).run();
        const config = withBuiltinPalace(emptyConfig, fakeLocalDb(db));
    ...
    it('会话没有 agent 参与者时兜底 assistant（老会话/建会话竞态）', () => {
        const db = createMigratedTestDb();
        db.prepare(`INSERT INTO conversations (id, user_id, type, title, is_active, created_at, last_msg_at)
       VALUES ('conv-5', 'local-user', 'direct', 't', 1, '2026-09-18T00:00:00Z', '2026-09-18T00:00:00Z')`).run();
        const config = withBuiltinPalace(emptyConfig, fakeLocalDb(db));
