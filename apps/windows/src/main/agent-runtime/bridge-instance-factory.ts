...
import { type AgentRegistry, type ToolRegistry, type MessageBus, type AgentDefinitionStore, type AgentOrchestrator, type AuditRepo, type ConversationRepo, type FileRepo, type MemoryManager, ModelRouter, createDirectStreamFn, ProactivityScheduler, BUILT_IN_AGENTS, createStreamFnFactory, assembleAgent, type AssembleAgentRuntime, type ConfigProvider, type PromptContextProvider, type EventSink, type ModelOverride, type ResolvedModel, type PermissionProvider, type AgentRuntimeEvent, type AgentRuntimeFeatureFlags, type AgentDefinition, type ToolExecutionContext, type SkillInfo, type CustomAgentInfo, type UserDeviceInfo, type McpServerHint, type ContextFile, } from '@mtbot/agent-runtime';
...
import { createBashCommandLogHook, applyToolDefinitionStyle, type BashCommandRepo, type PromptStyle, } from '@mtbot/agent-runtime';
...
import { createAgentInstanceRuntimeEventHandler, type InstanceRuntimeMetrics, } from './bridge-agent-instance-events';
...
export interface BridgeInstanceFactoryDeps {
    ...
    createSummaryGenerator: (innerStream: StreamFn, model: import('@earendil-works/pi-ai/compat').Model<any>) => any;
    /**
...
export class BridgeInstanceFactory {
    ...
    /** 创建 Agent 实例 */
    async createInstance(agentDef?: AgentDefinition, sessionKey?: string, conversationId?: string, options?: {
         ...
         */
        const resolveDirectBaseUrl = (cfg = readLiveProviderCfg()) => cfg ? ensureProviderBaseUrl(cfg.baseUrl, cfg.type) : undefined;
        /**
        ...
        const buildLiveDirectStream = (): StreamFn => {
            return (model, context, options) => {
                ...
                try {
                    ...
                    const existingSessionId = (options as {
                        sessionId?: unknown;
                    })?.sessionId;
                    const optionsWithSession = {
                        ...options,
                        sessionId: typeof existingSessionId === 'string' && existingSessionId
                            ? existingSessionId
                            : ctx.sessionKey,
                    };
        ...
        const wrapStreamFn = (_inner: StreamFn, resolved: ResolvedModel): StreamFn => {
            ...
            return (model, context, options) => {
                ...
                log.info(`[streamFn] rootKey=${rootSessionKey} effKey=${effectiveSessionKey} pref=${pref ?? '(none)'} defaultModel=${model.id} thinking=${thinking.thinkingEnabled} effort=${thinking.reasoningEffort}`);
                ...
                const level = resolveReasoningOptions(thinking.reasoningEffort, targetModel.api, liveCfg?.baseUrl ?? '');
        ...
        const isDisabledMcpTool = (name: string): boolean => disabledMcpServers.some((server) => name.startsWith(`mcp__${server}__`));
        ...
        const permission: PermissionProvider = {
            requestPermission: async (input) => {
                ...
                const sessionKey = input.rootSessionKey ?? this.deps.instanceToConversation.get(input.instanceId);
        ...
        const skillHitRateTracker = createSkillHitRateHook(this.deps.config.updateSkillAutoScope);
        {
            ...
            if (s)
                s.skillHitRateTracker = skillHitRateTracker;
        }
        const optionalHooks = [
        ...
        const promptContext: PromptContextProvider = {
            getSkills: async () => {
                ...
                try {
                    ...
                    log.info(`[createInstance] Loaded ${kept.length}/${r.length} skills for system prompt（会话禁用 ${r.length - kept.length} 个）`);
                    return kept;
                }
                catch (err) {
            ...
            getCustomAgents: async () => {
                if (!this.deps.config.getCustomAgents)
            ...
            getUserDevices: async () => {
                if (!this.deps.config.getUserDevices)
            ...
            getSoulContent: async () => {
                if (!this.deps.config.getSoulContent)
                    return undefined;
                try {
                    const r = await this.deps.config.getSoulContent();
                    if (r)
                        log.info(`[createInstance] 已加载用户 SOUL 内容 (${r.length} chars)`);
                    return r;
                }
                catch (err) {
        ...
        const richHandler = createAgentInstanceRuntimeEventHandler({
            ...
            calibrateSessionCharsPerToken: (k, modelId, tokens) => this.deps.calibrateSessionCharsPerToken(k, modelId, tokens),
        ...
        const runtime: AssembleAgentRuntime = {
            ...
            toolLogger: {
                log: (...args: unknown[]) => log.info(args.map((a) => (typeof a === 'string' ? a : JSON.stringify(a))).join(' ')),
                error: (...args: unknown[]) => log.error(args.map((a) => (typeof a === 'string' ? a : JSON.stringify(a))).join(' ')),
            },
            onTelemetry: (metric) => {
                log.info(`[ToolTelemetry] ${metric.toolName} durationMs=${metric.durationMs} success=${metric.success}` +
        ...
        log.info(`[createInstance] model.id=${model.id}, model.api=${(model as any).api}, purpose=${purpose}, ` +
        ...
        const initialPrompt = await this.deps.promptComposer.buildPromptWithMemory(instanceId, promptResult, injSettings);
        ...
        if (def.proactivity?.triggers?.length) {
            ...
            sched.start();
            {
                const s = this.deps.instanceStates.get(instanceId);
                if (s)
                    s.proactivityScheduler = sched;
            }
            log.info(`[Proactivity] scheduler started instance=${instanceId} triggers=${def.proactivity.triggers.length}`);
        }
    ...
    async buildImageContents(imagePaths?: readonly string[]): Promise<import('@earendil-works/pi-ai/compat').ImageContent[]> {
        ...
        for (const p of imagePaths) {
            try {
                ...
                if (wasResized) {
                    log.info(`[buildImageContents] 已压缩: ${path.basename(absPath)} ${(originalBytes / 1024 / 1024).toFixed(2)}MB → ${(finalBytes / 1024 / 1024).toFixed(2)}MB (${mimeType})`);
                }
                else {
        ...
        log.info(`[buildImageContents] 完成: 输入 ${imagePaths.length} 张 → 成功 ${blocks.length} 张, 总 ${(totalFinalBytes / 1024 / 1024).toFixed(2)}MB (base64 后约 ${(totalFinalBytes * 4 / 3 / 1024 / 1024).toFixed(2)}MB)`);
        return blocks;
    }
}
