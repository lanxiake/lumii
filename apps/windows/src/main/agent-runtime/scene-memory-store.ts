 ...
 */
export function resolveSceneFilePath(baseDir: string, scene: SceneKind, key: string, projectPath?: string | null): string {
...
export async function loadRegistry(baseDir: string): Promise<SceneRegistry> {
    ...
    try {
        ...
        if (!parsed || !Array.isArray(parsed.projects))
            return { projects: [] };
        return {
            projects: parsed.projects.filter((e): e is ProjectEntry => !!e && typeof e.key === 'string' && typeof e.name === 'string'),
        };
    }
    catch (err) {
...
async function saveRegistry(baseDir: string, registry: SceneRegistry): Promise<void> {
    ...
    await fsp.writeFile(resolveSceneRegistryPath(baseDir), JSON.stringify({ projects: registry.projects }, null, 2), 'utf-8');
}
// ── 项目登记 ──────────────────────────────────────────────────────────────────
...
export function findProject(registry: SceneRegistry, ref: string): ProjectEntry | null {
    ...
    const byPath = registry.projects.find((p) => p.path && normalizePathForCompare(p.path) === asPath);
    if (byPath)
        return byPath;
    const byName = registry.projects.find((p) => p.name.toLowerCase() === lower);
    if (byName)
        return byName;
    const byAlias = registry.projects.find((p) => p.aliases.some((a) => a.toLowerCase() === lower));
    return byAlias ?? null;
}
/**
 ...
 * @returns 登记后的条目
 */
export async function registerProject(baseDir: string, input: {
    name: string;
    path?: string | null;
    ...
    if (!existing)
        existing = findProject(registry, name);
    if (existing) {
        const aliasSet = new Set(existing.aliases);
        if (projPath)
            aliasSet.add(projPath);
...
async function touchProject(baseDir: string, key: string): Promise<void> {
    try {
        const registry = await loadRegistry(baseDir);
        const projects = registry.projects.map((p) => p.key === key ? { ...p, lastActiveAt: Date.now() } : p);
        await saveRegistry(baseDir, { projects });
    }
    catch (err) {
...
export async function writeSceneMemory(filePath: string, content: string): Promise<boolean> {
    try {
        if (content.length > SCENE_MEMORY_MAX_CHARS) {
            log.warn(`[writeSceneMemory] 超出 ${SCENE_MEMORY_MAX_CHARS} 字符限制 (${content.length})，拒绝写入 path=${filePath}`);
            return false;
        }
