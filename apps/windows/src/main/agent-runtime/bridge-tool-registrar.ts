 ...
 */
import { createMtBotTool, type MtBotToolConfig, type SpawnAgentParams, todoWriteToolConfig, spawnAgentToolConfig, sendMessageToolConfig, shouldNudgeVerification, VERIFICATION_NUDGE_TEXT, } from '@mtbot/agent-runtime';
...
export class BridgeToolRegistrar {
        ...
        const realConfig: MtBotToolConfig = {
            ...
            execute: async (_toolCallId, rawParams, _toolCtx) => {
                ...
                const withVerificationNudge = (result: ReturnType<typeof jsonToolResult>) => {
                    if (!this.deps.getFeatureFlags().ENABLE_VERIFICATION_NUDGE)
                        return result;
                    try {
                ...
                switch (params.action) {
                    ...
                    case 'batch_create': {
                        ...
                        const created = items.map((item) => taskRepo.create({
                            conversationId,
                    ...
                    case 'batch_update': {
                        ...
                        const results = updates.map((item) => {
                            if (!item.taskId)
                                return { taskId: '', status: 'error', message: 'taskId required' };
                        ...
                        return withVerificationNudge(jsonToolResult({
                            ...
                            tasks: results.filter((r) => r.status === 'ok').map((r) => (r as {
                                task: unknown;
                            }).task),
    ...
    private registerSpawnAgentOverride(): void {
        ...
        const realConfig: MtBotToolConfig = {
            ...
            execute: async (toolCallId, params, _toolCtx) => {
                ...
                const result = await this.deps.ensureOrchestrator().spawnAgent(params as SpawnAgentParams, parentId);
                ...
                if (result.mode === 'sync') {
                    ...
                    return jsonToolResult({
                        ...
                        note: "You MUST now summarize or integrate the above `output` into your " +
    ...
    private recordAgentMessageDelivered(fromId: string, to: string, message: string): void {
        ...
        try {
            ...
            const fromLabel = agents.find((a) => a.agentId === fromId)?.name
    ...
    private registerBrowserTools(): void {
        ...
        if (!this.deps.toolContext || !getBrowserContext)
            return;
    ...
    private registerAppUiTools(): void {
        ...
        if (!this.deps.toolContext || !getMainWindow)
            return;
        registerAppUiToolsFn(this.deps.toolRegistry, this.deps.toolContext, {
            ...
            readSettingsJson: async () => {
                ...
                if (!win || win.isDestroyed())
                    return null;
                try {
                    return await win.webContents.executeJavaScript(`localStorage.getItem('mtbot-assistant-settings')`);
                }
                catch {
                    return null;
                }
