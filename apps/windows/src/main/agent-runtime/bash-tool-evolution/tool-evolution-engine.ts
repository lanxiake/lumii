...
import { BashCommandRepo, checkToolDraft, draftToolFromPattern, mineCommandPatterns, normalizeCommand, openTemplateRejectionReason, selectHighValuePatterns, DEFAULT_MIN_COUNT_EXCLUSIVE, DEFAULT_TOP_N_FOR_LLM, type CommandSample, type TemplateToolDefinition, } from '@mtbot/agent-runtime';
import { agentRuntimeLog as log } from '../bridge-utils';
import { loadPendingDrafts, savePendingDrafts, saveApprovedTool, loadApprovedTools, loadStoredTools, updateToolStatus, removeApprovedTool, newDraftId, type PendingToolDraft, } from './tool-writer';
/** 统计与冷却窗口：7 天 */
...
export function buildApprovalPrompt(draft: PendingToolDraft): string {
    ...
    if (whenToUse)
        lines.push(`建议使用：${whenToUse}`);
    if (whenNotToUse)
        lines.push(`不建议：${whenNotToUse}`);
    ...
    lines.push(``);
    lines.push(`回复「启用」将其注册为系统工具（之后 Agent 直接调用，不再重写命令）；回复「不用」丢弃。`);
    return lines.join('\n');
}
/**
 * 找出与候选模板等价（参数位归一后相同）的已批准工具名。
 */
function findSimilarApprovedTools(commandTemplate: string, approved: Array<{
    name: string;
...
export class ToolEvolutionEngine extends EventEmitter {
    ...
    async runMiningCycle(): Promise<MiningSummary> {
        ...
        const approvedCanonical = new Set(loadStoredTools().map(({ def }) => canonicalizeParamSlots(def.commandTemplate)));
        let attemptedLlm = false;
        for (const pattern of highValue) {
            if (summary.drafted >= maxCandidates)
                break;
            if (this.pending.length >= maxPending)
                break;
            ...
            if (approvedCanonical.has(draftCanonical) ||
            ...
            const gate = checkToolDraft({ ...draft, needsPermission: true }, pattern, { normalize: normalizeCommand, registeredNames: existingNames });
            if (!gate.passed) {
                summary.gatedOut++;
        ...
        log.info(`[ToolEvolution] 挖掘周期完成: scanned=${summary.samplesScanned} patterns=${summary.patternsFound} ` +
