...
/** app_act 工具 description（设计 §13.2 分工原文 + type/key/scroll） */

const APP_ACT_DESCRIPTION = `点控件：app_act click（先截图拿 ref，ref 不跨 snapshotId 复用）

输入文本：app_act type（ref + text，中文用 native setter，禁止 keyCode 打拼音）

按键：app_act key（白名单 Enter/Escape/Tab/Backspace/Delete/方向键，无需 ref）

滚动：app_act scroll（ref + dx/dy）
...
const APP_ACT_ACTIONS = ['click', 'type', 'key', 'scroll'] as const
    ...
    createMtBotTool(

      {
        ...
        parameters: Type.Object({
          ...
          action: Type.String({ description: '操作类型：click | type | key | scroll' }),

          ref: Type.Optional(

            Type.String({ description: '来自 app_screenshot 的元素 ref（click/type/scroll 必填）' }),
          ...
          clear: Type.Optional(Type.Boolean({ description: 'type 操作是否先清空再写入' })),
          ...
          key: Type.Optional(
            ...
            Type.String({
              ...
              description:
                ...
                'key 操作的白名单按键：Enter | Escape | Tab | Backspace | Delete | ArrowUp/Down/Left/Right',
