...
const SEED_JOBS: readonly SeedJob[] = [
  ...
  },
  {
    ...
    systemPrompt: [
      ...
      '3. 按下面的顺序组织内容，全文控制在 300 字内；条目用序号排列（1. 2. 3.）、每条一行，不要用表格和代码块：',
      '   - 今天的日期与星期；',
      '   - 从昨天日报中提取今天最该动手的 2-3 件事，各一句话；',
      '   - 若有明确的截止时间或约定时间，单独列出；',
      '   - 结尾一句简短的开场提示，不要说教、不要客套。',
      '如果 work_report_read 找不到昨天的日报，先用 memory_manage action=window days=2 取用户近期的工作记忆，再考虑 memory_search 或 wiki_search。',
  ...
  },
  {
    ...
    systemPrompt: [
      '你在帮用户整理当天工作日报。请：',
      '1. 读取用户今天的工作记忆：用 memory_manage action=window days=1 取全量（跨 Agent，能看到用户在其他 Agent 里的工作痕迹）；',
      '   返回带 hasMore 时用 offset 继续翻页取完，不要只看第一页；',
      '2. 从中提取今天实际完成的工作、进行中的任务、遇到的问题；',
      '3. 输出三段，全文不超过 400 字；各段条目用序号排列（1. 2. 3.）、每条一行，不要用表格和代码块：',
      '   「今天完成」——具体做完的事，动词开头；',
  ...
  },
  {
    ...
    systemPrompt: [
      ...
      '2. 基于这些日报汇总本周工作情况；',
      '3. 输出四段，全文不超过 600 字；各段条目用序号排列（1. 2. 3.）、每条一行，不要用表格和代码块：',
      '   「本周产出」——按主题归并，不要逐日罗列；',
      '   「卡住的地方」——写清阻塞原因，而不只是现象；',
      '   「值得留下的判断」——本周做过的关键取舍，各一句；',
      '   「下周计划」——最多 5 条，可执行、有明确产出。',
      '如果找不到本周的日报，就用 memory_manage action=window days=7 读取本周工作记忆（跨 Agent 全量，带 hasMore 时翻页取完）来汇总。',
      '信息不足的段落如实说明缺什么，不要凑数。',
    ].join('\n'),
    scheduleType: 'cron',
    scheduleExpr: '0 17 * * 5',
  ...
  },
  {
    ...
    activeHourEnd: 18,
    notifyTargets: 'system',
    // 默认开启（用户可在定时任务页关闭）：这条每 2 小时触发一次，
    // 若觉得打扰，关闭后种子哨兵保证不会在重启时被重新打开。
    enabled: true,
  },
  {
