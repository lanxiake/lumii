...
import { WeixinAuthError, normalizeIlinkBaseUrl, buildMediaFallbackText, extractMediaFileKey, extractMediaCrypto, downloadAndSaveMediaItems, isQrLoginComplete, isQrScannedPendingConfirm, isQrExpiredStatus, } from './weixin-message-utils.js';
...
export class WeixinLoginService extends EventEmitter {
    ...
    constructor(opts?: {
        ...
        const rawBase = opts?.baseUrl ?? process.env.MTBOT_WEIXIN_BASE_URL ?? ILINK_DEFAULT_URL;
    ...
    async initialize(): Promise<void> {
        ...
        if (savedSession) {
            if (savedSession.baseUrl)
                this.baseUrl = normalizeIlinkBaseUrl(savedSession.baseUrl);
    ...
    private async probeSession(session: WeixinSession): Promise<boolean> {
        try {
            await this.apiGetUpdates(session, '', AbortSignal.timeout(8000));
            return true;
        }
        catch (err) {
    ...
    private async runCheckLoginLoop(qrcode: string, signal: AbortSignal): Promise<void> {
        ...
        while (this.checkLoginLoopRunning && !signal.aborted) {
            ...
            try {
                ...
                if (!this.checkLoginLoopRunning || signal.aborted)
                    return;
    ...
    private async handleLoginSuccess(result: QRStatusResponse): Promise<void> {
        ...
        const effectiveBaseUrl = normalizeIlinkBaseUrl(result.baseurl ? result.baseurl.replace(/\/$/, '') : this.baseUrl);
    ...
    private async pollLoop(session: WeixinSession, signal: AbortSignal): Promise<void> {
        ...
        while (!signal.aborted && this.status === 'logged_in') {
            try {
                ...
                if (typeof result.longpolling_timeout_ms === 'number' &&
                    Number.isFinite(result.longpolling_timeout_ms) &&
                    result.longpolling_timeout_ms >= 5000 &&
                    result.longpolling_timeout_ms <= 120000) {
                        ...
                        if (normalized.mediaItems && normalized.mediaItems.length > 0) {
                            ...
                            if (this.silkAsrCallback) {
                                ...
                                for (const item of normalized.mediaItems) {
                                    if (!item.localPath?.endsWith('.silk'))
                                        continue;
    ...
    private normalizeMessage(raw: WeixinRawMessage, session?: WeixinSession): WeixinNormalizedMessage | null {
        ...
        if (items.length > 0) {
            const itemsSummary = items.map((it) => ({
                ...
                hasMediaEncrypt: Boolean(it.image_item?.media?.encrypt_query_param ??
     ...
     */
    async sendTextReply(toUserId: string, text: string, contextToken: string, botToken?: string, ilinkBaseUrl?: string, title?: string): Promise<boolean> {
        ...
        for (let i = 0; i < segments.length; i++) {
            if (i > 0)
                await new Promise((resolve) => setTimeout(resolve, SEGMENT_SEND_INTERVAL_MS));
     ...
     */
    async sendMediaReply(toUserId: string, filePath: string, fileName: string | undefined, contextToken: string, botToken?: string, ilinkBaseUrl?: string): Promise<boolean> {
        const session = await this.sessionStore.loadSession();
        const token = botToken ?? session?.botToken;
        const baseUrl = ilinkBaseUrl ?? session?.baseUrl ?? this.baseUrl;
        if (!token) {
    ...
    private async apiFetchQrCode(): Promise<{
        qrcode: string;
    ...
    private async apiGetUpdates(session: WeixinSession, getUpdatesBuf: string, signal: AbortSignal, timeoutMs = LONG_POLL_TIMEOUT_MS): Promise<GetUpdatesResponse> {
