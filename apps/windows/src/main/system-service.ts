...
import { securityUtils, SecurityError, createSafeRegExp, createSearchRegExp, } from './security-utils';
...
export class SystemService {
    ...
    async listDirectory(dirPath: string): Promise<FileInfo[]> {
        ...
        try {
            ...
            const visibleEntries = entries.filter((entry) => !HIDDEN_DIRS.has(entry.name));
    ...
    async searchFiles(dirPath: string, pattern: string, options: {
        ...
        const extSet = options.extensions && options.extensions.length > 0
        ...
        const search = async (currentPath: string) => {
            ...
            try {
                ...
                for (const entry of entries) {
                    ...
                    if (isDirectory && skipDirs.has(name))
                        continue;
                    // 工作区根层跳过挂载项目目录（其自身可能是编译产物遍地的代码仓库，严重拖慢搜索）；
                    // 仅根层跳过，避免误伤用户项目内部同名子目录
                    if (isRoot && isDirectory && name === 'projects')
                        continue;
