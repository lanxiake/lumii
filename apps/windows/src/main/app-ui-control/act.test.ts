...
import { assertClickAllowed, buildClickPrepareScript, buildScrollScript, buildSelectScript, buildTypeScript, CLICK_BLOCK_ROLES, isKeyAllowed, KEY_WHITELIST, } from './act';
...
describe('assertClickAllowed', () => {
    it('缺少 ref 返回 missing_ref', () => {
        expect(assertClickAllowed({ ref: '', snapshotId: '42', current, blockRoles: [] })).toEqual({ ok: false, error: 'missing_ref' });
        expect(assertClickAllowed({ ref: undefined, snapshotId: '42', current, blockRoles: [] })).toEqual({ ok: false, error: 'missing_ref' });
    });
    it('snapshotId 不匹配返回 stale_snapshot', () => {
        expect(assertClickAllowed({ ref: 'e1', snapshotId: '99', current, blockRoles: [] })).toEqual({ ok: false, error: 'stale_snapshot' });
    });
    it('ref 不在当前快照返回 stale_snapshot', () => {
        expect(assertClickAllowed({ ref: 'e99', snapshotId: '42', current, blockRoles: [] })).toEqual({ ok: false, error: 'stale_snapshot' });
    });
    ...
    it('校验通过返回匹配 ref', () => {
        expect(assertClickAllowed({ ref: 'e1', snapshotId: '42', current, blockRoles: ['composer'] })).toEqual({ ok: true, ref: sampleRef });
    });
    it('未提供 snapshotId 时跳过过期校验', () => {
        expect(assertClickAllowed({ ref: 'e1', snapshotId: undefined, current, blockRoles: [] })).toEqual({ ok: true, ref: sampleRef });
    });
...
describe('buildSelectScript', () => {
    ...
    it('按 value 选中并派发 change', () => {
        const select = mountSelect('<select><option value="openai">OpenAI 兼容</option><option value="anthropic">Anthropic</option></select>');
    ...
    it('按可读文案模糊匹配选中', () => {
        const select = mountSelect('<select><option value="openai">OpenAI 兼容</option><option value="ollama">Ollama（本地）</option></select>');
 ...
 */
function stubScrollableContainer(el: HTMLElement, scrollHeight: number, clientHeight: number): void {
    let top = 0;
