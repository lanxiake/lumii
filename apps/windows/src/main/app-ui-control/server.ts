...
import { createAppUiController, type AppUiController, type AppUiScreenshotOptions, type ResizeImageFn, } from './controller';
import { isAppUiControlEnabled } from './enabled';
import { createSlidingWindowRateLimiter } from './rate-limit';
import { assertWritablePatch, buildPatchScript, buildReadScript, expandPathValue, } from './settings-channel';
import { loadProviderConfig, loadProviderSlotsConfig, loadSlotConfig, saveProviderConfig, isCapabilitySlot, PROVIDER_DEFAULT_BASE_URL, type CapabilitySlot, type LocalProviderConfigView, type ProviderType, } from '../provider-config';
 ...
 * 处理已鉴权的路由。
 */
async function handleRoute(controller: AppUiController, pathname: string, body: unknown, res: http.ServerResponse): Promise<void> {
    switch (pathname) {
        case '/screenshot': {
...
async function handleSkillsSetEnabledRoute(body: unknown, res: http.ServerResponse): Promise<void> {
    const skillId = (body as {
        skillId?: unknown;
    } | null)?.skillId;
    const enabled = (body as {
        enabled?: unknown;
    } | null)?.enabled;
 ...
 */
async function handleCloudSyncConfirmMassDeleteRoute(body: unknown, res: http.ServerResponse): Promise<void> {
    const m = getCloudSyncManager();
    if (!m) {
        sendJson(res, 200, { ok: false, error: 'not_ready' });
        return;
    }
    const fingerprint = (body as {
        fingerprint?: unknown;
    } | null)?.fingerprint;
...
async function handleCloudSyncResolveRoute(body: unknown, res: http.ServerResponse): Promise<void> {
    ...
    if (Array.isArray(b.choices)) {
        choices = b.choices.filter((c): c is {
            path: string;
            side: 'local' | 'remote';
        } => !!c &&
            typeof c === 'object' &&
...
async function handleStatusRoute(res: http.ServerResponse): Promise<void> {
    ...
    if (!status.configuration.hasProviders) {
        ...
        status.recommendations.push(status.configuration.apiKeyDecryptFailed
...
async function handleSettingsReadRoute(body: unknown, res: http.ServerResponse): Promise<void> {
    ...
    const keyPath = (body as {
        keyPath?: unknown;
    } | null)?.keyPath;
    const raw = await win.webContents.executeJavaScript(buildReadScript(typeof keyPath === 'string' ? keyPath : undefined));
...
async function handleCommandRoute(body: unknown, res: http.ServerResponse): Promise<void> {
    ...
    const dispatch = activeDeps?.dispatchCommand ??
        (async (cmd: unknown) => {
            const bridge = getAgentRuntimeBridge();
            if (!bridge)
...
function createRequestHandler(controller: AppUiController, token: string): http.RequestListener {
    return async (req, res) => {
        ...
        const enabled = await isAppUiControlEnabled(activeDeps?.readSettingsJson ?? (async () => null));
        if (!enabled) {
 ...
 */
export async function startAppUiControlServer(deps: AppUiControlServerDeps): Promise<AppUiRuntimeConfig> {
    if (httpServer) {
        throw new Error('App UI control server already running');
    }
    activeDeps = deps;
    activeRateLimiter =
        deps.rateLimiter ?? createSlidingWindowRateLimiter({ limit: 100, windowMs: 60000 });
