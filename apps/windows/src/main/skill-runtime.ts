...
import { BUILTIN_SKILLS, type SkillRunMode, type SkillExecuteRequest, type SkillExecuteResult, type SkillExecuteError, type SkillErrorCode, type SkillDefinition, type SkillExecutionContext, } from './skill-runtime-types';
...
export type { SkillExecuteRequest, SkillExecuteResult, SkillErrorCode, SkillDefinition, } from './skill-runtime-types';
// 日志输出
...
export class ClientSkillRuntime extends EventEmitter {
     ...
     * 创建错误结果
     */
    private createErrorResult(requestId: string, code: SkillErrorCode, message: string, startTime: number): SkillExecuteResult {
        return {
            requestId,
            success: false,
            error: { code, message },
     ...
     */
    private selectRunner(runtime: string): {
        execute: (opts: import('./ts-runner').RunnerOptions) => Promise<import('./ts-runner').RunnerResult>;
    } | null {
     ...
     */
    private createExternalSkillDefinition(manifest: SkillManifest, entryPath: string): SkillDefinition {
        // 确保 skillStore 已初始化
        if (!this.skillStore) {
    ...
    async queryExecutionLogs(filter: {
        ...
        entries: import('./skill-execution-logger').ExecutionLogEntry[];
        total: number;
    }> {
     ...
     * @returns 安装结果
     */
    async handleSkillInstallPush(request: SkillInstallPushRequest): Promise<{
        success: boolean;
        error?: string;
    }> {
