...
import { getChannelInteractionHub, tryHandleChannelOutOfBand, } from '../channel-interaction-hub';
...
import { CHANNEL_ACK_TEXT, buildChannelErrorMessage, } from '../channel-error-helper';
import { extractMediaAttachmentLines, isPureMediaMessage, } from '../../weixin-message-utils.js';
import { pendingAttachments, makePendingKey, ATTACHMENT_HELD_HINT, type PendingAttachment, } from '../pending-attachments';
...
import { getChannelVoiceAsrFailedHint, isWeixinSilkAsrFailed, } from '../channel-voice-asr-hint.js';
const log = {
...
export class WeixinChannelAdapter implements IChannelAdapter {
    ...
    constructor(private readonly weixinLoginService: WeixinLoginService, private readonly bridge: AgentRuntimeBridge, private readonly acpBackendManager: AcpBackendManager, replyContextStore?: WeixinReplyContextStore | null) {
    ...
    private persistReplyContext(msg: WeixinNormalizedMessage): void {
        if (!this.replyContextStore || !msg.contextToken)
            return;
    ...
    async sendTextReply(session: ChannelSession, text: string): Promise<void> {
        const ctx = session.replyContext as {
            contextToken?: string;
            botToken?: string;
            ilinkBaseUrl?: string;
        } | undefined;
        ...
        const ok = await this.weixinLoginService.sendTextReply(session.channelUserId, text, ctx.contextToken, ctx.botToken, ctx.ilinkBaseUrl);
        if (!ok) {
    ...
    startListening(): void {
        this.weixinLoginService.on('message', (msg: WeixinNormalizedMessage) => {
            ...
            if (this.tryHandleOutOfBand(msg))
                return;
    ...
    private tryHandleOutOfBand(msg: WeixinNormalizedMessage): boolean {
        return tryHandleChannelOutOfBand({
            ...
            onError: (err) => log.error(`[tryHandleOutOfBand] 失败: ${err instanceof Error ? err.message : String(err)}`),
        });
    }
    ...
    private async handleMessage(msg: WeixinNormalizedMessage): Promise<void> {
        ...
        if (!hasCommand) {
            ...
            log.info(`[handleMessage] 纯附件消息已挂起 channelUserId=${msg.channelUserId} count=${pendingAttachments.count(pendingKey)}`);
            if (isFirstOfBatch) {
                const session = this.buildSession(msg);
                // 通知渲染进程展示用户消息，避免对话断层
                this.bridge.ensureConversationExists(session.sessionKey, `微信对话 - ${msg.channelUserId}`, this.channelType);
        ...
        const allMediaLines = [...pending, ...currentAttachments].map((a) => `[media attached: ${a.mediaPath}${a.fileName ? ` (${a.fileName})` : ''}]`);
        // 指令部分 = 用户纯文本 + 语音转录（如果有）
        const commandParts: string[] = [];
        if (userTextOnly)
            commandParts.push(userTextOnly);
        if (transcript)
            commandParts.push(`[语音转录: ${transcript}]`);
    ...
    setActiveSessionKey(channelUserId: string, sessionKey: string, source: RouteSource = 'own'): void {
        this.route.setActive(channelUserId, sessionKey, source);
        log.info(`[setActiveSessionKey] channelUserId=${channelUserId} → sessionKey=${sessionKey} source=${source}`);
    }
    /**
     ...
     */
    private async handleAcpPrompt(msg: WeixinNormalizedMessage, session: ChannelSession, prompt: string, backendId: string, cwd?: string): Promise<void> {
        ...
        try {
            ...
            if (abortController.signal.aborted) {
                ...
                const waitedMinutes = Math.max(1, Math.round((Date.now() - startedAt) / 60000));
