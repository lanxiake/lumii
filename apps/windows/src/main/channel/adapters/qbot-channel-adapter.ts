...
import { getChannelVoiceAsrFailedHint, isChannelVoiceAsrFailed, } from '../channel-voice-asr-hint.js';
const qbotNewCommand: CommandHandler = {
    ...
    async execute(ctx: CommandContext): Promise<void> {
        ...
        await adapter.sendTextReply({ ...session, sessionKey: newSessionKey, instanceId: null }, `✅ 已新建对话。\n会话ID: ${newSessionKey.slice(-8)}`);
    },
};
const log = {
...
export class QbotChannelAdapter implements IChannelAdapter {
    ...
    constructor(private readonly qbotLoginService: QbotLoginService, private readonly bridge: AgentRuntimeBridge) {
