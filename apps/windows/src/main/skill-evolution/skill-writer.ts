...
// ─── Patch 应用逻辑 ──────────────────────────────────────────────────────────
function applyPatchToContent(skillMd: string, oldString: string, newString: string): {
    success: boolean;
    result: string;
} {
...
export async function writeNewSkill(skillName: string, skillMd: string, meta: SkillMeta, category?: string): Promise<void> {
 ...
 */
async function findSkill(skillName: string): Promise<{
    skillName: string;
    category?: string;
} | null> {
    // 先尝试根目录
...
export async function applyPatch(skillName: string, patchOldString: string, patchNewString: string, record: Omit<EvolutionRecord, 'version'>): Promise<{
    method: 'patch' | 'full_rewrite';
}> {
    const found = await findSkill(skillName);
    if (!found)
