...
import { registerLocalMediaSchemePrivileged, registerLocalMediaProtocolHandler, setLocalMediaWorkspaceCwdGetter, } from './local-media-protocol';
import { registerPetAssetSchemePrivileged, registerPetAssetProtocolHandler, } from './pet/pet-asset-protocol';
...
import { loadProviderConfig, loadProviderSlotsConfig, saveProviderConfig, saveProviderSlotsConfig, loadSlotConfig, applyImageSlotToDrawEnv, isCapabilitySlot, type LocalProviderConfigView, type ProviderSlotsConfigView, type CapabilitySlot, } from './provider-config';
import { listAgentDefinitions, getAgentRecord, listUserAgentRecords, forkAgentRecord, updateAgentRecord, deleteAgentRecord, } from './agents-repo';
...
import { createChannelHub, createChannelPeerStore, createWeixinReplyContextStore, type ChannelHub, type ChannelPeerStore, } from './channel/channel-hub-bootstrap';
import { resolveWindowsClientDataRoot } from './client-data-root';
import { transcribeVoiceFile } from './channel/media-pipeline';
import { setActiveWorkspaceDirGetter, ensureWorkspaceTempLayout, resolveRecordingsDir, } from './workspace-paths';
import { createScreenRecordService, createRealScreenRecordServiceDeps, parseScreenRecordSettings, registerScreenRecordIpc, type ScreenRecordService, } from './screen-record';
...
import { AgentRuntimeBridge, installAgentRuntimeCommandIpc, setAgentRuntimeBridgeForIpc, setWeixinBindingManagerForIpc, setAudioTranscribeCallback, setIpcMainWindow, getAcpBackendManager, getSessionKeyForInstance, invalidateAgentInstancesForProviderChange, } from './agent-runtime';
import { submitVoiceTranscript } from './ipc/agent-runtime-ipc.js';
import { readSoulFile, readUserMemoryFile, setupCloakBrowserIpcHandlers, writeSoulFile, writeUserMemoryFile, } from './ipc/plugin-ipc';
...
import { applyCodingDevAcpEnvToProcess, buildCodingDevEnvInfo, defaultWorkspaceFallback, resolveCodingDevAcpWorkspacePath, } from './coding-dev-env.js';
...
import { registerPetModeIpc, switchPetMode, isPetMode, isPetForceIgnore, disablePetForceIgnore, disposePetModeIpc, } from './pet/pet-mode-ipc';
...
function initScreenRecordService(): void {
    ...
    const deps = createRealScreenRecordServiceDeps({
        ...
        readSettingsJson: async () => {
            if (!mainWindow || mainWindow.isDestroyed())
                return null;
            try {
                return await mainWindow.webContents.executeJavaScript(`localStorage.getItem('mtbot-assistant-settings')`);
            }
            catch {
                return null;
            }
    ...
    deps.emitStatusChanged = (detail) => {
        origEmit(detail);
        if (detail.ok) {
            trayManager?.updateScreenRecordState(detail.status === 'recording', detail.elapsedMs ?? 0, detail.status === 'paused');
        }
        else {
...
function mountScreenRecordMediaServices(): void {
    const screenRecordMediaDeps = {
        ...
        readSettings: async () => {
            ...
            if (mainWindow && !mainWindow.isDestroyed()) {
                try {
                    json = await mainWindow.webContents.executeJavaScript(`localStorage.getItem('mtbot-assistant-settings')`);
                }
                catch {
                    json = null;
                }
        ...
        generateAudioFile: async (text: string, destDir: string) => {
            if (!voiceCallService)
                throw new Error('语音服务未初始化，请稍后再试');
...
async function initAgentRuntime(): Promise<void> {
    ...
    agentRuntimeBridge = new AgentRuntimeBridge({
        ...
        getAllowedRoots: () => (configManager?.getAppConfig().codingDevProjects ?? []).map((p) => p.realPath),
        ...
        getSkills: async () => {
            ...
            for (const s of installed) {
                ...
                if (effectiveScope)
                    entry.activationScope = effectiveScope;
                results.push(entry);
            }
        ...
        updateUserMemory: async (content: string) => {
            ...
            if (reconciled.removed > 0 || reconciled.added > 0) {
                log.info(`[updateUserMemory] 条目对账：新增 ${reconciled.added}、沿用 ${reconciled.kept}、删除 ${reconciled.removed}`);
            }
        ...
        sendFeishuMessage: async (text: string) => {
            if (!feishuLoginService)
                return { ok: false, error: '飞书服务未初始化' };
            return feishuLoginService.pushText(text);
        },
        getChannelRouter: () => channelHub?.router ?? null,
        generateVoiceFile: async (text: string, opts?: {
            speaker?: string;
            speed?: number;
        }) => {
            if (!voiceCallService)
                throw new Error('语音通话服务未初始化');
        ...
        forkAgent: async (systemAgentId: string, data: {
            name?: string;
            description?: string;
        }) => {
            try {
    ...
    voiceCallService = new VoiceCallService(mainWindow!, (sessionKey, content, audioWavBase64) => submitVoiceTranscript(sessionKey, content, audioWavBase64), voiceModelManager, savedVoiceConfig);
...
let promptStyleSettingsCache: {
    style: PromptStyleValue;
} | null = null;
/**
 * 同步系统提示词风格到主进程缓存（渲染进程切换时 IPC 调用；实验功能）
 */
function setPromptStyleSettingsCache(settings: {
    style?: PromptStyleValue;
}): void {
...
async function getRendererSettings(): Promise<{
    ...
    try {
        const json = await mainWindow.webContents.executeJavaScript(`localStorage.getItem('mtbot-assistant-settings')`);
        if (json) {
...
async function initialize(): Promise<void> {
    ...
    cloudSyncManager.setOnConflictDetected((conflict) => {
        ...
        void agentRuntimeBridge.executeSyncConflictGoal().then((summary) => {
            if (summary)
                log.info(`[CloudSync] 冲突 Agent 处理完成: ${summary}`);
        }, (err) => {
    ...
    syncScheduler?.setOnConflictPending(async () => {
        if (!agentRuntimeBridge?.isInitialized)
            return;
    ...
    setImmediate(() => {
        void (async () => {
            ...
            if (weixinLoginService) {
                ...
                weixinChannelAdapter = new WeixinChannelAdapter(weixinLoginService!, agentRuntimeBridge!, weixinAcpBackendManager, weixinReplyContextStore);
...
async function performCleanup(): Promise<void> {
    ...
    try {
        ...
        try {
            ...
            if (n > 0)
                log.info(`[performCleanup] 已回收 ${n} 个残留技能子进程`);
        }
        catch (err) {
