...
import { readSoulFile, readUserMemoryFile, writeSoulFile, writeUserMemoryFile, } from '../ipc/plugin-ipc';
...
import { newsFeedAgentId, newsFeedConversationId, newsFeedConversationTitle, resolveNewsFeedJob, } from '../news-feed-job';
import { diffFindings, listMaintenanceReports } from '../maintenance-report-store';
import { readActiveDashboardFeedSnapshot, readActiveDashboardFeedId, readDashboardFeedMeta, readDashboardFeedPage, readDashboardFeedBatches, ensureDashboardFeedMigrated, setActiveDashboardFeedId, } from '../dashboard-feed-store';
import { listAgents, getAgentRecord, forkAgentRecord, updateAgentRecord, deleteAgentRecord, } from '../agents-repo';
...
export function registerApiIpcHandlers(): void {
    if (!deps)
        throw new Error('ApiIpc deps not set');
    ...
    ipcMain.handle('api:updateSoulContent', async (_event, content: string) => {
        try {
            ...
            if (!result)
                return { success: false, error: '写入 AI 灵魂失败' };
    ...
    ipcMain.handle('api:updateUserMemory', async (_event, content: string) => {
        ...
        if (!result)
            return { success: false, error: '写入个人记忆失败' };
        return { success: true, data: result };
    });
     ...
     */
    ipcMain.handle('dashboard-feed:page', async (_event, feedId: string, opts?: {
        limit?: number;
        before?: {
            timestamp: number;
            id: string;
     ...
     */
    ipcMain.handle('dashboard-feed:batches', async (_event, feedId: string, opts?: {
        limit?: number;
        before?: {
            createdAt: string;
            id: string;
    ...
    ipcMain.handle('dashboard-feed:refresh', async () => {
        try {
            ...
            if (!agentRuntimeBridge)
                throw new Error('Agent Runtime 未就绪');
     ...
     */
    ipcMain.handle('api:forkAgent', async (_event, systemAgentId: string, data: {
        name?: string;
        description?: string;
    }) => {
        try {
    ...
    ipcMain.handle('api:setSearchConfig', async (_event, searchConfig: {
