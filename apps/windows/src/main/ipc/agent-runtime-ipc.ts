...
import { handleCronCreate, handleCronList, handleCronDelete, handleCronUpdate, handleCronRun, handleCronRuns, } from './agent-runtime/cron-commands';
import { handleSkillConfirmDraft, handleSkillRejectDraft, handleSkillDeprecate, } from './agent-runtime/skill-commands';
import { handleToolEvolutionList, handleToolEvolutionConfirm, handleToolEvolutionReject, handleToolEvolutionSetEnabled, handleToolEvolutionRemove, handleToolEvolutionSimulate, handleToolEvolutionMine, handleToolEvolutionGetEnabled, handleToolEvolutionSetFeatureEnabled, handleToolEvolutionSetTriggerThreshold, handleToolEvolutionStats, } from './agent-runtime/tool-commands';
import { handleCodingDevSetBackend, handleCodingDevGetBackend, handleCodingDevSetProject, handleCodingDevGetDevContext, handleCodingDevListBackends, setAcpBackendManagerGetter, } from './agent-runtime/coding-dev-commands';
import { handleRuntimePing, handleRuntimeFeatureFlagsGet, handleRuntimeEnabled, handleRuntimeModelCatalogSet, } from './agent-runtime/runtime-commands';
import { handleConversationCreate, handleConversationClose, handleConversationList, handleConversationDelete, handleConversationRename, handleConversationPinToggle, handleConversationMessages, handleConversationContextUsage, handleConversationDismissInterrupt, handleConversationContinueInterrupted, handleConversationTransferAgent, handleConversationFork, setConversationDependencies, } from './agent-runtime/conversation-commands';
import { handleStorageStats, handleStorageExportJsonl, handleStorageClearMalformed, handleStorageListBackups, handleStorageCreateBackup, handleStorageRestoreBackup, handleStorageRestoreLatestBackup, handleStorageDeleteBackup, handleStorageAuditRecent, } from './agent-runtime/storage-commands';
import { handleAgentDefinitionsList, handleAgentMemoriesList, handleAgentMemoriesDelete, handleAgentMemoriesUpdate, handleAgentMemoriesClear, handleAgentMemoriesExport, handleAgentMemoriesProvenance, handleAgentMemoriesSearch, handleAgentMemoriesArchiveCold, handleAgentMemoriesUnarchive, handleAgentMemoriesRebuildIndex, handleAgentMemoriesStats, handleAgentDefinitionSyncStatus, handleAgentDefinitionSyncUserAgents, handleAgentInstancePrompt, handleAgentInstanceAbort, handleAgentInstanceDestroy, handleAgentInstanceList, } from './agent-runtime/agent-commands';
import { handleWikiInboxList, handleWikiInboxCount, handleWikiInboxRetry, handleWikiInboxDiscard, handleWikiInboxOrganize, handleWikiFolderScan, handleWikiFolderImport, handleWikiOrganizeRun, handleWikiSearch, handleWikiSourceGet, handleWikiRunsList, handleWikiIndexRebuild, handleWikiTopicTreeGet, handleWikiTopicTreeSet, handleWikiTopicTreeMigrate, handleWikiTopicMutate, handleWikiReclassifyRun, handleWikiReclassifyEstimate, handleWikiReclassifyGet, handleWikiReclassifyApply, handleWikiReclassifyIgnore, handleWikiReclassifyDiscard, handleWikiReclassifyCancel, handleWikiMigrateGet, handleWikiMigrateApply, handleWikiMigrateCancel, handleWikiMigrateDiscard, handleWikiMigrateUndo, handleWikiMigrateReplan, handleWikiMigrateUpdateMapping, handleWikiSourceCreateNote, handleWikiSourceRename, handleWikiSourceList, handleWikiSourceCounts, handleWikiSourceUpdateTopic, handleWikiSourceMoveToParking, handleWikiSourceOpen, handleWikiSourceClearTopic, handleWikiLinkAdd, handleWikiLinkSave, handleWikiVaultEnsureLayout, handleWikiAutoClassifyGet, handleWikiAutoClassifySet, handleWikiCleanupScan, handleWikiSourceArchive, handleWikiSourceRestore, handleWikiSourceDelete, handleWikiExport, handleWikiVectorRebuild, handleWikiSourceSummary, } from './agent-runtime/wiki-commands';
import { handleAutonomousStatus, handleAutonomousGoalsList, handleAutonomousGoalsApprove, handleAutonomousGoalsReject, handleAutonomousCapabilities, handleAutonomousReflections, handleAutonomousSatisfactionHistory, handleAutonomousPromptVariants, handleAutonomousEnable, handleAutonomousDisable, handleAutonomousGetSettings, handleAutonomousUpdateSettings, handleAutonomousReflect, } from './agent-runtime/autonomous-commands';
import { handleToolsList, handleToolsToggle, handleToolsUsageByAgent, handleToolsUsageExport, handleMcpStatus, handleMcpUpsert, handleMcpImport, handleMcpRemove, handleMcpSetEnabled, handleMcpSetSessionEnabled, handleMcpSessionDisabled, handleSkillSetSessionEnabled, handleSkillSessionDisabled, handleMcpReconnect, handleMcpReadConfigFile, handleMcpWriteConfigFile, } from './agent-runtime/tools-and-mcp-commands';
import { handleNewsPreferencePreview } from './agent-runtime/news-commands';
import { handleFilesList, handleFilesSearch, handleFilesDelete, handleFilesOpen, handleFilesSaveAs, handleFilesReadPreviewContent, handleFilesReadPreviewByPath, handleFilesImport, setFilesDependencies, } from './agent-runtime/files-commands';
import { handleUserSend, handleUserSteer, handleUserAbort, handleUserPermissionRespond, handleUserAskUserRespond, handleUserAutoApproveSet, handleUserCompactContext, handleUserAbortCompactContext, setUserDependencies, } from './agent-runtime/user-commands';
import { handleHandoffConfirm } from './agent-runtime/handoff-commands';
import { handleSessionPreferredModelSet, handleSessionPreferredModelPrime, handleSessionThinkingPrefsSet, handleSessionThinkingPrefsSetGlobal, handleMessageDelete as handleMiscMessageDelete, handleMessageEdit as handleMiscMessageEdit, handleMessageEditAndResend, handleTasksList, handleCommandsList, handleImageRecognize as handleMiscImageRecognize, handleImageGenerate as handleMiscImageGenerate, handleImageProcess as handleMiscImageProcess, setMiscDependencies, } from './agent-runtime/misc-commands';
...
export function invalidateAgentInstancesForProviderChange(): void {
    ...
    for (const [sessionKey, instanceId] of sessionToInstance) {
        if (!deferredIds.has(instanceId))
            sessionToInstance.delete(sessionKey);
    }
    log.info(`[invalidateAgentInstancesForProviderChange] 失效 ${instances.length} 个实例，其中 ${deferredIds.size} 个运行中已推迟销毁`);
}
/**
...
export function installAgentRuntimeCommandIpc(performanceMonitor?: PerformanceMonitor): void {
    ...
    const commandHandler = async (_event: Electron.IpcMainInvokeEvent, command: AgentRuntimeCommand): Promise<unknown> => {
        if (!ipcBridgeRef) {
    ...
    ipcMain.handle('agent-runtime:command', performanceMonitor
 ...
 */
export async function handleCommand(bridge: AgentRuntimeBridge, command: AgentRuntimeCommand): Promise<unknown> {
    switch (command.type) {
        // ---- 用户交互 ----
        case 'user:send':
        ...
        case 'wiki:topic:tree:migrate':
            return handleWikiTopicTreeMigrate(bridge, command);
        ...
        default: {
            ...
            throw new Error(`Unknown command type: ${(command as {
                type?: string;
            }).type}`);
        }
    }
...
function parseAtScheduleExprLite(rawExpr: string): number | undefined {
    ...
    if (direct !== undefined) {
        return direct > 0 && direct < 1000000000000 ? direct * 1000 : direct;
    }
 ...
 */
async function getInstanceForSession(bridge: AgentRuntimeBridge, sessionKey: string, agentId?: string): Promise<string | undefined> {
