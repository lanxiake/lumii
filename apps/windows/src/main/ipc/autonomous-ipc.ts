...
export function setAutonomousIpcDeps(deps: {
...
function requireBridge(): AgentRuntimeBridge {
    ...
    if (!bridge)
        throw new Error('AgentRuntimeBridge 未就绪');
    return bridge;
}
...
ipcMain.handle('autonomous:getStatus', async () => {
    try {
        ...
        if (!latest)
            return { ...emptyStatus(enabled), pendingGoalsCount };
...
ipcMain.handle('autonomous:replan', async () => {
    const bridge = requireBridge();
    const agentId = subjectAgentId();
    if (agentId === NO_SUBJECT_AGENT_ID)
...
ipcMain.handle('autonomous:approveGoal', async (_event, goalId: string, note?: string) => {
    ...
    if (!ok)
        throw new Error('目标不存在或不处于 pending 状态');
    notifyAutonomousGoalApproved(goalId);
    return { success: true, goalId };
});
ipcMain.handle('autonomous:rejectGoal', async (_event, goalId: string, options?: {
    reason?: string;
}) => {
    const bridge = requireBridge();
...
ipcMain.handle('autonomous:getDiary', async (_event, limit = 20, before?: {
    timestamp: number;
    id: string;
}) => {
    const bridge = requireBridge();
    ...
    const page = bridge.conversationRepo.loadMessagesPage(bridge.evolutionConversationIdFor(agentId), {
        limit,
        before: before
    ...
    const nextBefore = page.hasMore && oldest
