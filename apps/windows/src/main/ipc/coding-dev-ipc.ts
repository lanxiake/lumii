...
import { buildCodingDevEnvInfo, defaultWorkspaceFallback, } from '../coding-dev-env.js';
import { detectLocalAcpTool, isPrimaryLocalAcpToolId, listLocalAcpToolsMetadata, needsWindowsShell, } from '../coding-dev-cli-detect.js';
import { installLocalAcpTool, previewUninstallLocalAcpTool, uninstallLocalAcpTool, } from '../coding-dev-cli-install.js';
import { createProject, openExistingProject, removeProject, reconcileProjectsWithDisk, } from '../coding-dev-projects.js';
...
export function registerCodingDevIpcHandlers(options: CodingDevIpcOptions): void {
    ...
    ipcMain.handle('app:listCodingDevProjects', async () => {
        if (!getConfigManager() || !directoryManager)
    ...
    ipcMain.handle('app:setCodingDevAgentBindings', async (_event, bindings: unknown) => {
        ...
        const sanitized = list
        ...
        } => Boolean(b && typeof b === 'object' && typeof (b as {
            agentId?: unknown;
        }).agentId === 'string' && (b as {
            agentId: string;
        }).agentId.trim()))
            ...
            backendId: (['claude', 'codex', 'cursor', 'opencode'].includes(b.backendId) ? b.backendId : 'claude') as 'claude' | 'codex' | 'cursor' | 'opencode',
    ...
    ipcMain.handle('app:removeCodingDevProject', async (_event, name: string) => {
        ...
        const activeProject = cfg.codingDevActiveProject && projects.some((p) => p.name === cfg.codingDevActiveProject)
