...
export function batchesFor(c) {
  ...
  return [
    ...
    },
    {
      ...
      // **一次性动作**：从待机进场、播完回待机，所以首末格要贴近站姿。
      // 循环动作要的是「末格接回首格」，两者对首末格的要求正好相反——
      // 早先共用一句话，等于让挥手结束在挥手的中途。
      //
      // 真正的无缝由装配保证：`build.mjs` 给这批加了 `idlePin: true`，
      // 首末格会**按名引用待机首帧**（见 pet-core 的 `model/idle-pin.ts`）。
      // 这里只是让模型把中间几格画对。
      loopMode: 'idle_pin',
      // **这条留在网格里**：横条对「抬手」这种横向动作不够宽。实测 4×1 时每格 313px，
      // 角色本身就 313 宽（手臂一抬就撑满整格），S1 报 0.83% 越界；6×1 更差（209px，7.77%）。
      // 待机那种纵向微动适合横条，挥手需要 627px 的方格子。
      cols: 2,
      rows: 2,
    },
    isMecha
