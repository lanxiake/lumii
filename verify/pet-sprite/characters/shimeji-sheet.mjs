...
export const GROUPS = [
  {
    ...
    from: 'REST',
    kind: 'loop',
    // ⚠️ 待机**不能**用 9fps（2026-09-21）。
    //
    // REST 行有 8 帧。参考项目把这一行当 GREET（打招呼）用、`loop = false`，
    // 9fps 下 0.89 秒走完——**那是一次性快动作的正常速度**。我们把它接成待机之后，
    // 待机就以"打招呼"的速度无限循环，肉眼看着就是在原地急促地动。
    //
    // 3fps → 2.67 秒一轮，是待机该有的呼吸节奏（8 帧里本来就只有 5 个不同姿势，
    // 成对重复，慢放才看得出层次）。
    fps: 3,
