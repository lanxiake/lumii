...
- 「体检」是读 + 判断，不是改：先用 maintenance_report_read 看上一期发现了什么，再按手册 §0 的顺序走一遍四类资产，逐项给出「问题 / 依据 / 建议」。
- **结束必须调 maintenance_report_write 落库**（手册 §4 给了字段格式）。只在对话里回复等于没报告：概览页看不到，系统也没法对比出「上期的问题这次还在不在」。findings 的 key 要跨期稳定，checked 要写清查过且没问题的项。
- 维护动作分两类：
