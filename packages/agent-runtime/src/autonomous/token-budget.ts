 ...
 *
 * ---------------------------------------------------------------------------
 * ⚠ `maxTokensPerDay` 是**每个 agent 一份**，不是全机合计（2026-09-24 明确）
 * ---------------------------------------------------------------------------
 * 分键之后没有任何跨 agent 汇总，所以 N 个自主 agent + 1 只宠物一天的上限是
 * `N × maxTokensPerDay`（宠物另有自己的绝对闸门 `MAX_PET_TOKENS_PER_DAY`，两者谁先到算谁）。
 * 这是**刻意的**，不是漏做汇总：分键要解决的正是"共用一个计数器"——
 * 汇总回去等于把「宠物跑一次，助手当天少跑一个」原样搬回来，T3.2 就白做了。
 *
 * 代价是设置页那个数字的含义从「全机」变成了「每个 agent」。**文案必须跟着改**
 * （`AutonomousPage` 的 label 已改为「每个 Agent 每日 Token 上限」）——
 * 一个用户以为管全机、实际只管单份的限额，比没有限额更糟：他会按 100k 估自己的用量，
 * 而机器按 N×100k 烧。
 */
