...
export function buildExtractionPrompt(
  ...
  return [
    ...
    "| feedback | 个人记忆 | user_memory Markdown | 任何场景都成立的交互风格纠正与确认 |",
    ...
    "",
    "**关键区分**：",
    "- 个人记忆 = 「你是谁、你喜欢怎样被对待」→ **任何场景都成立**、变化慢",
    "- 工作记忆 = 「你在做什么、用什么资源」→ 与具体项目/Agent 绑定、变化快",
    "- 全局偏好（如「回复要简洁」「用 image_generate 生图」）→ feedback",
    "- **仅对某个项目/技能/渠道成立的偏好（如「某仓库用 pnpm」「绘本技能勿自动全屏」「微信回复要简短」）→ 不要提取为任何类别候选**：它属于场景记忆，由 agent 用 scene_memory 工具写入；提取流程无法确定项目归属，提取出来只会污染全局记忆",
    "",
    "## 记忆类型与保存时机",
    "",
    "### user（用户画像）→ 个人记忆",
    ...
    "3. **场景内容不提取**：仅对特定项目/技能/渠道成立的约束不要输出为候选（由 scene_memory 管理）；其余全局偏好无需标注范围",
