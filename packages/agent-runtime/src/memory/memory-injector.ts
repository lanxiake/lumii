 ...
 * 拿摘要凑答案。现在把触发条件写具体：
 * - 读：用户问细节/原话/完整过程，或与当前说法冲突需核实
 * - 不读：只是复述结论、或本轮话题与这条无关
 *
 * 同时点明**可以直接读**：旧文案没说这个，模型每次都先多搜一轮 memory_search
 * 才敢 memory_read（2026-09-18 实测）。指针的意义就是省掉那一轮。
 */
const POINTER_LEGEND = [
  "- 行首 `[d:xxxx]` 是这条记忆的**原文指针**，可直接交给 `memory_read` 读全文，**不需要先 memory_search**。",
  "- 用户问的是**细节、原话、完整过程、当时的取舍**时，条目里的摘要答不全——先读原文再答。只是复述结论、或这条与本轮无关时，不必读。",
