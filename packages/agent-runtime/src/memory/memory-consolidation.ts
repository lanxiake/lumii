...
export function buildMemoryConsolidationPrompt(
  ...
  return [
    ...
    "## 整理任务",
    "",
    "### 个人记忆 vs 场景记忆 vs 工作记忆（写入边界）",
    "- **个人记忆**（本任务输出）：user（身份/偏好/习惯）+ feedback（交互风格纠正与确认），且必须**任何场景都成立**",
    "- **场景记忆**（不写入本任务）：仅对某个项目/技能/渠道成立的内容（如「使用 picture-book-studio 时…」「某仓库用 pnpm」「微信回复要简短」）→ 由 scene_memory 工具管理",
    "- **工作记忆**（不写入本任务）：project（进行中项目）+ reference（工具/资源）→ 由另一路径写入 SQLite",
    "- 项目进度类内容（如 K8s 系列写到第几篇）应删除或压缩，留给工作记忆；个人记忆只保留全局偏好",
    "- **核心判据：换个项目/渠道还成立吗？** 不成立的条目一律从本文件删除（其内容由 agent 迁移到场景记忆）",
    "",
    "### 去重规则",
    ...
    "### 冲突消解规则（优先级从高到低）",
    "1. **最近用户明确陈述** > 历史记忆",
    "2. **全局偏好** > **任务级规则**；任务级规则（仅对某项目/技能/渠道成立）不写入本文件，直接删除",
    "3. **工具/方法冲突**：若同时存在 generate_image.py 与 image_generate → 保留 image_generate，删除脚本方案",
    "4. **场景互斥**：不同项目的规则不得写在本文件；本文件只保留跨场景稳定的偏好",
    "",
    "### 输出格式",
    "输出整理后的完整 Markdown，推荐结构（不得新增其他一级章节）：",
    "## 基本信息",
    "## 交互偏好",
    "- 每条用 `- ` 列表项，格式为「**规则**：… / **原因**：…」",
    "- **不要写「**应用**：」**：它是规则的同义复述，实测每条都只是在重排规则的词，纯占篇幅",
    "- **每条压到 100 字以内**（规则一句话 + 原因半句）。冗长条目的价值并不会因为写长而提高，只会把注入预算吃光",
    "- 声明了具体适用范围（某项目/某渠道）的条目**直接删除**，本文件只保留跨场景稳定的偏好；全局偏好不必写「适用范围」",
