...
export async function normalizeBrowserScreenshot(
  ...
  const imageOps = await getImageOps();
  if (!imageOps) {
    return { buffer };
  }

  const meta = await imageOps.getImageMetadata(buffer);
  ...
  for (const side of sideGrid) {
    for (const quality of qualities) {
      const out = await imageOps.resizeToJpeg({
        buffer,
        maxSide: side,
        quality,
