...
// 扫描 ~/.cloakbrowser/chromium-* 目录，返回版本最新的 CloakBrowser 可执行文件
export function findCloakBrowserExecutable(): BrowserExecutable | null {
  ...
  try {
    entries = fs.readdirSync(cloakDir, { withFileTypes: true });
  } catch {
    return null;
  }
  ...
  // 收集所有 chromium-* 子目录下的可执行文件
  const candidates: Array<{ version: string; exePath: string }> = [];
  for (const entry of entries) {
    if (!entry.isDirectory()) continue;
    if (!entry.name.startsWith("chromium-")) continue;

    const exePath = path.join(cloakDir, entry.name, exeName);
    if (isExecutable(exePath)) {
      candidates.push({ version: entry.name, exePath });
    }
  }
  ...
  // 按目录名降序排列，取最新版本（chromium-v146 > chromium-v135）
  candidates.sort((a, b) => b.version.localeCompare(a.version, undefined, { numeric: true }));

  return { kind: "chromium", path: candidates[0].exePath };
}
...
function isExecutable(filePath: string): boolean {
  ...
  if (process.platform === "win32") {
    ...
    return [".exe", ".cmd", ".bat"].includes(ext);
  }
...
export function resolveBrowserExecutableForPlatform(
  ...
  // CloakBrowser 反检测 Chromium 优先（C++ 源码级补丁，reCAPTCHA v3 得 0.9 分）
  const cloakBrowser = findCloakBrowserExecutable();
  if (cloakBrowser) {
    return cloakBrowser;
  }
