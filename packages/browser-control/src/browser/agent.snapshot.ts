...
export function registerBrowserAgentSnapshotRoutes(
  ...
  app.post("/pdf", async (req, res) => {
    ...
    try {
      ...
      await mediaStore1.ensureMediaDir();
      const saved = await mediaStore1.saveMediaBuffer(
        pdf.buffer,
        "application/pdf",
        "browser",
  ...
  app.post("/screenshot", async (req, res) => {
    ...
    try {
      ...
      await mediaStore2.ensureMediaDir();
      const saved = await mediaStore2.saveMediaBuffer(
        normalized.buffer,
    ...
    try {
      ...
      if (format === "ai") {
        ...
        if (labels) {
          ...
          await mediaStore3.ensureMediaDir();
          const saved = await mediaStore3.saveMediaBuffer(
            normalized.buffer,
