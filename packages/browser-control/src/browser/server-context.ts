...
function createProfileContext(
  ...
  const ensureBrowserAvailable = async (): Promise<void> => {
    ...
    if (isExtension) {
      ...
      throw new Error(
        `Chrome extension relay is running, but no tab is connected. Click the MtBot Chrome extension icon on a tab to attach it (profile "${profile.name}").`,
      );
    }
    ...
    if (!httpReachable) {
      ...
      const launched = await launchMtBotChrome(current.resolved, profile);
    ...
    if (!profileState.running) {
      throw new Error(
        `Port ${profile.cdpPort} is in use for profile "${profile.name}" but not by mtbot. ` +
    ...
    await stopMtBotChrome(profileState.running);
    setProfileRunning(null);

    const relaunched = await launchMtBotChrome(current.resolved, profile);
      ...
      if (profile.driver === "extension") {
        throw new Error(
          `tab not found (no attached Chrome tabs for profile "${profile.name}"). ` +
            "Click the MtBot Browser Relay toolbar icon on the tab you want to control (badge ON).",
        );
      }
  ...
  const resetProfile = async () => {
    ...
    if (httpReachable && !profileState.running && typeof profile.cdpPort === "number") {
      ...
      try {
        ...
        const usage = await inspectPortUsage(profile.cdpPort);
        for (const listener of usage.listeners ?? []) {
          if (listener.pid && listener.pid > 0) {
            console.info(
              `[browser/resetProfile] killing pid=${listener.pid} occupying port ${profile.cdpPort} (profile "${profile.name}")`,
            );
        ...
        console.warn(
          `[browser/resetProfile] failed to kill port ${profile.cdpPort} occupant: ${String(err)}`,
        );
      }
      // Also disconnect any lingering Playwright connection
      try {
