...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".mtbot-profile-decorated");
}
 ...
 */
export function decorateMtBotProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_MTBOT_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_MTBOT_BROWSER_COLOR).toUpperCase();
