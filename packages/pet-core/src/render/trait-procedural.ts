 ...
 * ## ⚠️ `bob` / `sway` 在待机组上到不了
 *
 * 本层作用在**参数**上，而 `sprite-runtime.ts` 的 `stripIdleDrift` 在**清单解析期**
 * 就把 `bob` 与 `sway` 从任何名字带 `idle` 的组里删掉了（只留 `breathe` 与眨眼）。
 * 用户 2026-09-24 拍板保留那条规则（"待机确实不该浮动"），所以：
 *
 * - 这两个倍率**只在非待机组上生效**（模型在 `Talk` 之类上声明了 `bob` 才有落点）；
 * - **别把它们算进"静止时看得出来"的证据里**——实测过，宠物在待机时一动不动，
 *   而日志里的倍率却是对的（`[setProceduralScales] bob=1.68` 与手算逐位一致）。
 *   这种"数值对、效果无"最难查，所以写在这里而不是留给下一个人去踩。
 *
